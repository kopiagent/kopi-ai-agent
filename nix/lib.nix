      ...
      let
        ...
        excludedDirs =
          ...
          ++ [
            ...
            "nix"
            # Skills are shipped via KOPI_BUNDLED_SKILLS /
            # KOPI_OPTIONAL_SKILLS (see kopi-ai-agent.nix), not via the
            ...
            # __init__.py) shipped via symlinks + KOPI_BUNDLED_LOCALES
            # / KOPI_OPTIONAL_MCPS, not via the wheel. Excluding them
...
in
{
  ...
  #
  # Usage:
  #   npm = kopiNpmLib.mkNpmPassthru { dirs = [ "ui-tui" ]; };
  #   npm = kopiNpmLib.mkNpmPassthru { dirs = [ "apps/desktop" "apps/shared" ]; };
