...
:::tip 最快路径：KOPI Proxy
[KOPI Proxy](/user-guide/features/tool-gateway) 在单一订阅下提供 300+ 个模型。全新安装后，运行 `kopi setup --portal` 即可登录并一键将 Nous 设为提供商。使用 `kopi portal info` 查看当前配置。
:::
...
- **右列** — 所选提供商的精选模型列表。这些是 Kopi 针对该提供商推荐的 agentic 模型，而非原始的 `/models` 接口返回结果（OpenRouter 的原始列表包含 400+ 个模型，涵盖 TTS、图像生成器和重排序器）。

在过滤框中输入提供商名称、slug 或模型 ID 进行筛选。

选择模型后点击 **Switch**，Kopi 会将其写入 `~/.kopi/config.yaml` 的 `model` 部分。**此操作仅对新会话生效** — 已打开的聊天标签页将继续使用启动时的模型。如需在当前聊天中热切换，请在聊天内使用 `/model` 斜杠命令。
...
每个辅助任务默认为 `auto`，即 Kopi 对该任务也使用主模型。当某个边缘任务需要更便宜或更快的模型时，可单独覆盖该槽位。
...
通过仪表板保存时，Kopi 写入 `~/.kopi/config.yaml`：
...
`provider: auto` 加 `model: ''` 表示 Kopi 对该任务使用主模型。
...
Kopi 仅列出具有有效凭据的提供商。检查侧边栏中的 **Keys** — 应存在以下之一：API key、成功的 OAuth 或自定义端点 URL。若所需提供商不在列表中，运行 `kopi setup` 进行配置，或前往 **Keys** 添加环境变量。
