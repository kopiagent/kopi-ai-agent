...
description: 通过 Home Assistant 集成，使用 Kopi Agent 控制您的智能家居。
...
Kopi Agent 通过以下两种方式与 [Home Assistant](https://www.home-assistant.io/) 集成：
...
Agent 发出的消息将以 **Home Assistant 持久通知**的形式推送（通过 `persistent_notification.create`），标题为"Kopi Agent"，显示在 HA 通知面板中。
