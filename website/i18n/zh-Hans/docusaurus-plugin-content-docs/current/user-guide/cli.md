...
sidebar_position: 1
title: "CLI 界面"
description: "掌握 Kopi Agent 终端界面——命令、快捷键、人格设定等"
---

# CLI 界面

Kopi Agent 的 CLI 是一个完整的终端用户界面（TUI），而非 Web UI。它支持多行编辑、斜杠命令自动补全、对话历史、中断并重定向，以及流式工具输出。专为常驻终端的用户而生。

:::tip
Kopi 还提供了一个现代 TUI，支持模态覆盖层、鼠标选择和非阻塞输入。使用 `kopi --tui` 启动——参见 [TUI](tui.md) 指南。
:::
...
<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Kopi CLI 布局的风格化预览，展示了横幅、对话区域和固定输入提示符。" />
<p className="docs-figure-caption">Kopi CLI 横幅、对话流和固定输入提示符，以稳定的文档图示形式呈现，而非脆弱的文字艺术。</p>
...
| `Ctrl+D` | 退出 |
| `Ctrl+Z` | 将 Kopi 挂起到后台（仅 Unix）。在 shell 中运行 `fg` 恢复。 |
...
输入 `/` 查看自动补全下拉菜单。Kopi 支持大量 CLI 斜杠命令、动态 skill 命令和用户自定义快捷命令。
...
Kopi 会在第一轮对话前将每个指定的 skill 加载到会话 prompt 中。该标志在交互模式和单次查询模式下均有效。
...
大多数终端默认对 `Enter` 和 `Shift+Enter` 发送相同的字节序列，因此应用程序无法区分它们。Kopi 仅在终端通过 [Kitty 键盘协议](https://sw.kovidgoyal.net/kitty/keyboard-protocol/)或 xterm 的 `modifyOtherKeys` 模式发送不同序列时才能识别 `Shift+Enter`。
...
当终端无法区分时，`Alt+Enter` 和 `Ctrl+J` 在所有终端中均可正常使用。**特别是在 Windows Terminal 中，`Alt+Enter` 被终端捕获（切换全屏），永远不会传递给 Kopi——请直接使用 `Ctrl+Enter`（传递为 `Ctrl+J`）或 `Ctrl+J` 来换行。**
...
:::tip 首次提示
第一次在 Kopi 工作时按下 Enter，Kopi 会打印一行提示，说明 `/busy` 选项（`"(tip) Your message interrupted the current run…"`）。每次安装只触发一次——`config.yaml` 中 `onboarding.seen.busy_input_prompt` 下的标志会锁定它。删除该键可再次看到提示。
:::

### 挂起到后台

在 Unix 系统上，按 **`Ctrl+Z`** 将 Kopi 挂起到后台——与任何终端进程一样。shell 会打印确认信息：

```
Kopi Agent has been suspended. Run `fg` to bring Kopi Agent back.
```
...
CLI 会话存储在 Kopi 的 SQLite 状态数据库 `~/.kopi/state.db` 中。数据库保存：
