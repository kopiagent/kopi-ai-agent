...
通过 AgentMail 为 Agent 提供专属电子邮件收件箱。使用 Agent 专属电子邮件地址（例如 kopi-agent@agentmail.to）自主发送、接收和管理电子邮件。
...
### 创建收件箱并发送邮件
1. 创建专属收件箱：
   - 使用 `create_inbox` 并指定用户名（例如 `kopi-agent`）
   - Agent 获得地址：`kopi-agent@agentmail.to`
2. 发送邮件：
