...
Kopi 以后台子进程方式运行完整的语言服务器——pyright、gopls、rust-analyzer、
...
这与顶级编码 agent 所采用的架构相同。Kopi 将其作为自包含组件提供：
...
Kopi 会自动检测 PATH 上或 `<KOPI_HOME>/lsp/bin/` 中的二进制文件。
...
当 `install_strategy: auto` 时，Kopi 将二进制文件安装到 `<KOPI_HOME>/lsp/bin/`。
...
服务器在 Kopi 进程的整个生命周期内保持运行。没有空闲超时回收机制——每次写入都
