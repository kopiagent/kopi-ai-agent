...
:::tip 一个后端同时覆盖模型与工具
Kopi 本身需要配置好 provider（提供商）和工具后端，API 服务器才能发挥作用。[KOPI Proxy](/user-guide/features/tool-gateway) 订阅同时处理两者——300+ 个模型，以及通过 Tool Gateway 提供的网络/图像/TTS/浏览器功能。在启动 API 服务器之前运行一次 `kopi setup --portal`，Open WebUI 或 LobeChat 等前端即可获得一个完整配备工具的后端。
:::
...
**流式传输**（`"stream": true`）：返回逐 token 响应块的 Server-Sent Events（SSE）。对于 **Chat Completions**，流使用标准 `chat.completion.chunk` 事件，以及 Kopi 自定义的 `kopi.tool.progress` 事件用于工具启动的 UX 展示。对于 **Responses**，流使用 OpenAI Responses 事件类型，如 `response.created`、`response.output_text.delta`、`response.output_item.added`、`response.output_item.done` 和 `response.completed`。

**流中的工具进度：**
- **Chat Completions**：Kopi 发出 `event: kopi.tool.progress` 以提供工具启动可见性，同时不污染持久化的 assistant 文本。
- **Responses**：Kopi 在 SSE 流期间发出符合规范的 `function_call` 和 `function_call_output` 输出项，让客户端能够实时渲染结构化工具 UI。
...
在集成仪表板、浏览器 UI 或控制平面时使用此端点，以便它们能够发现当前运行的 Kopi 版本是否支持 runs、流式传输、取消和 session 连续性，而无需依赖私有 Python 内部实现。
...
Runs 接受简单的 `input` 字符串，以及可选的 `session_id`、`instructions`、`conversation_history` 或 `previous_response_id`。当提供 `session_id` 时，Kopi 会在 run 状态中暴露它，以便外部 UI 将 run 与自己的对话 ID 关联。
...
中断正在运行的 agent 轮次。端点立即返回 `{"status": "stopping"}`，同时 Kopi 要求活跃 agent 在下一个安全中断点停止。
...
通过 `API_SERVER_KEY` 环境变量配置密钥。如果需要浏览器直接调用 Kopi，还需将 `API_SERVER_CORS_ORIGINS` 设置为明确的允许列表。
...
要为多个用户提供各自隔离的 Kopi 实例（独立的配置、记忆、技能），请使用 [profiles](/user-guide/profiles)：
...
在 Open WebUI 中，将每个添加为单独的连接。模型下拉列表显示 `alice` 和 `bob` 作为不同模型，每个均由完全隔离的 Kopi 实例支持。详见 [Open WebUI 指南](/user-guide/messaging/open-webui#multi-user-setup-with-profiles)。
...
API 服务器还作为 **gateway 代理模式**的后端。当另一个 Kopi gateway 实例配置了指向此 API 服务器的 `GATEWAY_PROXY_URL` 时，它会将所有消息转发到这里，而不是运行自己的 agent。这支持分离部署——例如，一个处理 Matrix E2EE 的 Docker 容器将请求中继到宿主机侧的 agent。
