...
sidebar_position: 17
title: "扩展 Dashboard"
description: "为 Kopi Web Dashboard 构建主题和插件——调色板、字体排版、布局、自定义标签页、shell 插槽、页面级插槽以及后端 API 路由"
---

# 扩展 Dashboard

Kopi Web Dashboard（`kopi dashboard`）在设计上支持换肤和扩展，无需 fork 代码库。对外暴露三个层次：
...
| 主题 | 调色板 | 字体排版 | 布局 |
|-------|---------|------------|--------|
| **Kopi Teal**（`default`） | 深青色 + 奶油色 | 系统字体栈，15px | 0.5rem 圆角，comfortable |
| **Kopi Teal (Large)**（`default-large`） | 同 default | 系统字体栈，18px，行高 1.65 | 0.5rem 圆角，spacious |
...
引用 Google Fonts 的主题（除 Kopi Teal 外均如此）会按需加载样式表——首次切换时会向 `<head>` 注入一个 `<link>` 标签。
...
Dashboard 插件是一个包含 `manifest.json`、预构建 JS bundle，以及可选的 CSS 文件和带 FastAPI 路由的 Python 文件的目录。插件与其他 Kopi 插件一起存放在 `~/.kopi/plugins/<name>/`——dashboard 扩展是该插件目录内的 `dashboard/` 子文件夹，因此一个插件可以从单次安装中同时扩展 CLI/gateway 和 dashboard。
...
参考插件（[`kopi-example-plugins`](https://github.com/NousResearch/kopi-example-plugins/tree/main/example-dashboard) 中的 `example-dashboard`）提供了一个向 `sessions:top` 注入横幅的实时演示——安装它可端到端了解该模式。
...
后端路由在 dashboard 进程内运行，因此可以直接从 kopi-agent 代码库导入：
...
[`strike-freedom-cockpit`](https://github.com/NousResearch/kopi-example-plugins/tree/main/strike-freedom-cockpit) 插件（伴随仓库 `kopi-example-plugins`）是一个完整的换肤演示。它将主题 YAML 与仅插槽插件配对，在不 fork dashboard 的情况下生成驾驶舱风格的 HUD。
...
```bash
git clone https://github.com/NousResearch/kopi-example-plugins.git
...
打开 dashboard，从主题切换器中选择 **Strike Freedom**。驾驶舱侧边栏出现，徽标显示在顶栏，标语替换底栏。切换回 **Kopi Teal**，插件仍然安装但不可见（`sidebar` 插槽仅在 `cockpit` 布局变体下渲染）。
