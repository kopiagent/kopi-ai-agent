...
Kopi Agent 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。
...
**配置：** `$KOPI_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$KOPI_HOME/honcho.json` > `~/.kopi/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/kopi-ai/kopi-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/kopi)。
...
Honcho 将对话建模为 peer 之间的消息交换——每个 Kopi profile 对应一个用户 peer 加一个 AI peer，共享同一个 workspace。workspace 是共享环境：用户 peer 在各 profile 间全局共享，每个 AI peer 拥有独立身份。每个 AI peer 从自身的观察中独立构建表示/card，因此 `coder` profile 保持代码导向，而 `writer` profile 针对同一用户保持编辑导向。

映射关系：

| 概念 | 含义 |
|---------|-----------|
| **Workspace** | 共享环境。同一 workspace 下的所有 Kopi profile 共享同一用户身份。 |
| **用户 peer**（`peerName`） | 人类用户。在 workspace 内跨 profile 共享。 |
| **AI peer**（`aiPeer`） | 每个 Kopi profile 一个。host key `kopi` → 默认；其他 profile 使用 `kopi.<profile>`。 |
...
`--clone` 在 `honcho.json` 中创建一个 `kopi.coder` host 块，包含 `aiPeer: "coder"`、共享的 `workspace`、继承的 `peerName`、`recallMode`、`writeFrequency`、`observation` 等。AI peer 会在 Honcho 中提前创建，确保在第一条消息之前就已存在。
...
扫描所有 Kopi profile，为没有 host 块的 profile 创建 host 块，从默认 `kopi` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/kopi-ai/kopi-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/kopi)。
...
| `retain_context` | `conversation between Kopi Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/NousResearch/kopi-agent/blob/main/plugins/memory/hindsight/README.md)。
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `kopi-{identity}` → `kopi-coder`），按 Kopi profile 隔离记忆
