...
Kopi Agent 可以在**后台**驱动你的 Mac 桌面——点击、输入、滚动、拖拽。你的光标不会移动，键盘焦点不会改变，macOS 也不会切换 Spaces。你和 Agent 可以在同一台机器上协同工作。
...
3. 在提示时授予 macOS 权限：
   - **系统设置 → 隐私与安全性 → 辅助功能** → 允许终端（或 Kopi 应用）。
...
cua-driver 项目会定期发布修复（例如 v0.1.6 修复了 UTM 工作流中的 Safari 窗口焦点问题）。Kopi 在两处刷新二进制文件，避免你停留在过时版本：

- **`kopi update`** — 更新 Kopi 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
...
- **私有 SPI 风险。** Apple 可能在任何 OS 更新中更改 SkyLight 的符号接口。Kopi 始终安装最新版 cua-driver，并在已安装的二进制文件低于其测试基线版本（按操作系统分别设定）时发出警告。没有版本固定开关——如需可复现的版本，请将 `KOPI_CUA_DRIVER_CMD` 指向特定的二进制文件。
...
- [通用技能：`macos-computer-use`](https://github.com/NousResearch/kopi-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
