...
2. **Kopi 启动**一个全新的 agent 会话，使用你的 prompt（提示词）
3. **网页搜索**拉取你关注主题的最新新闻
4. **汇总**将内容提炼为简洁的简报格式
...
---
3 stories • Sources searched: 8 • Generated by Kopi Agent
```
...
在创建 cron 任务之前，请确保 Kopi 已全局配置了默认模型和 provider。如果你希望某个任务使用不同的值，可在创建时设置该任务专属的 model/provider 覆盖项。
...
Kopi 将使用统一的 `cronjob` 工具为你创建 cron 任务。
...
若要加快简报生成速度，可以告诉 Kopi 将每个主题委托给子 agent：
...
Kopi 将使用 `cronjob(action="list")` 查找任务，并使用 `cronjob(action="remove")` 将其删除。
