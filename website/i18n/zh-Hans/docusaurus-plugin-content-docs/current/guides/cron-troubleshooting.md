...
sidebar_position: 12
title: "Cron 故障排查"
description: "诊断并修复常见的 Kopi cron 问题——任务未触发、投递失败、skill 加载错误及性能问题"
---
...
`script` 路径必须为绝对路径（或相对于 Kopi 配置目录的路径）。验证：
```bash
...
3. 在 [github.com/NousResearch/kopi-agent](https://github.com/NousResearch/kopi-agent) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息
