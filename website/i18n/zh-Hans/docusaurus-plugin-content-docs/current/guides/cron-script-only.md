...
Kopi 将此称为**无 agent 模式**。这是去掉 LLM 的 cron 系统。
...
无 agent 模式的真正优势在于：agent 本身可以为你设置看门狗——无需编辑器、无需 shell、无需记忆 CLI 参数。你描述需求，Kopi 编写脚本、安排计划，并告知你何时触发。

### 示例对话

> **你：** 每 5 分钟检查一次，如果内存超过 85% 就在 telegram 通知我
>
> **Kopi：** *（写入 `~/.kopi/scripts/memory-watchdog.sh`，然后以 `no_agent=true` 调用 `cronjob(...)`）*
>
...
当你的请求类似"当 X 时告警我"或"每 N 分钟检查 Y，如果 Z 则通知我"，Kopi 的 `cronjob` 工具描述会指示它在消息内容完全由脚本决定时使用 `no_agent=True`。当请求需要推理时（*"总结新 issue"*、*"挑选最有趣的头条"*、*"起草一条友好提醒"*），则回退到普通的 LLM 驱动路径。
...
> **你：** 今晚先停掉内存看门狗
>
> **Kopi：** *（调用 `cronjob(action='pause', job_id='abc123')`）*
>
> 已暂停。说"重新开启"或执行 `kopi cron resume abc123` 可恢复。

> **你：** 改成每 15 分钟一次
>
> **Kopi：** *（调用 `cronjob(action='update', job_id='abc123', schedule='every 15m')`）*
...
| 方式 | 运行内容 | 适用场景 |
|----------|-----------|-------------|
| `cronjob --no-agent`（本页） | 你的脚本，由 Kopi 调度 | 不需要推理的周期性看门狗 / 告警 / 指标 |
| `cronjob`（默认，LLM） | 带可选预检脚本的 agent | 消息内容需要对数据进行推理时 |
| OS cron + `curl` 到 [webhook 订阅](/user-guide/messaging/webhooks) | 你的脚本，由 OS 调度 | 当 Kopi 本身可能不健康时（即被监控对象） |

对于必须在 **gateway 宕机时也能触发**的关键系统健康看门狗，请使用 OS 级 cron 配合 `curl` 调用 Kopi webhook 订阅（或任何外部告警端点）——这些作为独立 OS 进程运行，不依赖 Kopi 是否在线。当被监控对象是外部系统时，in-gateway 调度器才是正确选择。
