...
本教程将引导你搭建一个由 Kopi Agent 驱动的 Telegram 机器人，供多名团队成员使用。完成后，你的团队将拥有一个共享 AI 助手，可以向它发消息寻求代码、研究、系统管理等方面的帮助——并通过按用户授权保障安全。
...
- **已在服务器或 VPS 上安装 Kopi Agent**（不是你的笔记本——机器人需要持续运行）。如尚未安装，请参阅[安装指南](/getting-started/installation)。
- **一个 Telegram 账号**（机器人所有者）
- **已配置 LLM 提供商**——至少在 `~/.kopi/.env` 中配置了 OpenAI、Anthropic 或其他受支持提供商的 API 密钥

:::tip
一台 $5/月的 VPS 足以运行 gateway（网关）。Kopi 本身很轻量——花钱的是 LLM API 调用，而那些调用发生在远端。
:::
...
2. **发送 `/newbot`**——BotFather 会询问两件事：
   - **显示名称**——用户看到的名字（例如 `Team Kopi Assistant`）
...
完整指南请参阅[在 Kopi 中使用 SOUL.md](/guides/use-soul-with-kopi)。
