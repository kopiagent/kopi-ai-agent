---
sidebar_position: 1
title: "通过 Nous Portal 运行 Kopi Agent"
description: "完整操作指南：订阅、配置、切换模型、启用 gateway 工具并验证路由"
---

# 通过 Nous Portal 运行 Kopi Agent

本指南带你从头到尾完成在 [Nous Portal](https://portal.nousresearch.com) 订阅下运行 Kopi Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [Nous Portal 集成页面](/integrations/nous-portal)。本页是操作步骤脚本。

## 前提条件

- 已安装 Kopi Agent（[快速入门](/getting-started/quickstart)）
...
OAuth 需要浏览器，但 loopback 回调运行在 Kopi 所在的机器上。有两种方案：
...
你应该看到 Kopi 调用 `web_search`（通过 gateway 由 Firecrawl 提供支持）并返回摘要。如果搜索正常执行且响应内容合理，说明配置完成——Portal 已端到端连通。
...
### 不要在 agent 任务中使用 Kopi-4

Kopi-4-70B 和 Kopi-4-405B 在 Portal 上以大幅折扣提供，但它们是**对话/推理模型**，并非针对工具调用优化的模型。它们在多步骤 agent 循环中表现不佳。请通过 [Nous Chat](https://chat.nousresearch.com) 将它们用于对话/研究工作，或通过[订阅代理](/user-guide/features/subscription-proxy)从非 agent 工具中使用。对于 Kopi Agent 本身，请坚持使用上述前沿 agentic 模型。

Portal 的[信息页面](https://portal.nousresearch.com/info)也有此说明——这是 Nous 官方指导，并非仅代表 Kopi 一方的意见。
...
之后在任何消息平台会话中（Telegram、Discord、Signal 等），发送语音消息，Kopi 将转录内容、生成回复并以合成语音回复——全部通过你的 Portal 订阅完成。
...
如果你使用 [Kopi profiles](/user-guide/profiles)（例如每个项目单独一套配置），Portal refresh token 会通过共享 token 存储自动在所有 profiles 之间共享。在任意 profile 上登录一次，其余 profiles 会自动获取。
...
你的 Portal refresh token 已失效（密码更改、手动撤销、会话过期）。该 token 现已在本地被隔离，以防 Kopi 无限重试。重新登录即可：
...
如果某个模型确实不可用，请[提交 issue](https://github.com/NousResearch/kopi-agent/issues)——大多数缺失是我们可以更新的路由配置问题。
...
- 存在多个 Kopi profiles，你使用的是错误的那个（检查 `kopi profile list`）
...
- **[订阅代理](/user-guide/features/subscription-proxy)** — 在非 Kopi 工具中使用你的 Portal 订阅
- **[语音模式](/user-guide/features/voice-mode)** — 在 Portal 订阅上配置语音对话
- **[OAuth over SSH](/guides/oauth-over-ssh)** — 远程/无头主机登录方案
- **[Profiles](/user-guide/profiles)** — 在多个 Kopi 配置之间共享一个 Portal 登录