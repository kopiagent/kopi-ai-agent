...
本页介绍如何为 KOPI AI AGENT 配置推理提供商——从 OpenRouter、Anthropic 等云端 API，到 Ollama、vLLM 等自托管端点，再到高级路由与故障转移配置。使用 Kopi 至少需要配置一个提供商。
...
| 提供商 | 配置方式 |
|----------|-------|
| **KOPI Proxy** | `kopi model`（OAuth，订阅制） |
...
[KOPI Proxy](https://kopiaiagent.com/portal) 是 Kopi Ai Agent Pte Ltd 的统一订阅网关，也是**运行 KOPI AI AGENT 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [Nous Chat](https://chat.kopiaiagent.com)——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。

```bash
kopi setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
kopi model              # 已有安装——从列表中选择"KOPI Proxy"
kopi portal info        # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.kopiaiagent.com/manage-subscription](https://kopiaiagent.com/portal/manage-subscription) 购买。

**完整详情：** 参见专属的 [KOPI Proxy 集成页面](/integrations/subscription-proxy)（订阅内容、模型目录、故障排查）。


:::info Codex 说明
OpenAI Codex 提供商通过设备码（device code）认证——打开一个 URL 并输入验证码。Kopi 将生成的凭据存储在 `~/.kopi/auth.json` 的自有认证存储中，并在存在 `~/.codex/auth.json` 时可导入现有的 Codex CLI 凭据。无需安装 Codex CLI。

如果 token 刷新因终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等）失败，Kopi 会将该刷新 token 标记为失效并停止重试，避免出现大量重复的认证失败。下一次请求会显示类型化的重新认证提示。运行 `kopi auth add codex-oauth`（或 `kopi model` → OpenAI Codex）开始新的设备码登录；成功交换后隔离状态自动解除。
:::

:::warning
即使使用 KOPI Proxy、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Kopi 将这些任务路由到你的**主聊天模型**——即你在 `kopi model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 KOPI Proxy 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`kopi setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `kopi model` 或 `kopi tools` 按工具启用。随时使用 `kopi portal info` 查看路由状态。
:::
...
| **`/model`** | Kopi 聊天会话内部 | 在**已配置的**提供商和模型之间快速切换 |
...
通过 `kopi model` → Anthropic OAuth（或 `kopi auth add anthropic --type oauth`）认证时，Kopi 以 Claude Code 身份路由到你的 Anthropic 账户。**仅当你订阅了 Claude Max 计划且购买了额外用量积分时才有效。** Claude Max 基础计划的配额（Claude Code 默认包含的用量）不会被 Kopi 消耗——只有你额外购买的超额积分才会被使用。Claude Pro 订阅者无法使用此路径。
...
# 推荐：通过 `kopi model` 认证
# 如果已使用 Claude Code，Kopi 会直接使用其凭据存储
kopi model
...
通过 `kopi model` 选择 Anthropic OAuth 时，Kopi 优先使用 Claude Code 自身的凭据存储，而不是将 token 复制到 `~/.kopi/.env`。这样可以保持 Claude 凭据的可刷新性。
...
Kopi 以一等提供商身份支持 GitHub Copilot，提供两种模式：
...
:::info Kopi 中的 Copilot 认证行为
Kopi 将支持的 GitHub token（`gho_*`、`github_pat_*` 或 `ghu_*`）直接发送到 `api.githubcopilot.com`，并附带 Copilot 专用请求头（`Editor-Version`、`Copilot-Integration-Id`、`Openai-Intent`、`x-initiator`）。

收到 HTTP 401 时，Kopi 在回退前会执行一次性凭据恢复：

1. 通过正常优先级链重新解析 token（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2. 使用刷新后的请求头重建共享 OpenAI 客户端
3. 重试请求一次

部分旧版社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点对某些账户类型可能不可用（返回 404）。因此 Kopi 以直接 token 认证为主路径，依靠运行时凭据刷新 + 重试保证健壮性。
:::
...
:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，Kopi 会自动探测多个端点（全球版、中国版、编程版）以找到接受你 API key 的端点。无需手动设置 `GLM_BASE_URL`——可用端点会被自动检测并缓存。
:::
...
SuperGrok 和 X Premium+ 订阅者可以用浏览器 OAuth 登录，无需 API key——在 `kopi model` 中选择 **xAI Grok OAuth (SuperGrok / Premium+)**，或运行 `kopi auth add xai-oauth`。同一 OAuth bearer token 会被 xAI 直连工具（TTS、图像生成、视频生成、转录）自动复用。完整流程参见 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md)——如果 Kopi 运行在远程主机上，还需参见 [SSH / 远程主机上的 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道配置。

使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），Kopi 会在每次 API 请求中自动发送 `x-grok-conv-id` 请求头以启用 prompt（提示词）缓存。这会将同一会话的请求路由到同一服务器，使 xAI 基础设施能够复用已缓存的系统 prompt 和对话历史。
...
[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开源模型目录，无需 GPU。在 `kopi model` 中选择 **Ollama Cloud**，粘贴来自 [ollama.com/settings/keys](https://ollama.com/settings/keys) 的 API key，Kopi 会自动发现可用模型。
...
阿里巴巴 Qwen Portal，支持基于浏览器的 OAuth 登录。在 `kopi model` 中选择 **Qwen OAuth (Portal)**，通过浏览器登录，Kopi 会持久化刷新 token。
...
如果你订阅了阿里巴巴的 **Coding Plan**（独立于标准 DashScope API 访问的计费 SKU），Kopi 将其作为独立的一等提供商暴露：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。与常规 `alibaba` 提供商一样兼容 OpenAI，但基础 URL 和计费面不同。
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7——无需 API key。在 `kopi model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，Kopi 会持久化访问 token 和刷新 token。底层使用 Anthropic Messages 兼容端点（`/anthropic`）。
...
Kopi 会在每次向 `build.nvidia.com` 发送请求时自动附加 NIM 计费来源请求头——无需任何配置。这会在 NVIDIA 计费仪表板中将消耗路由到正确的来源。
...
KOPI AI AGENT 可与**任何 OpenAI 兼容 API 端点**配合使用。只要服务器实现了 `/v1/chat/completions`，就可以将 Kopi 指向它。这意味着你可以使用本地模型、GPU 推理服务器、多提供商路由器或任何第三方 API。
...
:::warning 旧版环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Kopi 的任何部分都不再读取这两个变量——`config.yaml` 是模型和端点配置的唯一来源。如果你的 `.env` 中有过时条目，下次运行 `kopi setup` 或配置迁移时会自动清除。请使用 `kopi model` 或直接编辑 `config.yaml`。
:::
...
**`/model`**（在活跃的 Kopi 聊天会话中输入）只能在**已配置的**提供商和模型之间**切换**。它无法添加新提供商、运行 OAuth 或提示输入 API key。如果你只配置了一个提供商（如 OpenRouter），`/model` 只会显示该提供商的模型。
...
切换提供商时，Kopi 会将基础 URL 和提供商持久化到配置中，使更改在重启后保留。从自定义端点切换到内置提供商时，过时的基础 URL 会自动清除。
...
**无法通过 OpenAI 兼容 API**（`/v1/chat/completions`）设置上下文长度。必须在服务端或通过 Modelfile 配置。这是将 Ollama 与 Kopi 等工具集成时最常见的困惑来源。
:::
...
| 标志 | 用途 |
|------|---------|
| `--enable-auto-tool-choice` | `tool_choice: "auto"` 所必需（Kopi 的默认值） |
| `--tool-call-parser <name>` | 模型工具调用格式的解析器 |

支持的解析器：`kopi`（Qwen 2.5、Kopi 2/3）、`llama3_json`（Llama 3.x）、`mistral`、`deepseek_v3`、`deepseek_v31`、`xlam`、`pythonic`。没有这些标志，工具调用将无法工作——模型会将工具调用以文本形式输出。
...
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型会尝试在响应文本中写入 JSON 来调用工具，但 Kopi 不会将其识别为工具调用——你会看到原始 JSON（如 `{"name": "web_search", ...}`）作为消息打印出来，而不是实际执行搜索。

原生工具调用支持（最佳性能）：Llama 3.x、Qwen 2.5（包括 Coder）、Kopi 2/3、Mistral、DeepSeek、Functionary。其他所有模型使用通用处理器，可以工作但效率可能较低。完整列表参见 [llama.cpp 函数调用文档](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md)。
...
Kopi 会自动以 64K 上下文长度加载 LM Studio 模型。
...
**工具调用：** 自 LM Studio 0.3.6 起支持。具有原生工具调用训练的模型（Qwen 2.5、Llama 3.x、Mistral、Kopi）会被自动检测并显示工具徽章。其他模型使用通用回退，可靠性可能较低。
...
如果收到列出模型的 JSON 响应，说明配置正确。在 Kopi 配置中使用相同的 URL 作为 `base_url`。

---

### 本地模型故障排查

以下问题影响与 Kopi 配合使用的**所有**本地推理服务器。

#### 从 WSL2 连接 Windows 托管模型服务器时"连接被拒绝"

如果你在 WSL2 内运行 Kopi 而模型服务器在 Windows 主机上，在 WSL2 默认 NAT 网络模式下 `http://localhost:<port>` 无法工作。参见上方的 [WSL2 网络](#wsl2-networking-windows-users) 了解解决方案。
...
| **vLLM** | 添加 `--enable-auto-tool-choice --tool-call-parser kopi` |
...
**原因：** 上下文窗口太小。当对话超过上下文限制时，大多数服务器会静默丢弃较早的消息。Kopi 的系统 prompt 加工具 schema 单独就可能占用 4k–8k tokens。
...
Kopi 从服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或根本不报告），Kopi 使用模型声明的限制，该值可能不正确。
...
2. **上下文耗尽** — 模型填满了上下文窗口。增加 `model.context_length` 或在 Kopi 中启用[上下文压缩](/user-guide/configuration#context-compression)。
...
然后通过 `kopi model` → 自定义端点 → `http://localhost:4000/v1` 配置 Kopi。
...
然后通过 `kopi model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto` 配置 Kopi。
...
:::note 两个设置，容易混淆
**`context_length`** 是**总上下文窗口**——输入和输出 token 的合计预算（例如 Claude Opus 4.6 为 200,000）。Kopi 用它来决定何时压缩历史记录以及验证 API 请求。
...
7. **KOPI Proxy** — 将 Nous 模型 ID 后缀匹配到 OpenRouter 元数据
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key 省略——Kopi 对无 key 的本地服务器使用"no-key-required"
  - name: work
...
某些 OpenAI 兼容端点需要特定于提供商的请求体字段。在对应的自定义提供商中添加 `extra_body` 映射，Kopi 会将其合并到该端点的每个 chat-completions 请求中：
...
| 使用场景 | 推荐方案 |
|----------|-------------|
| **只想让它工作** | OpenRouter（默认）或 KOPI Proxy |
| **本地模型，简单配置** | Ollama |
...
默认情况下，Kopi 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你希望在本地运行 Firecrawl，可以将 Kopi 指向自托管实例。完整配置说明参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
配置一个备用提供商链，当主模型失败时（速率限制、服务器错误、认证失败）Kopi 按顺序尝试。规范格式是顶级 `fallback_providers:` 列表：
