...
- **KOPI Proxy** — Kopi Ai Agent Pte Ltd 自有推理端点
...
**原生不支持。** KOPI AI AGENT 需要类 Unix 环境。在 Windows 上，请安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运行 Kopi。标准安装命令在 WSL2 中可完美运行：

```bash
curl -fsSL https://kopiaiagent.com/install.sh | bash
```

### 我在 WSL2 中运行 Kopi，如何控制 Windows 上的普通 Chrome？
...
- 让 Kopi 使用生成的 MCP 浏览器工具

这比强制 Kopi 核心浏览器传输直接跨越 WSL2/Windows 边界进行附加更为可靠。
...
支持 — Kopi 现已为 Android 手机提供经过测试的 Termux 安装路径。
...
Kopi 会将端点、提供商和 base URL 持久化到 `config.yaml`，重启后仍然有效。如果您的本地服务器只加载了一个模型，`/model custom` 会自动检测到它。您也可以在 config.yaml 中设置 `provider: custom` — 这是一个一等提供商，不是其他任何东西的别名。

此方式适用于 Ollama、vLLM、llama.cpp server、SGLang、LocalAI 等。详情请参阅[配置指南](../user-guide/configuration.md)。

:::tip Ollama 用户
如果您在 Ollama 中设置了自定义 `num_ctx`（例如 `ollama run --num_ctx 16384`），请确保在 Kopi 中设置匹配的上下文长度 — Ollama 的 `/api/show` 报告的是模型的*最大*上下文，而非您配置的实际 `num_ctx`。
:::

:::tip 本地模型超时问题
Kopi 会自动检测本地端点并放宽流式传输超时（读取超时从 120s 提升至 1800s，禁用停滞流检测）。如果在非常大的上下文下仍然超时，请在 `.env` 中设置 `KOPI_STREAM_READ_TIMEOUT=1800`。详情请参阅[本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::
...
**原因：** Kopi 在启动时通过运行一次 `bash -l` 构建每个会话的环境快照。bash 登录 shell 会读取 `/etc/profile`、`~/.bash_profile` 和 `~/.profile`，但**不会 source `~/.bashrc`** — 因此在 `~/.bashrc` 中安装自身的工具（`nvm`、`asdf`、`pyenv`、`cargo`、自定义 `PATH` 导出）对快照不可见。当 Kopi 在 systemd 下运行或在未预加载交互式 shell 配置的最小 shell 中运行时，此问题最为常见。

**解决方案：** Kopi 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.kopi/config.yaml` 中列出需要额外 source 的文件：
...
**原因：** 对话内容超出模型的上下文窗口，或 Kopi 检测到的模型上下文长度有误。
...
如果在第一次长对话时就出现此问题，Kopi 可能检测到了错误的模型上下文长度。检查检测结果：
...
**原因：** Kopi 检测到潜在的破坏性命令（例如 `rm -rf`、`DROP TABLE`）。这是一项安全功能。
...
:::tip
这是预期行为 — Kopi 绝不会静默执行破坏性命令。审批提示会向您显示将要执行的确切内容。
:::
...
# 更改配置后重启 Kopi 或重新加载 MCP
kopi chat
```

另请参阅：
- [MCP（模型上下文协议）](/user-guide/features/mcp)
- [在 Kopi 中使用 MCP](/guides/use-mcp-with-kopi)
...
:::warning
如果 MCP 服务器在请求中途崩溃，Kopi 会报告超时。请检查服务器自身的日志（而非仅 Kopi 日志）以诊断根本原因。
:::
...
**解决方案：委托配置。** Kopi 可以自动将子智能体路由到不同的模型。在 `~/.kopi/config.yaml` 中设置：
...
现在当您告诉 Kopi "帮我写一个关于 X 的 Twitter 帖子"并生成 `delegate_task` 子智能体时，该子智能体将在 Gemini 上运行，而非您的主模型。您的主对话仍在 GPT-5.4 上进行。
...
**场景：** 在 OpenClaw 中，您可以将多个独立智能体绑定到特定的 WhatsApp 聊天 — 一个用于家庭购物清单群组，另一个用于您的私聊。Kopi 能做到吗？

**当前限制：** Kopi 的每个 profile 都需要自己的 WhatsApp 号码/会话。您无法将多个 profiles 绑定到同一个 WhatsApp 号码上的不同聊天 — WhatsApp bridge（Baileys）每个号码使用一个已认证的会话。
...
**场景：** 您在 Telegram 中看到了网关执行日志、Kopi 推理过程和工具调用详情，而不是最终输出。
...
**当前行为：** Kopi 在大多数平台上按用户 ID 创建会话，因此每个人都有自己的对话上下文。这是出于隐私和上下文隔离的设计考量。
...
:::tip
`kopi backup` 即使在 Kopi 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::

### 安装后重新加载 shell 时出现权限拒绝

**场景：** 运行 Kopi 安装程序后，`source ~/.zshrc` 提示权限拒绝错误。

**原因：** 这通常发生在 `~/.zshrc`（或 `~/.bashrc`）文件权限不正确，或安装程序无法干净写入时。这不是 Kopi 特有的问题 — 而是 shell 配置权限问题。
...
2. **向社区提问：** [Kopi Ai Agent Pte Ltd Discord](https://discord.gg/kopi-ai-agent)
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、Kopi 版本（`kopi --version`）以及完整的错误信息