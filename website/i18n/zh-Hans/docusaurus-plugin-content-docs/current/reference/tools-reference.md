...
sidebar_position: 3
title: "内置工具参考"
description: "Kopi 内置工具权威参考，按工具集分组"
---

# 内置工具参考

本页记录 Kopi 的内置工具，按工具集分组。可用性因平台、凭据和已启用的工具集而异。
...
:::tip MCP 工具
除内置工具外，Kopi 还可从 MCP 服务器动态加载工具。MCP 工具以 `mcp_<server>_` 为前缀（例如，`github` MCP 服务器的 `mcp_github_create_issue`）。配置方法见 [MCP 集成](/user-guide/features/mcp)。
:::
...
| 工具 | 描述 | 所需环境 |
|------|------|----------|
| `execute_code` | 运行可以编程方式调用 Kopi 工具的 Python 脚本。当需要 3 次以上工具调用且调用之间有处理逻辑、需要在大型工具输出进入上下文前过滤/压缩、需要条件分支（…）时使用。 | — |
