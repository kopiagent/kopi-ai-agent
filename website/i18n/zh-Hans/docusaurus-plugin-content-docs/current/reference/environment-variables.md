...
sidebar_position: 2
title: "环境变量"
description: "Kopi Agent 使用的所有环境变量完整参考"
---
...
| `NOUS_BASE_URL` | 覆盖 KOPI Proxy base URL（极少使用；仅用于开发/测试） |
...
| `MINIMAX_BASE_URL` | 覆盖 MiniMax base URL（默认：`https://api.minimax.io/anthropic`——Kopi 使用 MiniMax 的 Anthropic Messages 兼容端点）。**`minimax-oauth` 不使用此变量**。 |
...
| `KOPI_HOME` | 覆盖 Kopi 配置目录（默认：`~/.kopi`）。同时限定 gateway PID 文件和 systemd 服务名称，允许多个安装并发运行 |
...
| `KOPI_KANBAN_HOME` | 覆盖锚定 kanban 看板（数据库 + 工作区 + 工作日志）的共享 Kopi 根目录。回退到 `get_default_kopi_root()`（任意活动 profile 的父目录）。适用于测试和非常规部署 |
...
对于原生 Anthropic 认证，Kopi 在 Claude Code 自身凭证文件存在时优先使用，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要购买了额外使用额度的 Claude Max 计划**——Kopi 以 Claude Code 身份路由，仅消耗 Max 计划的额外/超额额度，不消耗基础 Max 配额，且不适用于 Claude Pro。没有 Max + 额外额度时，请改用 API 密钥。`ANTHROPIC_TOKEN` 等环境变量作为手动覆盖仍然有用，但不再是 Claude Max 登录的首选路径。

| 变量 | 描述 |
|----------|-------------|
| `KOPI_PORTAL_BASE_URL` | 覆盖 KOPI Proxy URL（用于开发/测试） |
...
| `AGENT_BROWSER_ARGS` | 额外的 Chromium 启动标志（逗号或换行分隔）。以 root 身份运行或在 AppArmor 限制的非特权用户命名空间（Ubuntu 23.10+、DGX Spark、许多容器镜像）中运行时，Kopi 自动注入 `--no-sandbox,--disable-dev-shm-usage`；仅在需要覆盖或添加其他标志时手动设置。 |
...
| 变量 | 描述 |
|----------|-------------|
| `TOOL_GATEWAY_DOMAIN` | Tool Gateway 路由的基础域名（默认：`kopiaiagent.com`） |
...
| `KOPI_DOCKER_BINARY` | 覆盖 Kopi 调用的容器二进制（例如 `podman`、`/usr/local/bin/docker`）。未设置时，Kopi 自动在 `PATH` 上发现 `docker` 或 `podman`。当两者都已安装且需要非默认选项，或二进制不在 `PATH` 中时使用。 |
...
对于云沙箱后端，持久化以文件系统为导向。`TERMINAL_LIFETIME_SECONDS` 控制 Kopi 何时清理空闲终端会话，后续恢复可能会重新创建沙箱而非保持相同的活跃进程。
...
| `MATRIX_USER_ID` | Matrix 用户 ID（例如 `@kopi:matrix.org`）——密码登录时必填，使用访问 token 时可选 |
...
| `API_SERVER_MODEL_NAME` | `/v1/models` 上公告的模型名称。默认为 profile 名称（默认 profile 为 `kopi-agent`）。适用于 Open WebUI 等前端需要每个连接使用不同模型名称的多用户场景。 |
| `GATEWAY_PROXY_URL` | 将消息转发到的远程 Kopi API 服务器 URL（[代理模式](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)）。设置后，gateway 仅处理平台 I/O——所有 agent 工作委托给远程服务器。也可通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
...
| `KOPI_FILE_MUTATION_VERIFIER` | 启用每轮文件变更验证器页脚（默认：`true`）。启用后，Kopi 附加一个建议列表，列出本轮中失败且未被成功写入覆盖的 `write_file`/`patch` 调用。设为 `0`、`false`、`no` 或 `off` 可抑制。镜像 `config.yaml` 中的 `display.file_mutation_verifier`；设置时环境变量优先。 |
...
| `CODEX_HOME` | 启用 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime)时，覆盖 Codex CLI 读取其配置 + 认证的目录（默认：`~/.codex`）。Kopi 的迁移将托管块写入 `<CODEX_HOME>/config.toml`。 |
...
| `KOPI_WRITE_SAFE_ROOT` | 可选目录前缀，**硬阻止** `write_file`/`patch` 写入列出的根目录之外的路径（无审批提示）。支持多个目录，使用 `os.pathsep` 分隔（Unix 为 `:`，Windows 为 `;`）。详见下方 [KOPI_WRITE_SAFE_ROOT](#kopi_write_safe_root)。 |
...
### KOPI_WRITE_SAFE_ROOT {#kopi_write_safe_root}

设置此变量后，`write_file` 和 `patch` 只能写入列出的目录前缀内的路径。超出这些根目录的路径会被**立即拒绝**——不会进入危险命令审批流程，也没有聊天界面可以覆盖。

官方 Docker 镜像会设置 `KOPI_WRITE_SAFE_ROOT=/opt/data` 与 `KOPI_HOME=/opt/data`，防止 agent 逃出挂载的数据卷。

**除非有意沙箱化写入，否则不要将此变量加入 `~/.kopi/.env`。** 常见错误是将其指向项目目录，却期望 agent 编辑 `~/.kopi/cron/jobs.json`、`~/.kopi/skills/` 或 profile 下的脚本——这些路径在沙箱外，每次 `write_file`/`patch` 都会失败并返回 `outside KOPI_WRITE_SAFE_ROOT` 错误。

若需同时允许工作区和 Kopi 状态目录，列出两个前缀（顺序无关）：

```bash
export KOPI_WRITE_SAFE_ROOT=/path/to/project:/home/you/.kopi
```
...
| `KOPI_TUI_THEME` | 强制 TUI 颜色主题：`light`、`dark` 或原始 6 字符背景十六进制（例如 `ffffff` 或 `1a1a2e`）。未设置时，Kopi 使用 `COLORFGBG` 和终端背景查询自动检测；此变量覆盖不设置 `COLORFGBG` 的终端（Ghostty、Warp、iTerm2 等）上的检测。 |
...
| `KOPI_SESSION_ID` | **自动导出到 Kopi 生成的每个工具子进程**（`terminal`、`execute_code`、持久 shell、Docker/Singularity 后端、委托子 agent 运行）。由 agent 设置为当前会话 ID；从工具调用的用户脚本可读取它，以将其输出、遥测或副作用与原始 Kopi 会话关联。**不应手动设置**——从父 shell 覆盖仅在 agent 运行外生效，且 agent 启动会话时会被覆盖。 |
...
对于特定任务的直接端点，Kopi 使用该任务配置的 API 密钥或 `OPENAI_API_KEY`。不会为这些自定义端点复用 `OPENROUTER_API_KEY`。
