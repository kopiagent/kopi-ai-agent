...
ACP 适配器将 Kopi 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
Zed ACP Registry 路径通过 `uvx --from 'kopi-agent[acp]==<version>' kopi-acp` 启动同一适配器，指向 `kopi-agent` PyPI 发布包。
...
`acp_adapter/tools.py` 将 Kopi 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
因此 ACP 通告并使用当前配置的 Kopi provider/凭据。它还始终通告一个终端 setup 认证方法（`kopi-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 Kopi 的交互式模型/provider 配置。
