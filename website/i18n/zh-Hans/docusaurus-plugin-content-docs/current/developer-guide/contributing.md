...
- 构建自定义/本地工具而不修改 Kopi 核心？从 [构建 Kopi 插件](../developer-guide/plugins/index.md) 开始
- 为 Kopi 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
对大多数贡献者来说，最好的开发启动方式和用户安装方式相同：运行标准安装器，然后在它克隆出的仓库里开发。安装器会创建 Kopi venv、配置 `kopi` 命令、为 `kopi update` 写入安装方式标记，并把完整 git 项目克隆到 `$KOPI_HOME/kopi-ai-agent`（通常是 `~/.kopi/kopi-ai-agent`）。这样你的开发环境会和 CLI、updater、lazy dependency installer、gateway、docs 默认假设的布局一致。
...
只有在你明确不想使用 Kopi managed install layout 时才使用这种方式（例如容器或 CI job 里的临时 clone）。如果这样安装，请确保运行的是这个 venv 里的 `kopi` entrypoint；运行系统 `python3 -m kopi_cli.main` 可能会加载无关的系统 Python 包。
...
Kopi 官方支持 **Linux、macOS、WSL2 以及原生 Windows（通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。如果您主要在 Windows 上开发，推送前请运行 Windows 陷阱（footgun）lint（`scripts/check-windows-footguns.py`）。
...
- 请包含：操作系统、Python 版本、Kopi 版本（`kopi version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
