...
description: "使用 Nix 安装和部署 Kopi Agent——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Kopi Agent 提供了一个 Nix flake，支持三个层级的集成：
...
执行 `nix profile install` 后，`kopi`、`kopi-agent` 和 `kopi-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`kopi setup` 引导你完成提供商选择，`kopi gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.kopi/`。
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Kopi 默认使用 OpenRouter。
:::
...
| 给 Agent 设置个性 | `${services.kopi-agent.stateDir}/.kopi/SOUL.md` | 直接管理该文件 |
...
| 覆盖 kopi 包 | `package` | `inputs.kopi-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$KOPI_HOME/.env` 中。Kopi 在每次启动时读取此文件，因此更改在 `systemctl restart kopi-agent` 后生效——无需重建容器。
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Kopi 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：Kopi 从 `$KOPI_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.kopi-agent.stateDir}/.kopi/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Kopi 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Kopi 将授权 URL 打印到 stdout/日志，而不是打开浏览器。

**方案 A：交互式引导** — 通过 `docker exec`（容器）或 `sudo -u kopi`（原生）运行一次流程：
...
2. **`.managed` 标记文件**，位于 `KOPI_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it kopi-agent kopi config set ...` 也会被屏蔽）
...
|---|---|---|---|---|
| `systemctl restart kopi-agent` | 否 | 保留 | 保留 | 保留 |
...
插件在激活时以符号链接方式安装到 `$KOPI_HOME/plugins/`。Kopi 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。

### 入口点插件（`extraPythonPackages`）

对于通过 `[project.entry-points."kopi_agent.plugins"]` 注册的 pip 打包插件（例如 [rtk-kopi](https://github.com/ogallotti/rtk-kopi)）：
...
对于已在 kopi-agent 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `kopi` 和 `kopi-agent` 二进制文件存在且 `kopi version` 可运行 |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 kopi-agent 服务 |
| `package` | `package` | `kopi-agent` | 使用的 kopi-agent 包 |
| `user` | `str` | `"kopi"` | 系统用户 |
| `group` | `str` | `"kopi"` | 系统组 |
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Kopi 二进制文件 + 所有 Nix 依赖 |
...
| `kopi version` 显示旧版本 | 容器未重启 | `systemctl restart kopi-agent` |
| `/var/lib/kopi` 权限拒绝 | 状态目录为 `0750 kopi:kopi` | 使用 `docker exec` 或 `sudo -u kopi` |
| `nix-collect-garbage` 删除了 kopi | GC root 缺失 | 重启服务（preStart 会重新创建 GC root） |
| `no container with name or ID "kopi-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user kopi` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart kopi-agent` |