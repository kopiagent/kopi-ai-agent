...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for Kopi. Uses OAuth2 with automatic token refresh. Prefers the [Google Workspace CLI (`gws`)](https://github.com/googleworkspace/cli) when available for broader coverage, and falls back to Google's Python client libraries otherwise.
