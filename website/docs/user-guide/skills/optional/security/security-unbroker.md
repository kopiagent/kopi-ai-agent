...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/security/unbroker` |
...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  ...
  Kopi already loads for its own tools are picked up without re-exporting - each one converts a
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires Kopi to be pointed at the operator's own
