...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Kopi MCP catalog. The
...
Two halves, in this order: the editor side must be up before Kopi connects.
...
### One-time, Kopi side
    ...
    kopi mcp install unreal-engine
...
`~/.kopi/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Kopi — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Kopi connects
...
2. Start the Kopi session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Kopi
they appear as:
...
then happens at `kopi mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Kopi runs batched calls
...
- **Start order matters.** Editor + server up first, then the Kopi
...
- **Timeouts on long operations.** Kopi' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.kopi/config.yaml` for
