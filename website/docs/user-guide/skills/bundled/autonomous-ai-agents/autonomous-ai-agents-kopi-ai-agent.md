---
title: "Kopi Ai Agent — Use, configure, theme, extend, and orchestrate Kopi Agent"
...
Use, configure, theme, extend, and orchestrate Kopi Agent.
...
| Tags | `kopi`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `themes`, `skins`, `desktop-plugins`, `tui-widgets`, `petdex`, `development` |
...
- **Self-improving through skills** — Kopi learns from experience by saving reusable procedures as skills that load into future sessions.
...
- Source tree: https://github.com/NousResearch/kopi-ai-agent
...
```
~/.kopi/config.yaml       Main configuration (settings — never secrets)
~/.kopi/.env              API keys and secrets ONLY (under $KOPI_HOME if set)
$KOPI_HOME/skills/        Installed skills
~/.kopi/skins/            Custom themes (see references/themes.md)
~/.kopi/desktop-plugins/  Desktop app UI plugins (see references/desktop-plugins.md)
~/.kopi/tui-widgets/      TUI widget apps (see references/tui-widgets.md)
~/.kopi/pets/             Installed pet mascots (see references/petdex.md)
~/.kopi/state.db          Canonical session store (SQLite + FTS5)
~/.kopi/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
...
Profiles use `~/.kopi/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$KOPI_HOME` — never hardcode `~/.kopi`.
...
| AGENTS.md / .kopi.md / CLAUDE.md project rules | `references/project-context-files.md` |
...
| MCP servers (add, catalog, `kopi mcp`) | `references/native-mcp.md` |
...
Two theming rules that hold even without loading the reference: **you apply skins yourself** (`kopi config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`kopi skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.
...
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Kopi; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Desktop app** (`kopi desktop` / `kopi gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`kopi dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `kopi --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`kopi --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`kopi proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Profile-safe paths** — `get_kopi_home()` in code, `$KOPI_HOME` when resolving paths in a session.
- **Never hand-edit `config.yaml` for the user** — use `kopi config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
