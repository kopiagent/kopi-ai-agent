---
title: "Tui Widgets — Author live widget apps for the Kopi TUI dock"
sidebar_label: "Tui Widgets"
description: "Author live widget apps for the Kopi TUI dock"
---
...
Author live widget apps for the Kopi TUI dock.
...
Author widget apps for the Kopi TUI (`kopi --tui`): glanceable ambient
...
- The TUI must be in use (`kopi --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.kopi/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.kopi/logs/tui_gateway_crash.log` if a widget never appears.
