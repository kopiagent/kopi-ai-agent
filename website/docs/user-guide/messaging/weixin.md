...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect Kopi Agent to personal WeChat accounts via the iLink Bot API"
---
...
login connects one iLink bot identity to Kopi. Other people do not scan the
Kopi QR code with their own accounts; they must message the connected iLink
bot/contact through WeChat, and Kopi will process the DM only if the sender's
Weixin user ID is present in `WEIXIN_ALLOWED_USERS`.

A practical setup flow is:

1. Pair Kopi once with `kopi gateway setup` and note the connected iLink bot
   account.
...
If only the account that scanned the QR code can talk to Kopi, verify that the
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.kopi/kopi-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
