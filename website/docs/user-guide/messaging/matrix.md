...
Kopi sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Kopi now includes the current Matrix room name, room ID, topic, message ID,
...
Kopi Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Bridge and appservice deployments need extra loop protection. Kopi always
...
cd ~/.kopi/kopi-agent && uv pip install -e ".[matrix]"
```
...
In Matrix conversations, Kopi exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Kopi knows who requested the action.

### Media Limits

Kopi uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Kopi rejects arbitrary
...
Kopi includes an opt-in Synapse harness for local validation:
...
If Kopi bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
Kopi supports the same gateway commands in Matrix that it supports on other
...
Kopi only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/kopi-agent/issues/12614).
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "Kopi Agent"
     }'
   ```
...
RUN cd ~/.kopi/kopi-agent && uv pip install -e ".[matrix]"
...
**Fix**: Kopi uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Kopi's own
...
For more information on securing your Kopi Agent deployment, see the [Security Guide](../security.md).
