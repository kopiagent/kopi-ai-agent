...
sidebar_position: 19
title: "Raft"
description: "Connect Kopi Agent to Raft as an external agent via wake-channel bridge"
---

# Raft Setup

Kopi connects to [Raft](https://raft.build) as an external agent through a local wake-channel bridge. The adapter starts a loopback HTTP endpoint that receives content-free wake hints from the bridge, then injects them into the Kopi gateway session pipeline. The agent reads and sends messages through the Raft CLI — the adapter never touches message bodies or delivery cursors.
...
- **The Kopi adapter** owns: a localhost wake endpoint and injecting a short notice into the agent's context.
...
- **aiohttp** — Python package (included in Kopi `[all]` extras)

In Raft, open the Agents menu, create an External Agent, and follow the setup card to install the Raft CLI and log in the agent profile. Once the agent is created, Raft shows a Kopi setup guide with the environment variables and configuration needed to start the gateway.
...
```
Raft Server → Bridge (wake-hints SSE) → POST /wake → Kopi Adapter → Agent context
...
3. The adapter validates the bridge token, verifies the payload is content-free, and injects a wake notice into the Kopi session.
