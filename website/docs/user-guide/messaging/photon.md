...
Connect Kopi to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Kopi — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Kopi runs it in a
...
2. **Finds or creates** the `Kopi Agent` project on your account.
...
Photon uses the same authorization model as every other Kopi
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Kopi replies with a pairing code. Approve it with:
...
By default Kopi responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Kopi` and
`@Kopi agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Kopi will reply.
...
assigned iMessage line Kopi uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Kopi sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Kopi wake words  | JSON list / comma / newline regex patterns for group mentions |
