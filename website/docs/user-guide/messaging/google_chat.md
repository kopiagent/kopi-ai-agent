...
sidebar_position: 12
title: "Google Chat"
description: "Set up Kopi Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Kopi Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
...
`~/.kopi/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Kopi is thinking…").
...
4. Register the client with Kopi (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Kopi home (e.g.
