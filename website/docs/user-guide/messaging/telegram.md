...
sidebar_position: 1
title: "Telegram"
description: "Set up Kopi Agent as a Telegram bot"
---
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Kopi Agent") — this can be anything
...
Enable `status_indicator` and Kopi sets that short description to **Online**
...
`priority_mode` controls how your list combines with Kopi' built-in priority list:

- `prepend`: put your commands first, then Kopi defaults
- `append`: keep Kopi defaults first, then your commands
- `replace`: use only your list for priority ordering

Telegram allows up to 100 BotCommands, but large command payloads can fail. Kopi defaults to 60 for reliability and clamps configured values to `1..100`; use `/commands` for the full command list.
...
Kopi Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
With STT disabled, the gateway still downloads the voice/audio attachment into Kopi's audio cache, but **does not transcribe it**. The agent receives the message with a marker like:
...
Telegram's **public** Bot API caps `getFile` downloads at **20 MB**, so any voice note, audio file, video, or document larger than that is silently rejected by Kopi with a "too large" reply. The documented way around this is to run a **local** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) daemon — the same server software Telegram uses, but running on your network. A local server raises the file ceiling to **2 GB** and Kopi auto-lifts its own internal cap when it sees a custom `base_url` configured.
...
### Step 4: Point Kopi at the local server
  ...
  telegram:
    extra:
      ...
      local_mode: true        # see Step 5 below — only set this if the bot's data
                              # directory is readable by the Kopi process
```
...
2. **With `--local`** (set via `TELEGRAM_LOCAL=1` above): files are written to the server's filesystem and the `getFile` response returns an **absolute path** instead of an HTTP URL. The 20MB ceiling is lifted. Kopi must then read the bytes **from disk**, not over HTTP.

To make the disk-read path work, set `local_mode: true` in the config above **and** make sure the Kopi process can read the path the server returns. Two scenarios:

- **Same machine** — telegram-bot-api and Kopi run on the same host. Bind-mount the data volume to a directory that Kopi can read (e.g., `/var/lib/telegram-bot-api`), and make sure the file ownership matches. The container drops privileges to its internal `telegram-bot-api` user (uid varies by image); the simplest fix is to add `user: "<UID>:<GID>"` to the compose service so files are owned by a uid Kopi already runs as.
- **Different machines** — the bot server runs on one host (e.g., a NAS, a separate VM) and Kopi on another. The server's data directory must be shared with the Kopi machine at the **same absolute path** the server reports (typically `/var/lib/telegram-bot-api`). NFS works well for this; CIFS/SMB with `uid=` mount remapping is friendlier if you don't want to deal with uid mismatches at the filesystem level.

If `local_mode: true` is set but Kopi can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server — which in `--local` mode responds with `404 Not Found`. The symptom shows up in `gateway.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the Kopi host as the user the gateway runs as, and confirm a single file is `cat`-able without a permission error.
...
Kopi Agent works in Telegram group chats with a few considerations:
...
- In groups with multiple Kopi bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot profiles process it; other Kopi bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
...
### Multiple Kopi bots in one group

If you run several Kopi profiles in the same Telegram group, create one Telegram bot token per profile and start one gateway per profile. Do not reuse the same bot token in multiple running gateways; Telegram will reject concurrent polling for the same token.
...
With this setup, a group message like `@research_bot @ops_bot summarize this` is processed by `research_bot` and `ops_bot` only. Other Kopi bots in the group stay silent, even if the message is a reply to one of their earlier messages or would otherwise match a shared wake word.
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   Kopi profile uses a unique bot token and keep `exclusive_bot_mentions`
...
When this flag is on, Kopi still generates an internal session title (used by `kopi sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.
...
- **Bot API 9.5 (Mar 2026): Native streaming via `sendMessageDraft`.** Kopi supports Telegram's native streaming-draft API as an opt-in transport for private chats. The default remains the legacy `editMessageText` path because draft previews can visibly collapse and re-render on some Telegram clients.
...
**MarkdownV2 fallback.** When the rich path is unavailable for a message, Kopi converts markdown to MarkdownV2. Since MarkdownV2 has no native table syntax, pipe tables are normalized:
