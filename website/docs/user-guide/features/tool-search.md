...
**Tool Search** is Kopi' opt-in progressive-disclosure layer for that
...
:::info Built-in Kopi tools never defer
The tools that make up Kopi' core capability set (`terminal`,
...
When the model invokes `tool_call`, Kopi **unwraps the bridge** and
...
- **No JS sandbox.** Kopi uses the simpler "structured tools" mode
