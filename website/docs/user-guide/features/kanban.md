...
**Agent-side prevention:** Before the worker exits, Kopi injects up to two
...
guard is active only for dispatcher-spawned workers (`KOPI_KANBAN_TASK` is
set) and can be disabled with `KOPI_KANBAN_STOP_NUDGE=0`.
