...
Kopi can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Kopi becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default Kopi behavior is unchanged unless you flip the flag. Kopi never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? `kopi setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [KOPI Proxy](/integrations/subscription-proxy).
:::
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Kopi session.
- **Kopi' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Kopi for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Kopi' message shape so the self-improvement loop sees a normal-looking transcript.
...
These ship with `codex app-server` itself — no Kopi involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Kopi' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Kopi also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Kopi queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Kopi tool callback (MCP server, registered in `~/.codex/config.toml`)

Kopi registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Kopi' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Kopi' skill library.
- **`text_to_speech`** — TTS through Kopi' configured provider.

When the model wants one of these, codex spawns the `kopi_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Kopi' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Kopi tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Kopi' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Kopi' todo store (codex's `update_plan` is the in-runtime equivalent)
...
- The Kopi tool callback for browser_*, vision, image_gen, skills, TTS
...
|  | Kopi default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Kopi' own `kopi auth add openai-codex` writes to `~/.kopi/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Kopi auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   Kopi will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
- **Registers Kopi' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Kopi runtime)
```

## Self-improvement loop (memory + skill nudges)

Kopi' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Kopi runtime.
...
One detail: the review fork itself needs to call Kopi' agent-loop tools (`memory`, `skill_manage`), which require Kopi' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Kopi owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Kopi' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Kopi shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Kopi' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Kopi' managed block:

```toml
default_permissions = ":read-only"
```

(Kopi will preserve your override on re-migration as long as it lives outside the `# managed by kopi-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Kopi' auxiliary client uses the main provider/model when no per-task override is set.
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Kopi downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Kopi' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

Kopi wraps everything it manages between two marker comments:

```toml
# managed by kopi-agent — `kopi codex-runtime migrate` regenerates this section
default_permissions = ":workspace"
[mcp_servers.filesystem]
...
[plugins."github@openai-curated"]
...
# end kopi-agent managed section
```
...
- Add your own MCP servers Kopi doesn't know about
...
By default, Kopi points the codex subprocess at `~/.codex/` regardless of which Kopi profile is active. This means `kopi -p work` and `kopi -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.
...
```bash
# Inside the work profile, you might wrap kopi:
...
Kopi does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Kopi' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Kopi (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Kopi-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Kopi writes a `[plugins."<name>@openai-curated"]` block enabling it in your Kopi session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Kopi' codex runtime, Kopi activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Kopi doesn't touch credentials.

## Kopi tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Kopi registers itself as an MCP server in `~/.codex/config.toml`:
...
When the model calls `web_search` (or another exposed Kopi tool), codex spawns the `kopi_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Kopi runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Kopi Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Kopi UI
...
- Kopi web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Kopi auth and codex auth are separate sessions.** You need both `codex login` AND `kopi auth add openai-codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Kopi' `_import_codex_cli_tokens` — Kopi won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/kopi-agent/issues) with the output of `kopi logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Kopi shell (CLI / TUI / gateway) ───┐
...
For implementation details, see [PR #24182](https://github.com/NousResearch/kopi-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
