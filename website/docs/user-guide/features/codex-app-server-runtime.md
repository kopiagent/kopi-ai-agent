...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Kopi' own `kopi auth login codex` writes to `~/.kopi/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- **Kopi auth and codex auth are separate sessions.** You need both `codex login` AND `kopi auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Kopi' `_import_codex_cli_tokens` — Kopi won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
