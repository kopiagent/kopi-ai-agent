...
Kopi Agent supports both text-to-speech output and voice message transcription across all messaging platforms.
...
If the file contains `{transcript}` or `{{ transcript }}`, Kopi replaces that placeholder with the live TTS text. Otherwise, Kopi appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
Gemini 3.1 Flash TTS supports freeform square-bracket audio tags such as `[whispers]`, `[excitedly]`, `[very slow]`, `[laughs]`, and other expressive delivery notes. Enable `tts.gemini.audio_tags` to have Kopi run a hidden rewrite pass before Gemini TTS. The rewrite inserts inline tags into the TTS script only; the visible chat reply stays unchanged.
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `cd ~/.kopi/kopi-agent && uv pip install -e ".[mistral]"`.
...
Your command template can reference these placeholders. Kopi substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces are safe.
...
1. If `{output_path}` exists and is non-empty → Kopi reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout → Kopi uses that.
...
For `format: json` / `srt` / `vtt`, Kopi returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner — either configure `format: txt`, or post-process JSON downstream.
...
The shell command runs under the same user as Kopi with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `KOPI_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
