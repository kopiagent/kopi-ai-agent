...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [KOPI Proxy](/integrations/subscription-proxy) OAuth covers 300+ models, so most users don't need a pool when on KOPI Proxy.
:::
...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Kopi can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Kopi-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Kopi' credential store, so its token remains persistable.
