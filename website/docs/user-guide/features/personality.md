...
description: "Customize Kopi Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Kopi Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If the file is empty, whitespace-only, or cannot be read, Kopi falls back to a built-in default identity ("You are Kopi Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
- [SOUL.md Guide](/guides/use-soul-with-kopi)
