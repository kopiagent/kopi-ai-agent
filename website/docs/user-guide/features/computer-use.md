...
Kopi Agent can drive your desktop — clicking, typing, scrolling,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Kopi app). `kopi computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Kopi run declares its own cua-driver
...
Kopi intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Kopi-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Kopi skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Kopi
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Kopi agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Kopi agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Kopi warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Kopi from inside the RDP / console
...
upstream. **Kopi disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Kopi sets
...
unreleased fix — point Kopi at a binary you built from source instead
of the published release. Kopi resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Kopi testing (see notes).
...
### Option B — point Kopi straight at the built binary (fastest loop)
...
### Confirm Kopi is using your build
...
- **Kopi spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Kopi
  change needed either way.
- **Local builds skip the version check.** Kopi warns when the
...
- **Kopi-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Kopi `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Kopi (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Kopi among them)
