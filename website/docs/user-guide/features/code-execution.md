...
(plus `KOPI_RPC_DIR` / `KOPI_RPC_SOCKET` / `TZ` / `HOME`, which Kopi
...
the secret-stripping guarantee (Kopi-managed provider credentials can never
...
variables, Kopi emits a one-line `debug` log naming them and pointing at the
