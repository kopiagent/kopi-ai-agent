...
Kopi Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
Kopi stores session history so it can resume conversations, but it does not
...
Kopi explicitly injects for that turn.
...
For example, if a user sends an image and asks Kopi to make a meme from it,
Kopi may inspect that image once with vision and run an image-processing
...
<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Kopi session." />
...
read by Kopi. Safe to delete after verifying the corresponding session
exists in state.db.
:::
