...
:::tip
Inside the container, run `kopi setup --portal` once — the refresh token persists in the mounted `~/.kopi` volume. See [KOPI Proxy](/integrations/subscription-proxy).
:::
...
- **OAuth (KOPI Proxy)** — for hosted/public deploys: the `dashboard_auth/nous` provider activates whenever `KOPI_DASHBOARD_OAUTH_CLIENT_ID` is set.
