...
One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-kopi#wsl2-bridge-kopi-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Kopi'ss current working directory is `~`. In that case, start Kopi from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
The Kopi [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening Kopi quickly

If you just want a double-click launcher for an interactive Kopi shell, create
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "kopi"
   ```

3. Name it something obvious like `Kopi`.

That opens Windows Terminal, starts your WSL distro, drops you in your Linux
home directory, and launches Kopi. If `kopi` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run kopi` inside your project checkout.

Optional polish:

- **Custom icon:** open **Properties -> Change Icon** and point it at an `.ico`
  file, such as the Kopi favicon from the repo.
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside Kopi'ss test matrix — it may work with current drivers but we don't have a recipe to recommend.
...
Kopi'ss cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start Kopi from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-kopi#wsl2-bridge-kopi-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Kopi in WSL.
