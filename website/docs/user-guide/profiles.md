...
- `KOPI_HOME` is the profile boundary. It controls Kopi config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Kopi
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Kopi keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Kopi launches tool
...
Kopi also exposes `KOPI_REAL_HOME` to subprocesses so scripts can still find
