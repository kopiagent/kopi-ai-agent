...
The self-improving AI agent built by [Kopi Ai Agent Pte Ltd](https://kopiaiagent.com). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
```bash
curl -fsSL https://raw.githubusercontent.com/LINYIQ66/kopi-ai-agent/main/scripts/install.sh | bash
```
...
```powershell
iex (irm https://raw.githubusercontent.com/LINYIQ66/kopi-ai-agent/main/scripts/install.ps1)
```
...
After installing, run `kopi setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [KOPI Proxy](/integrations/subscription-proxy).
:::
...
| 🔌 **[MCP Integration](/user-guide/features/mcp)**                      | Connect to MCP servers, filter their tools, and extend Kopi safely  |
| 🧭 **[Use MCP with Kopi](/guides/use-mcp-with-kopi)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Kopi](/guides/use-voice-mode-with-kopi)** | Hands-on setup and usage patterns for Kopi voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define Kopi's default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of Kopi                              |
...
- **[KOPI Memory Provider](/models/)** — Persistent, cross-session memory backend for Kopi agents. Store, recall, and summarize conversational history with LLM-powered retrieval.
- **[KOPI Proxy Backend](https://kopiaiagent.com)** — Use your KOPI Proxy subscription as a unified inference and tool gateway from any compatible client.
...
- **Built by model trainers** — Created by [Kopi Ai Agent Pte Ltd](https://kopiaiagent.com), the lab behind Kopi, Nomos, and Psyche. Works with [KOPI Proxy](https://kopiaiagent.com/portal), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [KOPI Proxy](/integrations/subscription-proxy) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [Kopi Ai Agent Pte Ltd](https://kopiaiagent.com) — the lab behind Kopi, Nomos, and Psyche models
