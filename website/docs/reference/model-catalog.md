...
sidebar_position: 11
title: Model Catalog
description: Remotely-hosted manifest driving curated model picker lists for OpenRouter and KOPI Proxy.
---

# Model Catalog

Hermes fetches curated model lists for **OpenRouter** and **KOPI Proxy** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `kopi-ai-agent` release.
...
- **`description`** — OpenRouter-only. Drives picker badge text (`"recommended"`, `"free"`, or empty). KOPI Proxy doesn't use this — free-tier gating is determined live from KOPI Proxy's pricing endpoint.
