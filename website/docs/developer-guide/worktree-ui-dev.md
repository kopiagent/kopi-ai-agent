...
| Variable | Meaning |
|----------|---------|
| `KOPI_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `KOPI_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `KOPI_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Kopi itself — they're private to these helpers. The variables Kopi *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `kopi --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_kopi_root)" || { echo "htui: not in a Kopi checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$KOPI_MAIN_CHECKOUT/.venv/bin/python" -m kopi_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `KOPI_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_kopi_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `KOPI_TUI_DIR` are mutually exclusive
`KOPI_TUI_DIR` points Kopi at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `kopi --tui --dev` exits with an error. Run `unset KOPI_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_kopi_root)" || { echo "hgui: not in a Kopi checkout" >&2; return 1; }
  deps="${KOPI_GUI_DEPS_CHECKOUT:-$KOPI_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _kopi_link_deps "$desktop" "$deps/apps/desktop"
    _kopi_link_deps "$root" "$deps"
  else
  ...
  trap '_kopi_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    KOPI_DESKTOP_KOPI_ROOT="$root" \
    KOPI_DESKTOP_PYTHON="$KOPI_MAIN_CHECKOUT/.venv/bin/python" \
    KOPI_DESKTOP_IGNORE_EXISTING=1 \
    KOPI_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `KOPI_DESKTOP_KOPI_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `kopi`. |
| `KOPI_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `KOPI_DESKTOP_IGNORE_EXISTING` | Ignores any `kopi` on `PATH` so it can't shadow the worktree. |
| `KOPI_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Kopi checkout.
_kopi_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/kopi_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_kopi_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'kopi_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `kopi --tui --dev` and the `KOPI_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/kopi-ai-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `KOPI_*` variable Kopi reads
