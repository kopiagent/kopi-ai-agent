...
The Zed ACP Registry path launches the same adapter through `uvx --from 'kopi-agent[acp]==<version>' kopi-acp`, pointed at the `kopi-agent` PyPI release.
...
So ACP advertises and uses the currently configured Kopi provider/credentials. It also always advertises a terminal setup auth method (`kopi-setup`, args `--setup`) so first-run registry clients can open Kopi' interactive model/provider configuration before starting a normal ACP session.
