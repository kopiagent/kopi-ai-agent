...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Kopi Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Kopi Agent supports xAI Grok through a browser-based OAuth device-code login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Kopi automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Kopi — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Kopi opens or prints an accounts.x.ai verification URL
...
On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where Kopi cannot open a browser locally, Kopi prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and Kopi will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.
...
The same device-code flow applies when you sign in from the web dashboard or the desktop app: Kopi shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. Kopi requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. Kopi polls xAI until approval, then saves tokens to `~/.kopi/auth.json`.
4. From then on, Kopi refreshes the access token in the background — you stay signed in until you `kopi auth logout xai-oauth` or revoke access from your xAI account settings.
...
Kopi refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Kopi surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Kopi marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `kopi auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, Kopi raises a timeout error.

**Fix:** re-run `kopi auth add xai-oauth` (or `kopi model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions Kopi prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.
...
This is **not** a stale-token problem — re-running `kopi model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/kopi-agent/issues/26847)).
