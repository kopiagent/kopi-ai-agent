    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `kopi update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Kopi owns auxiliary retry + provider/model fallback policy (the
    ...
    # Kopi sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Kopi control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="kopi-protected-aux-provider",
        daemon=True,
    ).start()
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "KopiAgent",
}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Kopi Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Kopi plumbing (_reasoning_config
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import kopi_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=kopi_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_kopi_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``kopi_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._kopi_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Kopi' built-in discovery
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = kopi_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Kopi owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        from kopi_cli.providers import nous_api_mode
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(kopi_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Kopi reasoning into native
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Kopi reasoning config for direct model calls
