...
def _kopi_root_path() -> Path:
    """Resolve the Kopi root dir (always the parent of any profile, never per-profile)."""
    try:
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(kopi_home / "cache" / "bws_cache.enc.json"),
            str(kopi_root / "cache" / "bws_cache.enc.json"),
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside KOPI_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Kopi path.

    Three categories are blocked:

      * Internal Kopi cache files under ``KOPI_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under KOPI_HOME and the global Kopi
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Kopi root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in kopi_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Kopi credential store "
    ...
    for hd in kopi_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Kopi MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Kopi MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Kopi read (see
...
#
# Kopi profiles are separate KOPI_HOME dirs under
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Kopi scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Kopi entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Kopi profile {info['target_profile']!r}, but the "
...
# regardless of which Kopi profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner Kopi-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Kopi state.
    ...
    Detection is path-shape-only — does not require any Kopi resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Kopi process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Kopi state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Kopi "
