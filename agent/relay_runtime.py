"""Profile-scoped NeMo Relay runtimes owned by the Kopi agent core."""
...
SESSION_SCOPE = "kopi.session"
TURN_SCOPE = "kopi.turn"
LOGICAL_LLM_SCOPE = "kopi.logical_llm_call"
RUNTIME_SCHEMA_KEY = "kopi.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "kopi.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "kopi.relay.runtime_instance"
_PROFILE_KEY_CACHE: dict[str, str] = {}


@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Kopi session."""
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Kopi-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Kopi Relay session without creating one."""
        with self._sessions_lock:
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Kopi session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Kopi Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Kopi Relay session context is unavailable")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Kopi session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before Kopi authorizes a tool call."""
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Kopi Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Kopi Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Kopi profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Kopi Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one Kopi turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "kopi_relay_turn", default=None
)


class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Kopi core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Kopi Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"kopi.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "Kopi Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Kopi Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Kopi turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Kopi turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
            ...
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "kopi.execution_surface": lease.platform or "unknown",
                    },
                )
            except Exception:
                logger.warning("Kopi Relay turn initialization failed", exc_info=True)
                ...
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        try:
                            ...
                            logger.warning(
                                "Kopi Relay turn finalization failed", exc_info=True
                            )
            finally:
                try:
                    ...
                    logger.warning(
                        "Kopi Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            try:
                ...
                logger.warning(
                    "Kopi Relay logical LLM finalization failed",
                    exc_info=True,
                )
                break
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Kopi session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Kopi Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at Kopi's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Kopi core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Kopi Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Kopi session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Kopi Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Kopi Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Kopi session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Kopi Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active Kopi profile."""
