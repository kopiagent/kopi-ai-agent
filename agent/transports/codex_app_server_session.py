...
Owns one Codex thread per Kopi session. Drives `turn/start`, consumes
...
# Kopi' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Kopi/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Kopi session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Kopi' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Kopi' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Kopi' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Kopi
        approval-mode/timeout logic. The Kopi-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Kopi approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Kopi approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Kopi returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Kopi approval mode/timeout resolution
...
def _get_kopi_version() -> str:
    """Best-effort Kopi version string for codex's userAgent line."""
    try:
