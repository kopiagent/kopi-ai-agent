...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Kopi does
...
class EntraIdentityConfig:
    ...
    Captures the Kopi-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Kopi-selected knobs are passed as kwargs. Everything else
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Kopi processes use exactly one Entra config at a time (the
