...
(``gateway/platforms/webhook.py``): inbound wakes Kopi when the world
changes; outbound tells the world when Kopi does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Kopi-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``KOPI_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.kopi/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/kopi-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: KOPI_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `kopi hooks list`
...
Headers::
    ...
    User-Agent:              Kopi-Agent-Outbound-Webhook
    X-Kopi-Event:          <hook event name>
    X-Kopi-Delivery:       <delivery_id>
    X-Kopi-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("KOPI_SAFE_MODE"):
        logger.info("KOPI_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Kopi-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Kopi-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
