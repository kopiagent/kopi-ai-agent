...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Kopi accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Kopi' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Kopi should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Kopi state.

    The app-server owns the compacted thread context, so Kopi should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Kopi deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Kopi UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Kopi tool loop. Without this bridge gateway
...
# Codex item types that map to a Kopi tool_call in the projector (and
...
# Internal MCP server that wraps Kopi' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# kopi-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.kopi-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as Kopi tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "kopi-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Kopi tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Kopi' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Kopi' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Kopi'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Kopi' standard prompt flow if a
        ...
        # explicitly opted out of Kopi approvals — via `approvals.mode: off`
        ...
        # with a missing Kopi UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into Kopi' gateway UI callbacks
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            try:
                ...
                # and close the physical attempt scope before Kopi returns.
