...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_kopi_home() / "cache" / "local_endpoint_probes.json"
...
# Minimum context length required to run Kopi Agent.  Models with fewer
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Kopi' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Kopi minimum.
...
def get_model_context_length(
    ...
    # ``Kopi-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
