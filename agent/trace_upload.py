"""Upload a Kopi session transcript to Hugging Face as an agent trace.

Kopi stores sessions in its own SQLite store (``kopi_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Kopi' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Kopi OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Kopi' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Kopi conversation messages as Claude Code JSONL text.
