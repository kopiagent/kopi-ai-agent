...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    try:
        ...
        from kopi_cli.models import normalize_provider as normalize_model_provider
        from kopi_cli.providers import normalize_provider as normalize_registry_provider
...
def init_agent(
    ...
    agent._cache_ttl = "5m"
    try:
        from kopi_cli.config import load_config_readonly as _load_pc_cfg
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from kopi_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = kopi_xai_default_headers()
    ...
    try:
        from kopi_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `kopi --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from kopi_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Kopi behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Kopi' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "kopi", "off"}:
        _ra().logger.warning(
            ...
            "Valid values are: native, kopi, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from kopi_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Kopi Agent.  Choose a model with at least "
    ...
    # Nous Kopi 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            ...
            if _kopi_warn:
                _user_msg = (
                    "⚠ Nous Research Kopi 3 & 4 models are NOT agentic — they "
