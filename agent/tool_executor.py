...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Kopi process cwd (notably in Docker).  Resolve
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Kopi process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Kopi policy and dispatch exactly once."""
    # Pixel-office visualization (KOPI): the ONE per-tool choke point both the
    # concurrent and sequential execution paths pass through, and it runs AFTER
    # tool_search unwrap so function_name is already the real underlying tool.
    # Record it here so /office shows the live tool at the NPC's feet.
    # Best-effort; never affects execution.
    try:
    ...
    def _kopi_pipeline(relay_args: dict[str, Any]) -> Any:
