...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in kopi_state.py — kept local
# to avoid importing kopi_state at module load time (its module-level
# DEFAULT_DB_PATH = get_kopi_home() / "state.db" breaks tests that
# monkeypatch get_kopi_home to return a str).
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.kopi/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Kopi tool use: "
    ...
    return (
        ...
        f"context, but Kopi needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Kopi config, set `model.ollama_num_ctx: 65536` "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Kopi' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
def run_conversation(
    ...
    # Adopt any ~/.kopi/.env credential/base-url edits made since the last
    ...
    # all run inside Codex). Default Kopi path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        #
        # Kopi invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Kopi tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from kopi_cli.lifecycle import (
                        has_hook,
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Kopi/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    from kopi_constants import display_kopi_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Kopi-managed OAuth/setup tokens")
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from kopi_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `kopi model`."
                        )
                ...
                # which Kopi' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Kopi' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`kopi",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Kopi may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See kopi-lcm engine
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"timeout. (Kopi's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Kopi mid-turn, resume logic still sees the
                ...
                # timeout (KOPI_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from kopi_cli.lifecycle import has_hook
                    from kopi_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("KOPI_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
