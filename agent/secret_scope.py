...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Kopi writes
    ...
    ``kopi_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Kopi parser: save_env_value
    ...
    from kopi_cli.config import _parse_env_value
...
def build_profile_secret_scope(kopi_home: Path) -> Dict[str, str]:
    ...
    try:
        from kopi_cli.env_loader import get_secret_source_values
