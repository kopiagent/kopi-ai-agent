...
configured under ``monitoring.export.otlp``. Lets an operator stream Kopi
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install kopi-ai-agent[otlp]``), imported lazily so the
...
def _require_sdk(*, auto_install: bool = True, prompt: bool = True):
    ...
    try:
        ...
        raise OTLPUnavailable(
            ...
            "    pip install 'kopi-ai-agent[otlp]'\n"
            f"(import error: {e})"
        )
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"kopi.event": kind or "unknown"}
    keep_by_kind = {
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    tracer = provider.get_tracer("kopi.monitoring")
    n = 0
    for ev in batch:
        try:
            name = f"kopi.{ev.get('event', 'event')}"
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'kopi-ai-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
