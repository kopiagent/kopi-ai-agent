...
row taxonomy is inferred from the atlas shape so Kopi can render both legacy
...
# rows (1536x1872), while older Kopi/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Kopi' activity state names. They are not always identical to the
...
# Legacy Kopi/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Kopi activity names -> accepted row-name aliases in descending
