...
# Org mirrors live under ~/.kopi/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # The Kopi Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Kopi can discover and view
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Kopi metadata, VCS, virtualenv/dependency, cache, and skill
