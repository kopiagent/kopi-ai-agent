...
# Redirect a Kopi-managed Node's `npm install -g` to the command link dir
...
_nb_npm_range() {
    if [ -n "${KOPI_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$KOPI_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through kopi_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$KOPI_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$KOPI_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_install_bundled_node() {
    ...
    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # KOPI_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${KOPI_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
...
# ---------------------------------------------------------------------------
# Heal a broken Kopi-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
# The managed node runs but is below KOPI_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$KOPI_HOME/node/bin/node" "$KOPI_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$KOPI_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
heal_managed_node() {
    ...
    _nb_log "Kopi-managed Node is broken — redownloading to $KOPI_HOME/node/..."
    _nb_install_bundled_node
}
