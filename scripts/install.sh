...
# Configuration
REPO_URL_SSH="git@github.com:kopiagent/kopi-ai-agent.git"
REPO_URL_HTTPS="https://github.com/kopiagent/kopi-ai-agent.git"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://kopiaiagent.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Kopi owns its own uv at $KOPI_HOME/bin/uv.  Always install there —
...
# with the Kopi-managed Node $NODE_VERSION.
...
check_node() {
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing Kopi-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Kopi requires Node >=26) — installing Kopi-managed Node $NODE_VERSION..."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Kopi Agent itself does not require or retain root access."
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Kopi Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Kopi Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Kopi Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
# KOPI fork: provision a KOPI Proxy API key into ~/.kopi/.env.
# Resolution order (first hit wins), all non-interactive:
#   1. KOPI_API_KEY already present in .env  → keep it (idempotent reinstall)
#   2. KOPI_API_KEY set in the environment   → persist it
#   3. auto-provision endpoint               → POST for a free-quota key
# The config template reads ${KOPI_API_KEY}, so writing it here is enough.
provision_kopi_proxy_key() {
    local env_file="$KOPI_HOME/.env"
    local base_url="${KOPI_PROXY_BASE_URL:-https://kopiaiagent.com/v1}"

    if [ -f "$env_file" ] && grep -q '^KOPI_API_KEY=..*' "$env_file" 2>/dev/null; then
        log_info "KOPI Proxy key already present in .env — skipping provisioning"
        return 0
    fi

    local key="${KOPI_API_KEY:-}"
    if [ -z "$key" ]; then
        log_info "Provisioning a free KOPI Proxy API key..."
        local resp
        resp=$(curl -s -X POST "${base_url}/auto-provision/ready" \
...
" 2>/dev/null || echo "")
    ...
    if [ -z "$key" ]; then
        log_warn "Could not auto-provision a KOPI Proxy key; add one later with KOPI_API_KEY in ~/.kopi/.env"
        return 1
    fi

    # Replace any placeholder line, then append the real key.
    if [ -f "$env_file" ]; then
        grep -v '^KOPI_API_KEY=' "$env_file" > "$env_file.tmp" 2>/dev/null || true
        mv "$env_file.tmp" "$env_file" 2>/dev/null || true
    fi
    printf 'KOPI_API_KEY=%s\n' "$key" >> "$env_file"
    chmod 600 "$env_file"
    log_success "KOPI Proxy API key provisioned into ~/.kopi/.env"
    return 0
}
...
copy_config_templates() {
    ...
    # KOPI fork: auto-provision the KOPI Proxy API key here, in the
    # non-interactive `config` stage, so both the CLI installer AND the
    # desktop bootstrap (which skips the interactive `setup`/`gateway`
    # stages) land with a working key. The config template reads
    # ${KOPI_API_KEY} from this .env. Best-effort: a provisioning failure
    # must never fail the stage — the user can still add a key later.
    provision_kopi_proxy_key || true
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Kopi Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Kopi will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Kopi Agent browser tools — explicit browser override."
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.kopi-bootstrap-complete, which tells the Kopi
    ...
    local marker_path="$INSTALL_DIR/.kopi-bootstrap-complete"
...
# extract a tree MISSING the electron binary, so the `electron`->`Kopi` rename
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Kopi-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Kopi" ]; then
            app="$desktop_dir/release/linux-unpacked/Kopi"
        elif [ -x "$desktop_dir/release/linux-unpacked/kopi" ]; then
            app="$desktop_dir/release/linux-unpacked/kopi"
        fi
    else
        local cand
    ...
    # `kopi desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading KOPI_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if KOPI_HOME="$KOPI_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from kopi_cli.main import _desktop_macos_relaunchable_fixup
