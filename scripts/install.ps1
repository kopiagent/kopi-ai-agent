...
# Put the Kopi-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone kopi-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Kopi-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    $tmpCwd = Join-Path $env:TEMP ("kopi-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Kopi setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Kopi-managed PortableGit install instead..."
    }
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Kopi requires Node >=26)"
    }
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$KopiHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$KopiHome\node"
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. kopi-setup.exe bakes its build-time commit
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/kopiagent/kopi-ai-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/kopiagent/kopi-ai-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/kopiagent/kopi-ai-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                ...
                if ($extractedDir) {
                    ...
                    # `kopi update` (see the notes at the shared clone-path
...
function Install-Desktop {
    ...
    # enforces the build floor (Node >=26) and prepends the Kopi-managed
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, kopi-ai-agent#38216).
