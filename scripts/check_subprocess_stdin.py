...
When Kopi runs in TUI mode, the gateway child process communicates with
...
# ``get_kopi_home() / "plugins"`` (user) and ``./.kopi/plugins/`` (project,
# gated behind ``KOPI_ENABLE_PROJECT_PLUGINS``) — see
# ``kopi_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``kopi_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import kopi_constants (this script
    ...
    # plugins in get_kopi_home()/plugins/ and project plugins in
    # ./.kopi/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_kopi_home() / "plugins"]
    if os.environ.get("KOPI_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".kopi" / "plugins")
