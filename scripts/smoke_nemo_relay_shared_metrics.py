...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["kopi.model_call.count"] != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['kopi.model_call.count']}"
        )
    expected_start = {
...
def _validate_package(outbox: Path, schema_path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if metrics["kopi.model_call.count"]["dimensions"] != {
        "call_role": "primary",
        "locality": "local",
        "model_family": "gpt",
        "outcome": "success",
        ...
        raise AssertionError(
            f"Unexpected model metric: {metrics['kopi.model_call.count']}"
        )
...
def main() -> int:
    ...
    if not _ModelHandler.requests:
        raise AssertionError("Kopi did not call the local model endpoint")
    ...
    package_path, package = _validate_package(
        ...
        / "observability"
        / "schemas"
        / "kopi.shared_metrics.v1.schema.json",
    )
