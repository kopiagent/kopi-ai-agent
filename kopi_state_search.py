...
``kopi_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import kopi_state (cycle) — shared
module-level constants live in kopi_state_common.
"""
...
# Moved methods logged under the "kopi_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("kopi_state")
