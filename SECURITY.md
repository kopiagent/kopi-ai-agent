# Kopi Agent Security Policy

This document describes Kopi Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/kopiagent/kopi-ai-agent/security/advisories/new)
or **security@kopiaiagent.com**. Do not open public issues for
security vulnerabilities. **Kopi Agent does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Kopi Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Kopi Agent's documentation
...
Kopi Agent supports two OS-level isolation postures. They address
...
Kopi Agent supports this in two ways:

- **Kopi Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Kopi Agent's in-process heuristics
...
Kopi Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Kopi Agent
itself. Bugs in Kopi Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Kopi Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Kopi Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Kopi Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Kopi Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Kopi Agent. Bugs in Kopi
...
- Kopi Agent includes supply-chain guards for MCP server
