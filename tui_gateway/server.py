...
def _reap_idle_sessions() -> None:
    ...
    try:
        from kopi_cli.mem_trim import trim_memory
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``kopi`` / ``kopi --tui`` / CLI) is started from a
    ...
    try:
        ...
        from kopi_state import is_disk_full_error
    ...
    *,
    ...
    def run():
        ...
        try:
            ...
            # Run while any profile-specific KOPI_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile KOPI_HOME
    override, so ``get_kopi_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _kopi_home
    return base / "images"
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Kopi
