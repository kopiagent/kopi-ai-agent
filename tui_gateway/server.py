...
def _inside_compute_host_child() -> bool:
    return os.environ.get("KOPI_COMPUTE_HOST_CHILD") == "1"
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``kopi`` / ``kopi --tui`` / CLI) is started from a
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read KOPI_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # KOPI_SESSION_ID even though agent_init set it via
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``kopi config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_kopi_home_override()
    return Path(override if isinstance(override, str) and override else _kopi_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Kopi activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="kopi-change-watcher", daemon=True).start()
...
def _resolve_model() -> str:
    ...
    try:
        from kopi_cli.models import get_preferred_silent_default_model
...
def _load_approval_mode() -> str:
    ...
    canonical ``kopi_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`kopi_constants.resolve_reasoning_config` (per-model override >
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base Kopi): with no explicit pin, collapse to the
...
def _load_fallback_model():
    ...
    stays in parity with ``KopiCLI.__init__`` and ``gateway/run.py``:
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from kopi_cli.fallback_config import resolve_entry_api_key
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped KOPI_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / KOPI_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The KOPI_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_kopi_home)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Kopi has to support both the legacy 8-row petdex atlas and the current
...
# (agent/billing_view.py + kopi_cli/nous_billing.py) — same as /topup.
...
def _(rid, params: dict) -> dict:
    ...
    from kopi_cli.nous_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from kopi_cli.nous_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from kopi_cli.nous_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from kopi_cli.nous_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from kopi_cli.nous_billing import BillingError
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        home_token = (
            set_kopi_home_override(parent_home) if parent_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    from kopi_cli.input_sanitize import sanitize_user_prompt_text
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=KOPI_SESSION_KEY`` (see
    ...
    # DB when KOPI_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``KOPI_UI_SESSION_ID`` as
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from kopi_cli.voice import is_continuous_active
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Kopi Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Kopi/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Kopi Desktop. If it serves Kopi/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from kopi_cli.model_switch import parse_model_switch_args
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    KOPI_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    kopi_home = os.path.normcase(os.path.realpath(str(get_kopi_home())))
    return real == home or real == kopi_home
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Kopi sessions
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Kopi provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    if name == "init":
        ...
        from kopi_cli.init_command import build_init_prompt_for_cwd
...
def _(rid, params: dict) -> dict:
    try:
        from kopi_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from kopi_cli.inventory import build_models_payload
        ...
        # Save the key to ~/.kopi/.env via the unified credential lifecycle
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Kopi" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``kopi_cli.voice.set_voice_busy_probe``)
    ...
    try:
        ...
        if silent and not hint:
            hint = ("Microphone delivers only silence — on macOS grant the "
                    "Kopi backend mic access (System Settings > Privacy & "
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            # are tolerated.
            try:
                from kopi_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import kopi_cli.voice  # noqa: F401
