...
def _default_registry_path() -> Path:
    return get_kopi_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_kopi_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_kopi_home = expected_kopi_home if expected_kopi_home is not None else str(get_kopi_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["KOPI_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("kopi_home") or "")
        if got_home and got_home != self.expected_kopi_home:
            raise RuntimeError(f"compute host KOPI_HOME mismatch: {got_home} != {self.expected_kopi_home}")
