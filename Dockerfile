...
# node:26 image instead (Kopi pins its toolchain to Node 26 everywhere).
...
sys.exit('SQLite FTS5 trigram self-test failed') if db.execute(\"SELECT count(*) FROM docs WHERE docs MATCH 'opi'\").fetchone()[0] != 1 else None; \
db.close()"
...
# 'Kopi WebUI' catalog, NAS compose projects that preserve an old
...
#
# The [otlp] extra contains the SDK/exporter imported by Kopi when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Kopi production image.
#
...
COPY --chmod=0755 docker/kopi-exec-shim.sh /opt/kopi/bin/kopi
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/kopi/docker/entrypoint-dispatch.sh
...
ENTRYPOINT [ "/opt/kopi/docker/entrypoint-dispatch.sh" ]
CMD [ ]
