...
//! Electron app's APP_ROOT/../.. trick, because Kopi-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Kopi-Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/NousResearch/kopi-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "kopi-setup/0.0.1")
        .send()
        .await
...
mod tests {
    ...
    fn upgrade_cached_script_adds_bom_to_legacy_ps1() {
        ...
        let dir = std::env::temp_dir().join(format!("kopi-bom-test-{}", std::process::id()));
    ...
    fn upgrade_cached_script_leaves_sh_untouched() {
        let dir = std::env::temp_dir().join(format!("kopi-bom-sh-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
