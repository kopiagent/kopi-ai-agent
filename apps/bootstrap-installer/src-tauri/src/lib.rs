...
pub fn run() {
    ...
    // Kopi is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Kopi installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Kopi is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Kopi" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Kopi" shortcuts that install.ps1 creates, and a
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::kopi_home().join("kopi-agent");
        ...
        .expect("error while running Kopi Setup");
}
