 ...
 * packaged binary) with a real `kopi serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → kopi serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Kopi title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Kopi')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → kopi serve is spawned → WS connects → config
