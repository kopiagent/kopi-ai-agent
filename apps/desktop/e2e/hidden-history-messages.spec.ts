...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.kopiHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.kopiHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.kopiHome)
  const builder = await RealSessionBuilder.start(sandbox.kopiHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.kopiHome, mock.url)
  fs.appendFileSync(path.join(sandbox.kopiHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.kopiHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
