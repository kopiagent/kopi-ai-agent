 ...
 * AIAgent loop, using the E2E mock provider configured in `kopiHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(kopiHome: string) {
  ...
  static async start(kopiHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(kopiHome)
