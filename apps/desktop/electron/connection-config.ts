 ...
 *     `X-Kopi-Session-Token` header; WS uses `?token=`.
...
// The Nous portal (NAS) does NOT use Kopi gateway session cookies — it is a
...
// Kopi Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteKopiPath = String(entry.remoteKopiPath || '').trim()
 ...
 *     `--profile`, so its `KOPI_HOME` scopes it.
 *
 ...
 * the Kopi gateway session cookies, so cloud sign-in / discovery liveness
