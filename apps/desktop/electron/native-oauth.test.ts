...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/kopi', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/kopi/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/kopi'), 'https://gw.example.com/kopi/auth/native/refresh')
})
