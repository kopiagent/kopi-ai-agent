...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'kopi-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'kopi-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Kopi Test'], { cwd: dir })
