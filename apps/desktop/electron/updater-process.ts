 ...
 *
 * The Tauri installer self-copies into KOPI_HOME on *every* platform
 * (`kopi-setup.exe` on Windows, `kopi-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_kopi_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `kopi update`,
 ...
 */
export function resolveStagedUpdaterBinary(kopiHome: string, deps: ResolveStagedUpdaterBinaryDeps = {}): string | null {
  ...
  const candidate = path.join(kopiHome, 'kopi-setup.exe')
 ...
 *
 * `copy_self_to_kopi_home` deliberately no-ops during `--update`
 ...
 * "Another Kopi update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `kopi update`, which
 ...
 * the installer is written to KOPI_HOME at install/repair time, making mtime
