...
export interface KopiReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForKopiReady`).
   */
...
export async function waitForKopiReady(baseUrl: string, options: KopiReadyOptions): Promise<void> {
  ...
  throw new Error(`Kopi backend did not become ready: ${detail}`)
}
