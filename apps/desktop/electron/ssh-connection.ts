...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(KOPI_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Kopi-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.kopi/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'kopi-desktop-ssh')
  }

  return path.join(os.homedir(), '.kopi', 'desktop-ssh')
}
