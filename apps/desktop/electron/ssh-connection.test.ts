...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env KOPI_DASHBOARD_SESSION_TOKEN=abc123deadbeef KOPI_DESKTOP=1 kopi dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /KOPI_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /KOPI_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Kopi-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Kopi-Session-Token: hdr_888'), /X-Kopi-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Kopi-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('kopi', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v kopi', 15000)
  assert.equal(args[args.length - 1], 'command -v kopi')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `kopi-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `kopi-ssh-test-${process.pid}-${Date.now()}`)
