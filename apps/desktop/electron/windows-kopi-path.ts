/**
 * windows-kopi-path.ts
 *
 * Pure, dependency-injected pieces of Windows `kopi` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvKopiCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `kopi` shim shadowed the real
 *      kopi.cmd/kopi.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\kopi.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvKopiCommand() — unwrapWindowsVenvKopiCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `kopi`) must
 * not shadow `kopi.cmd`/`kopi.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv kopi shim, bootstrap-complete
 * marker) — gating on just the kopi.exe console-script shim alone is the
...
export interface ResolveVenvKopiCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportKopiCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `kopi`/`kopi.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/kopi(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m kopi_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportKopiCli(). A
 * venv whose update died mid-`pip install` still has python.exe + kopi.exe
 ...
 *
 * Returns null when `command` is not a venv kopi shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvKopiCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvKopiCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^kopi(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Kopi at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Kopi Python at ${python}`,
    command: python,
