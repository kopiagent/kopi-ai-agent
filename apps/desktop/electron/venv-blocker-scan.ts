...
const SCAN_MODULE = 'kopi_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Kopi process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Kopi installation is free.\n' +
    '\n' +
    'Close other Kopi windows and terminals, then retry.  If the problem\n' +
    'persists, run `kopi update` in a terminal for detailed diagnostics.'
  )
}
