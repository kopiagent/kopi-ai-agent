 ...
 * origin=git@github.com:NousResearch/kopi-ai-agent.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  ...
  assert.equal(canonicalGitHubRemote('git@github.com:NousResearch/kopi-ai-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/NousResearch/kopi-ai-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/NousResearch/kopi-ai-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:nousresearch/kopi-ai-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/NousResearch/kopi-ai-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@github.com:NousResearch/kopi-ai-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@github.com/NousResearch/kopi-ai-agent.git'), true)
  assert.equal(isSshRemote('https://github.com/NousResearch/kopi-ai-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  ...
  assert.equal(isOfficialSshRemote('git@github.com:NousResearch/kopi-ai-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/NousResearch/kopi-ai-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@gitlab.com:NousResearch/kopi-ai-agent.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://github.com/NousResearch/kopi-ai-agent.git'), false)
