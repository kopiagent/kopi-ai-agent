...
const READY_RE = /^KOPI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitKopiPath = '') {
  const explicit = psLiteral(explicitKopiPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$kopiHome=$env:KOPI_HOME',
    'if(-not $kopiHome){$kopiHome=Join-Path $env:LOCALAPPDATA "kopi"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command kopi.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $kopiHome "kopi-ai-agent\\venv\\Scripts\\kopi.exe")',
    '$candidates+=(Join-Path $HOME "kopi-ai-agent\\.venv\\Scripts\\kopi.exe")',
    '$kopi=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $kopi){throw "Kopi is not installed on the remote Windows host."}',
    'if($explicit -and $kopi -ne $explicit){throw "The configured Kopi path is not an executable file."}',
    '$python=Join-Path (Split-Path $kopi) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Kopi Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;kopiHome=$kopiHome;kopiPath=$kopi;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitKopiPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitKopiPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'kopi_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.kopiPath === 'string' &&
    typeof lock.kopiHome === 'string'
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteKopiPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.kopiPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Kopi on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.kopiPath = inspection.path
  const kopiVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located kopi at ${runtime.kopiPath}`)
  ...
  if (validLock(lock, ownershipId)) {
    ...
    const reusable =
      ...
      lock.kopiPath === runtime.kopiPath &&
      lock.kopiHome === runtime.kopiHome
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, kopiPath: runtime.kopiPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForKopi(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Kopi SSH"', 'powershell.exe -NoLogo')
