...
test('locateKopi prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/kopi/, 'OK']])
  assert.equal(await locateKopi(ssh, '/opt/kopi'), '/opt/kopi')
})

test('locateKopi throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong kopi" bug.
  const ssh = fakeSsh([
    [/command -v kopi/, '/home/u/.local/bin/kopi\n'],
    [/\[ -x .*\.local\/bin\/kopi/, 'OK']
  ])

  await assert.rejects(
    () => locateKopi(ssh, '/bad/path/kopi'),
    (err: any) => {
      assert.equal(err.kind, 'kopi-not-found')
      assert.match(err.message, /\/bad\/path\/kopi/)
...
test('locateKopi falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v kopi/, '/home/u/.local/bin/kopi\n'],
    [/\[ -x .*\.local\/bin\/kopi/, 'OK']
  ])

  assert.equal(await locateKopi(ssh, ''), '/home/u/.local/bin/kopi')
})

test('locateKopi canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v kopi/, '/home/u/.local/bin/kopi\n'],
    [/\[ -x .*\.local\/bin\/kopi/, 'OK'],
    [/python3 -c/, '/home/u/.kopi/kopi-ai-agent/venv/bin/kopi\n']
  ])

  assert.equal(await locateKopi(ssh, ''), '/home/u/.kopi/kopi-ai-agent/venv/bin/kopi')
})

test('locateKopi falls back to ~/.local/bin/kopi when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateKopi(ssh, ''), '~/.local/bin/kopi')
})

test('locateKopi tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/kopi/, 'OK']])
  assert.equal(await locateKopi(ssh, ''), '~/.kopi/kopi-ai-agent/venv/bin/kopi')
})

test('locateKopi throws a kopi-not-found error with an install hint', async () => {
...
test('locateKopi uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v kopi/, '/x/kopi'],
    [/\[ -x/, 'OK']
  ])

  await locateKopi(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.kopi/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.kopi/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.kopi/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/kopi'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/kopi serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/kopi'), true)
  assert.equal(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/kopi', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('KOPI_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { kopiPath: '/x/kopi', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('KOPI_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('KOPI_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { kopiPath: '/x/kopi', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.kopi\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/print\("OWNED"/, 'OWNED\n'],
    [/kill 333/, ''],
    [/--version/, 'Kopi Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile kopiPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ kopiPath: '/old/stale/kopi', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteKopiPath: '/new/kopi', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes kopiHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"kopiHome":"\/home\/alice\/\.kopi"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'KOPI_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/kopi'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.kopi/kopi-ai-agent/venv/bin/kopi'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/kopi'))
  assert.doesNotThrow(() => validateRemotePath('~/.kopi/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/kopi'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('kopi'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/kopi'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/kopi\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/kopi\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/kopi\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/kopi', '/usr/`id`/kopi', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.kopi/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/kopi')
  assert.ok(result.includes('/usr/local/bin/kopi'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/kopi')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('KOPI_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/kopi', 'work', {
    tokenFilePath: `~/.kopi/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.kopi\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/kopi', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { kopiPath: '/x/kopi', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { kopiPath: '/x/kopi', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.kopi/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.kopi/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/kopi serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/kopi serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/kopi'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/kopi'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/kopi'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/kopi'), false)
})
