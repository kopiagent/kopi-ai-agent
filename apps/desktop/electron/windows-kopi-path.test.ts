// Unit tests for the pure Windows `kopi` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvKopiCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `kopi` shim shadows
//      the real kopi.cmd/kopi.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the kopi.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvKopiCommand() — must probe the venv python via
//      canImportKopiCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvKopiCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportKopiCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    kopiHome: '/fake/kopi-home',
...
test('resolveVenvKopiCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvKopiCommand('/root/venv/Scripts/kopi.exe', [], deps), null)
})

test('resolveVenvKopiCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvKopiCommand('/root/venv/Scripts/kopi.cmd', [], deps), null)
})

test('resolveVenvKopiCommand: returns null when the basename is not kopi/kopi.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvKopiCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvKopiCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvKopiCommand('/root/venv/bin/kopi.exe', [], deps), null)
})

test('resolveVenvKopiCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvKopiCommand('/root/venv/Scripts/kopi.exe', [], deps), null)
})

test('resolveVenvKopiCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportKopiCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvKopiCommand('/root/venv/Scripts/kopi.exe', ['serve'], deps)
...
test('resolveVenvKopiCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvKopiCommand('/root/venv/Scripts/kopi.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'kopi_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvKopiCommand: is case-insensitive on kopi.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvKopiCommand('/root/venv/Scripts/KOPI.EXE', [], deps))
  assert.ok(resolveVenvKopiCommand('/root/venv/SCRIPTS/kopi.exe', [], deps))
})
