// Profile-delete routing logic for the `kopi:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_kopi_home() and
...
/**
 * Parse a `kopi:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `kopi:api` request away from the primary/window backend
 ...
 * ensure_kopi_home() recreates the directory the delete just removed.
 */
