 ...
 *   1. KOPI_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\kopi\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect KOPI_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.KOPI_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'kopi', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'kopi', 'git', 'usr', 'bin', 'bash.exe'))
  }
