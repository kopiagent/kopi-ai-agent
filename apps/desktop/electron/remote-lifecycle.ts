 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Kopi dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Kopi install on the remote (login-shell probe),
 ...
 *     `KOPI_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForKopi in.
 *
 * The minted KOPI_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^KOPI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.kopi/desktop-ssh'
...
// Resolve the remote kopi executable. An EXPLICIT path is honored strictly
...
async function locateKopi(ssh, remoteKopiPath) {
  ...
  if (remoteKopiPath) {
    if (await isExecutable(remoteKopiPath)) {
      return resolveLauncher(remoteKopiPath)
    }

    const err: any = new Error(
      `The Kopi path you set is not an executable on the remote host: "${remoteKopiPath}". ` +
        'Check the path (it must be the full path to the `kopi` binary on the remote, e.g. ' +
        '~/kopi-ai-agent/.venv/bin/kopi), or clear it to auto-detect.'
    )

    err.kind = 'kopi-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v kopi')}`)).trim()
  ...
  candidates.push('~/.local/bin/kopi')
  candidates.push('/usr/local/bin/kopi')
  candidates.push('~/.kopi/kopi-ai-agent/venv/bin/kopi')
  ...
  const err: any = new Error(
    'Kopi is not installed on the remote host (could not find a `kopi` executable). ' +
      'Install it on the remote with:  curl -fsSL https://kopi-ai-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Kopi path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<kopi> --version`,
// e.g. "Kopi Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH kopi a
// connection uses, so a stale/unexpected install is visible.
async function probeKopiVersion(ssh, kopiPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(kopiPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Kopi Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The KOPI_HOME the remote dashboard will use (explicit env wins, else
// ~/.kopi). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteKopiHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${KOPI_HOME:-$HOME/.kopi}"')).trim().split('\n').pop()

    return out || '~/.kopi'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Kopi home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'kopiPath', 'kopiHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, kopiPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !kopiPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(kopiPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.kopiPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(kopiPath, profile, opts: any = {}) {
  const kopi = expandRemotePath(kopiPath)
  ...
  const dashCmd = `env KOPI_DESKTOP=1 ${kopi} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, kopiPath) {
  const kopi = expandRemotePath(kopiPath)

  const out = await ssh.exec(
    `help="$(${kopi} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { kopiPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, kopiPath))) {
    const err: any = new Error(
      'The remote Kopi install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Kopi on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(kopiPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForKopi, a token-gated
...
async function connect(deps) {
  ...
  const kopiPath = await locateKopi(ssh, remoteKopiPath)
  log(`located kopi at ${kopiPath}`)
  const kopiVersion = await probeKopiVersion(ssh, kopiPath)

  if (kopiVersion) {
    log(`remote kopi version: ${kopiVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const kopiHome = await probeRemoteKopiHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.kopiPath))
    ...
    const reusable =
      ...
      lock.kopiPath === kopiPath &&
      lock.kopiHome === kopiHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForKopi(baseUrl, spawnToken)
    assertNotAborted(signal)
