...
How to build Kopi Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Kopi surface can also
...
- A **runtime home change** (switching the underlying `KOPI_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Kopi — do not assume
