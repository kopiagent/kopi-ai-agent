#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Kopi icon + version metadata onto the
// built Kopi.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Kopi.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Kopi.exe>
//
...
// Stamp the Kopi icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Kopi',
      FileDescription: 'Kopi',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Kopi icon + identity stamped')
}
