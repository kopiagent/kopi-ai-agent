...
//   · its own KOPI_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · KOPI_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated KOPI_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate KOPI_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = kopiHome ?? mkTemp('kopi-perf-home-')
  const userData = userDataDir ?? mkTemp('kopi-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !kopiHome) {
    seedConfigFrom(join(homedir(), '.kopi'), home)
  }
  ...
  try {
    ...
    // KOPI_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set KOPI_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.KOPI_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'kopi-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'kopi-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.kopi'), home)
