 ...
 * product name (`Kopi`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Kopi'
 *
 ...
 * (the mac-unpacked Kopi.app bundle) and Windows (win-unpacked), so we
 ...
 * Kopi.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `kopi desktop --build-only` (kopi_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Kopi.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Kopi'}.exe`
