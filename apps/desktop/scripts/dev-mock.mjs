...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `kopi-dev-mock-${Date.now()}`))
  ...
  return { root, kopiHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(kopiHome, mockUrl) {
  fs.writeFileSync(
    path.join(kopiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(kopiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.kopiHome, mock.url)
  console.log(`  KOPI_HOME: ${sandbox.kopiHome}`)
  ...
  const env = {
    ...
    KOPI_DESKTOP_APP_NAME: `KopiDevMock-${Date.now()}`,
  }
