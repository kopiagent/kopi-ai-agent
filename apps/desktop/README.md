...
<p align="center">
  <a href="https://github.com/NousResearch/kopi-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Kopi surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Kopi and hear it back.</td></tr>
...
### Install with Kopi (recommended)

Already have the Kopi CLI? Just run:

```bash
kopi desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Kopi gateway or install Kopi locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Kopi Desktop website.](https://kopi-agent.nousresearch.com/).
...
```bash
# throwaway KOPI_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Kopi Agent runtime into `KOPI_HOME`
(`~/.kopi`, or `%LOCALAPPDATA%\kopi` on Windows), using the same layout as a
CLI install.
...
- **Kopi Agent** runs as a headless `kopi serve` process and exposes the
...
1. `KOPI_DESKTOP_KOPI_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `KOPI_DESKTOP_KOPI`, or `kopi` on `PATH`
5. a system Python that can import the Kopi runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Kopi
...
screen offers **Connect to existing Kopi** before starting the local installer.
...
terminal commands, and file operations run against the remote Kopi host, not
...
rm "$HOME/.kopi/kopi-agent/.kopi-bootstrap-complete"
# Rebuild a broken Python venv
rm -rf "$HOME/.kopi/kopi-agent/venv"
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\kopi\kopi-agent\.kopi-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\kopi\kopi-agent\venv"
```

> The default Kopi home on Windows is `%LOCALAPPDATA%\kopi`. Set the `KOPI_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://kopi-agent.nousresearch.com/docs/)
