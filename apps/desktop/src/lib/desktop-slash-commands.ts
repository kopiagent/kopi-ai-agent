...
/** Why a known Kopi command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Kopi profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Kopi Agent version', surface: exec() },
...
function isKnownKopiSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Kopi' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownKopiSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Kopi and were never asked for. It is
