...
//
// When someone is in a hands-free "Hey Kopi" voice chat, the natural way to
...
// phrase (optionally addressed to Kopi), so a real turn that merely contains
...
// Optional address prefixes so "kopi stop" / "ok stop" / "hey kopi, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey kopi', 'hey kopi,', 'kopi', 'kopi,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("kopi") is not a stop command on its own.
      continue
    }
 ...
 * to Kopi). Returns false for anything that merely contains "stop" as part of
