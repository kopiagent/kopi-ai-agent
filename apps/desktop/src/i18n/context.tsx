...
import { getKopiConfigRecord, type KopiConfigRecord, saveKopiConfig } from '@/kopi'
...
export interface I18nConfigClient {
  getConfig: () => Promise<KopiConfigRecord>
  saveConfig: (config: KopiConfigRecord) => Promise<{ ok: boolean }>
}
...
export function getConfigDisplayLanguage(config: KopiConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: KopiConfigRecord, locale: Locale): KopiConfigRecord {
