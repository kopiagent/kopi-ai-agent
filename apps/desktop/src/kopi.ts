...
/** Options for a plugin REST call — mirrors the app's own `kopiDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see KopiApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.kopiDesktop?.api) {
    throw new Error('Kopi desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.kopiDesktop.api<T>({
...
export function pluginSocket(pluginId: string, path: string, onMessage: (data: unknown) => void): () => void {
  ...
  const connect = async () => {
    const connection = await window.kopiDesktop.getConnection().catch(() => null)
...
// "Error invoking remote method 'kopi:api': Error: 404: ..." through the
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.kopiDesktop.api<SidebarSessionsResponse>({
...
export function getKopiConfig(profile?: string): Promise<KopiConfig> {
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.kopiDesktop.api<CustomEndpointsResponse>({
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.kopiDesktop.api<CustomEndpointValidationResponse>({
...
export function authMcpServer(name: string): Promise<McpOAuthFlow> {
  return window.kopiDesktop.api<McpOAuthFlow>({
    ...profileScoped(),
...
export function selectToolsetProvider(
  ...
  return window.kopiDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.kopiDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.kopiDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getPairing(): Promise<PairingResponse> {
  return window.kopiDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.kopiDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function exportProfileArchive(
  ...
  return window.kopiDesktop.api<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return window.kopiDesktop.api<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
