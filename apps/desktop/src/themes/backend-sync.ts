/**
 * Live skin sync from the Kopi backend.
 *
 * The backend resolves the active skin (built-in or `$KOPI_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Kopi
 ...
 */
export function ingestBackendSkin(skin: KopiSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as KopiSkin)
