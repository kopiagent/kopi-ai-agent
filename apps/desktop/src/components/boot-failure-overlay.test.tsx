...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Kopi gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.kopiDesktop
  Object.defineProperty(window, 'kopiDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'kopiDesktop', { configurable: true, value: original })
}
