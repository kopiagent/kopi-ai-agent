...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Kopi Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Kopi docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Kopi Agent Desktop tool calls')
    expect(view.searchHits).toEqual([{ snippet: 'Desktop docs', title: 'Kopi docs', url: 'https://example.com/docs' }])
  })
})
...
describe('buildToolView browser_navigate title', () => {
  it('shows failed title when navigate returns success=false', () => {
    ...
    expect(view.title).toBe('Failed to open kopi-ai-agent.nousresearch.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    ...
    expect(view.title).toBe('Opened kopi-ai-agent.nousresearch.com/docs')
  })
})
