...
type KopiRefType = (typeof KOPI_REF_TYPES)[number]
...
const ICON_PATHS: Record<KopiRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as KopiRefType] ?? ICON_FALLBACK
...
export const kopiDirectiveFormatter: Unstable_DirectiveFormatter = {
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(KOPI_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as KopiRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: KopiRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Kopi directives (`@file:...`, `@image:...`) as
