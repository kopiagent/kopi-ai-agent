...
import type { KopiReviewFile, KopiReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Kopi' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'kopi.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'kopi.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'kopi.desktop.reviewTreeMode'
const SELECTED_KEY = 'kopi.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<KopiReviewFile[]>([])
...
export const $reviewShipInfo = atom<KopiReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['kopiDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: KopiReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly KopiReviewFile[], path: string): KopiReviewFile | undefined {
