...
const isOpen = (gateway: KopiGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: KopiGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<KopiGateway | null>>
}

const STATE_KEY = Symbol.for('kopi.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<KopiGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: KopiGateway | null, profile = 'default'): void {
...
export function activeGateway(): KopiGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<KopiGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
