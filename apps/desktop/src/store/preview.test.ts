...
describe('preview store', () => {
  ...
  it('persists file and url tabs but never artifacts, whose content is memory-only', () => {
    ...
    const stored = window.localStorage.getItem('kopi.desktop.previewTabs.v2') ?? ''
  ...
  it('strips inline image bytes rather than pushing megabytes into storage', () => {
    ...
    expect(window.localStorage.getItem('kopi.desktop.previewTabs.v2') ?? '').not.toContain('base64')
  })
})
