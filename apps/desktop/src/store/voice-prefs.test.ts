...
vi.mock('@/kopi', () => ({
  getKopiConfigRecord: vi.fn(async () => ({})),
  saveKopiConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye kopi', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye kopi')
  })
