...
import type { KopiGitWorktree, KopiRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: KopiGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, KopiRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, KopiGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<KopiRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<KopiGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<KopiRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<KopiGitWorktree[]>>()
const $noRepoStatus = atom<KopiRepoStatus | null>(null)
const $noWorktrees = atom<KopiGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<KopiRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<KopiGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: KopiRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: KopiGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<KopiRepoStatus | null>
  seq: number
  target: string
}
