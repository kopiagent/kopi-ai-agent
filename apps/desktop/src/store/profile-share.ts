 ...
 *
 * The archive is the CLI's own `kopi profile export` tar.gz (config, skills,
...
import type { ProfileDesktopOverlay } from '@/types/kopi'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Kopi profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.kopiDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.kopiDesktop?.selectPaths?.({
