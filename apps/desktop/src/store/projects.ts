...
import type { KopiGitBaseBranch, KopiGitBranch } from '@/global'
import { translateNow } from '@/i18n'
import { getKopiConfig, type KopiGateway } from '@/kopi'
...
const PROJECT_SCOPE_KEY = 'kopi.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Kopi gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Kopi profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: KopiGateway): Promise<void> {
...
const repoScanStates = new WeakMap<KopiGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<KopiGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getKopiConfig(context.profile))
...
// repo, returning where Kopi should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<KopiGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<KopiGitBaseBranch[]> {
  const git = desktopGit()
