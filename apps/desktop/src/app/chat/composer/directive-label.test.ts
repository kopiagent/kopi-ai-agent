...
import { kopiDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
describe('one label per reference, on every surface', () => {
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/NousResearch/kopi-ai-agent/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/NousResearch/kopi-ai-agent/pull/74533'
