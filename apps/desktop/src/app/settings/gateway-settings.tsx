...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.kopiDesktop?.sshConfigHosts) {
      return
    }
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      ...
      const errors = {
        ...
        'kopi-not-found': g.sshErrNotInstalled,
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.kopiDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.kopiDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.kopiDesktop.testConnectionConfig(payload())
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Kopi Cloud panel: one portal sign-in, then a discovered-agent picker
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteKopiPath: event.target.value }))}
                placeholder={g.sshKopiPathPlaceholder}
                value={state.sshRemoteKopiPath}
              />
            }
