...
const getKopiConfigSchema = vi.fn()
...
vi.mock('@/kopi', () => ({
  ...
  getKopiConfigSchema: () => getKopiConfigSchema(),
...
beforeEach(() => {
  ...
  getKopiConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getKopiConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveKopiConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveKopiConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
