...
let refreshKopiConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  handleEvent = null
  refreshKopiConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshKopiConfig).not.toHaveBeenCalled()

    await act(async () => {
      await vi.advanceTimersByTimeAsync(400)
    })

    expect(refreshKopiConfig).toHaveBeenCalledTimes(1)
  })
