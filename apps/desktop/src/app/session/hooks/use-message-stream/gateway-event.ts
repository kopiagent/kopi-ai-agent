import type { BillingBlock } from '@kopi/shared'
import type { KopiSkin } from '@kopi/shared/skin'
 ...
 * billing-specific toast — never the generic "Kopi error" — with a smart CTA
...
interface GatewayEventDeps {
  ...
  refreshKopiConfig: () => Promise<void>
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshKopiConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: KopiSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (Kopi activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as KopiSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshKopiConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Kopi reported an error',
        ...
        // native OS notification when Kopi is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Kopi reported an error'
