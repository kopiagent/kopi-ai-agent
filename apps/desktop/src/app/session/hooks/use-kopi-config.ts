...
import { getKopiConfig, getKopiConfigDefaults } from '@/kopi'
...
interface KopiConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useKopiConfig({ activeSessionIdRef }: KopiConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshKopiConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getKopiConfig(), getKopiConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Kopi' built-in medium over the user's config.
  ...
  return { refreshKopiConfig, sttEnabled, voiceMaxRecordingSeconds }
}
