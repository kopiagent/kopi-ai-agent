...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.kopiDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  // kopi:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
  ...
  useEffect(() => {
    const unsubscribe = window.kopiDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.kopiDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
