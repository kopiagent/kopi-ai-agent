...
import { getSessionMessages, triggerCronJob } from '@/kopi'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshKopiConfig, sttEnabled, voiceMaxRecordingSeconds } = useKopiConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('kopi:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('kopi:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshKopiConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshKopiConfig])
