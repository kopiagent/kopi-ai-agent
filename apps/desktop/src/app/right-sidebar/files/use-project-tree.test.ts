...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { kopiDesktop: { readDir: typeof readDir } }).kopiDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): KopiReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { kopiDesktop: unknown }).kopiDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: KopiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<KopiReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { kopiDesktop: unknown }).kopiDesktop = { readDir, sanitizeWorkspaceCwd }
