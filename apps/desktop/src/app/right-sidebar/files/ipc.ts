...
import type { KopiReadDirEntry, KopiReadDirResult } from '@/global'
...
export type ProjectTreeEntry = KopiReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: KopiReadDirEntry) {
...
async function filterIgnored(entries: KopiReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<KopiReadDirResult> {
