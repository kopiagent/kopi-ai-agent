 ...
 *   (`@kopi/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default KopiPlugin -> register(ctx)
 *
 ...
 * and `<kopi home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { createPluginContext, type KopiPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@kopi/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default KopiPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<kopi home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's kopi_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // kopi_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
      ...
      // KOPI: null = dir not there yet (see watchDirectory in electron/main.ts);
