 ...
 * `KopiPlugin`; it never touches the registry directly — it receives a
...
export type { PluginRestOptions } from '@/kopi'
 ...
 *  under `kopi.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.kopiDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Kopi — use `host.notify` for the in-app toast.
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `kopi.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.kopiDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.kopiDesktop
