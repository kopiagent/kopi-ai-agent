...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Kopi-managed secrets are not inherited
...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from KOPI_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("KOPI_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid KOPI_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    _ctx_tokens = set_session_vars(
        ...
        # KOPI_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ KOPI_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_session_var = _VAR_MAP["KOPI_CRON_SESSION"]
    ...
    try:
        ...
        # cron-fleet default) > KOPI_MODEL env > config.yaml ``model:``
