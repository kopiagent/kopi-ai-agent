...
commands and shell scripts they reference when ``_KOPI_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``kopi gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    ...
    # (kickstart/etc.): `launchctl submit -l ai.kopi.gateway-<suffix> --
...
# lines (e.g. `launchctl submit \` / `  -l ai.kopi.gateway-... \` / `  -- ...`,
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.kopi.svc-reload-tmp``)
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".kopi"`), not an executable reference.
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `kopi gateway restart` embedded in a .py script is still
