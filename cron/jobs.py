...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_kopi_home()
       (context-local override, then the KOPI_HOME env var) — so a test
       or embedder that re-points KOPI_HOME after this module was
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec kopi kopi cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Kopi timezone, not the server's local
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Kopi/system local time
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``kopi cron status``) never sees a torn/truncated
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``kopi cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``kopi cron
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_kopi_now().isoformat()}\n\n"
