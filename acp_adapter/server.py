"""ACP agent server — exposes Kopi Agent via the Agent Client Protocol."""
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        ...
        from kopi_cli.providers import custom_provider_slug
    except ImportError:
        return []
...
# already uses (`kopi_cli/moa_cmd.py`). This bounds each provider's row, not
...
class KopiACPAgent(acp.Agent):
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Kopi maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Kopi inventory is also used by ``kopi model``, the TUI,
        ...
        try:
            ...
            from kopi_cli.models import normalize_provider, provider_label
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Kopi changes it.

        When the internal Kopi head rotated (e.g. compression-driven session
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from kopi_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from kopi_cli.mcp_startup import join_mcp_discovery
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Kopi setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.kopi`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"kopi": {"compactionSummary": True}}
        if kind == "merged":
            return {"kopi": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Kopi
        ...
        **kwargs: Any,
        ...
        def _run_agent() -> dict:
            ...
            try:
                ...
                # Without it the prompt advertises the global Kopi workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.kopi/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global KOPI_CRON_SESSION (#37968).
        ...
        try:
            # Snapshot the internal Kopi DB session id before the turn so we
        ...
        # Kopi' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Kopi install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Kopi Agent v{KOPI_VERSION}"
