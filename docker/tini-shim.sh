...
# Kopi WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: KOPI_TINI_SHIM_TARGET overrides the `/init` path (and
# KOPI_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${KOPI_TINI_SHIM_TARGET:-/init}"
WRAPPER="${KOPI_TINI_SHIM_WRAPPER:-/opt/kopi/docker/main-wrapper.sh}"
