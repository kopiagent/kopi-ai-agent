...
# with-contenv before touching KOPI_HOME / PATH. On the non-PID-1
...
if [ -z "${KOPI_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${KOPI_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export KOPI_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset KOPI_MAIN_WRAPPER_ENV_READY
