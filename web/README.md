...
Browser-based dashboard for managing Kopi Agent configuration, API keys, and monitoring active sessions.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `kopi-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Kopi Agent” — mixed case, no Mondwest, no `text-display` |
