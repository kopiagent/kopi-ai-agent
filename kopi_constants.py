...
def _kopi_home_from_env() -> Path:
    """Resolve KOPI_HOME from the process environment only.

    Reads the ``KOPI_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_kopi_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_kopi_home`
    and :func:`get_process_kopi_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``KOPI_HOME`` is
...
def get_kopi_home() -> Path:
    ...
    :func:`set_kopi_home_override`) → ``KOPI_HOME`` env var → the
    ...
    if not os.environ.get("KOPI_HOME", "").strip():
        _warn_profile_fallback_once()

    return _kopi_home_from_env()


def get_process_kopi_home() -> Path:
    """Return the Kopi home for the running process, ignoring task overrides.

    Unlike :func:`get_kopi_home`, this never follows the context-local
    override set by :func:`set_kopi_home_override`.  It resolves only the
    process ``KOPI_HOME`` env var (falling back to the platform default),
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        3. ``<KOPI_HOME>/skills`` last-resort
    """
...
def get_kopi_dir(
    ...
    *,
    ...
    Args:
        ...
        home: Optional explicit Kopi home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local KOPI_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_kopi_home()
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$KOPI_HOME/node`` on POSIX.
    ...
    ``KOPI_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_kopi_managed_node() -> str | None:
    """Install a Kopi-managed Node tree and return its npm path.
    ...
    Kopi never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$KOPI_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_kopi_node_executable("npm")
    if existing:
        return existing
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_kopi_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_KOPI_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_kopi_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _KOPI_NODE_TARGET_MAJOR
    return False
...
def find_kopi_node_executable(command: str) -> str | None:
    ...
    Outdated trees (node major below ``_KOPI_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_kopi_managed_node():
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``kopi_cli`` modules using three different Windows
    ...
    A few sites outside ``kopi_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``kopi_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Kopi itself. An ImportError
...
# `kopi_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Kopi.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("kopi_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `kopi update` is exactly
    ...
    return [
        ...
        "    kopi update",
        "If that also fails, reinstall: https://kopi-ai-agent.nousresearch.com",
    ]
