...
Kopi drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The Kopi-side `computer_use` tool exposed
in this skill is a higher-level Kopi vocabulary; the raw cua-driver
...
Normal Kopi sessions use `standard`, which requires a certified protected
host and fails closed when Kopi has none. Explicit Kopi YOLO (`--yolo`,
...
Kopi intentionally keeps THIS skill focused on the Kopi-side
...
When `cua-driver skills install` autodetects Kopi (planned follow-up
