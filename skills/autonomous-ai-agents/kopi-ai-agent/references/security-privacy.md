...
### "Reset permissions" / "make Kopi ask again"
...
1. Shell-command allowlist: `kopi config set command_allowlist '[]'`
2. Shell-hook consent (only if present): `rm -f ~/.kopi/shell-hooks-allowlist.json`

Then sanity-check `kopi config get approvals.mode` (should not be `off`)
...
To keep the model away from network or media tools entirely, open `kopi tools` and toggle per-platform. Takes effect on next session (`/reset`). See `references/configuration.md` for the toolset list.
