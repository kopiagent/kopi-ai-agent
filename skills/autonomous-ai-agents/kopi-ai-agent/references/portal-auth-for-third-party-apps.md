...
## Layer 1 — Is this thing a Kopi plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Kopi process**. Its LLM calls go through Kopi's already-configured provider. | Already uses Portal if user's Kopi is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Kopi. That LLM call is already authenticated
via Kopi's provider config. The plugin's own server auth (e.g.
...
What Kopi Agent has that *feels* like OAuth — `kopi login --provider nous`
opening a browser, user signs in, token lands in `~/.kopi/auth.json` — is a
**Kopi-specific browser flow**. Under the hood it produces a credential
Kopi uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Kopi's existing Portal credential out of `~/.kopi/auth.json`.
...
rides on the credential Kopi already holds.

Where this could live in Kopi:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Kopi itself.
...
1. First decide: Kopi plugin (runs inside Kopi) or separate app? If plugin,
   it already uses Portal via Kopi's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Kopi agent itself") is fully supported. Kopi treats agent
...
pitch in the Tool Gateway announcement is specifically about Kopi Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Kopi is configured to use Portal as its provider. It is
