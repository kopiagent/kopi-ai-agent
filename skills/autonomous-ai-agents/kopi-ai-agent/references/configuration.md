...
Full reference: https://kopi-ai-agent.nousresearch.com/docs/user-guide/configuration
...
`kopi config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `kopi tools` (interactive) or `kopi tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_KOPI_CORE_TOOLS` is the default bundle most platforms inherit).
