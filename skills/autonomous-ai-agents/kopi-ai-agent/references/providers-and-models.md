...
Set via `kopi model` (picker) or `kopi setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://kopi-ai-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `kopi auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `kopi auth add openai-codex` |
| qwen-oauth | OAuth | `kopi auth add qwen-oauth` |
| minimax-oauth | OAuth | `kopi auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`kopi auth`).
Fallback chain when the primary fails: `kopi fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`kopi_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   kopi config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
