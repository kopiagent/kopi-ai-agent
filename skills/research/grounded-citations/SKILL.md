...
Ledger location: `$KOPI_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `KOPI_CITATION_LEDGER`.

## How to Run

```bash
S=~/.kopi/skills/research/grounded-citations/scripts/sources.py
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `KOPI_CITATION_LEDGER`) if their
