...
Ledger path resolution (first wins):
  --ledger PATH
  $KOPI_CITATION_LEDGER
  $KOPI_HOME/cache/citations/ledger.json
"""
...
from _kopi_home import get_kopi_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("KOPI_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_kopi_home() / "cache" / "citations" / "ledger.json"
