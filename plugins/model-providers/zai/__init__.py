...
request omits ``thinking``.  Kopi' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Kopi reasoning
...
(per Z.AI / BigModel docs).  Kopi' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Kopi reasoning effort onto GLM-5.2's native ``high``/``max``.
