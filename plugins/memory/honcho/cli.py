...
def cmd_setup(args) -> None:
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'kopi honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'kopi honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                ...
                return
            print(" approved")
            _apply_grant_to_host(kopi_host, cred)
    ...
    # These keys only affect the Kopi GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Kopi gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if kopi_cli internals move."""
    try:
...
def _apply_grant_to_host(kopi_host: dict, cred) -> None:
