...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Kopi profile."""
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      ...
      2. Active profile name via profiles system -> ``kopi_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"kopi"`` (default profile)
    """
    ...
    # but applying it before profile resolution makes every named Kopi
    # profile share that same host.  Keep named profiles isolated; only the
    # default Kopi profile may opt into the config's default host.
...
class HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Kopi session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Kopi session title (from /title command; non-per-session)
