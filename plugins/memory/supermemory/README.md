...
- Hosted: API key from [app.supermemory.ai/integrations?connect=kopi](http://app.supermemory.ai/integrations?connect=kopi)
...
Before running `kopi memory setup`, add the local endpoint to
`$KOPI_HOME/supermemory.json`:
...
Then run `kopi memory setup` and enter the local server's API key. Configuring
...
`https://api.supermemory.ai`. Kopi resolves it once and uses the same endpoint
