...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Kopi peer ID in OpenViking (default: kopi)
...
_DEFAULT_AGENT = "kopi"
_AGENT_PROMPT_LABEL = "Kopi peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Kopi slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Kopi to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Kopi refused the connection."
        ) from exc
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Kopi did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Kopi will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Kopi session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Kopi only", "write values only to Kopi .env"),
    ...
    _print_openviking_ready("Connection saved to Kopi .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Kopi peer ID in OpenViking, sent as the actor peer and "
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        kopi_home = str(kwargs.get("kopi_home") or "").strip()
        if not kopi_home:
            try:
                from kopi_constants import get_kopi_home
                kopi_home = str(get_kopi_home())
            except Exception:
                kopi_home = str(Path.home() / ".kopi")
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Kopi will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.kopi/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts kopi (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Kopi will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._kopi_home:
            return None
        return Path(self._kopi_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._kopi_home:
            return None
        return Path(self._kopi_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_kopi_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Kopi' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Kopi canonical messages into OpenViking batch payloads."""
