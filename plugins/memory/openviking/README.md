...
- OpenViking server running and reachable from Kopi

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Kopi can identify older servers that expose the legacy status-only health
...
connection values into Kopi, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.kopi/.env`; for a named profile use
`~/.kopi/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=kopi
```

## Config

OpenViking's server config is separate from Kopi:
...
Kopi-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `kopi` | Kopi peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Kopi lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Kopi sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Kopi built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Kopi action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Kopi
