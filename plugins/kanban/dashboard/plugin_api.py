...
from kopi_cli.kanban_db import (  # noqa: E402
...
def _active_profile_name() -> str:
    """Return the current Kopi profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    Thin wrapper around ``kopi_cli.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of Kopi
...
def list_kanban_projects():
    """List first-class Kopi projects for board scoping.
