"""nemo_relay — optional Kopi plugin for NeMo Relay observability."""
...
_SESSION_INITIALIZER_NAME = "kopi.nemo_relay.rich_observability"
...
class _Settings:
    ...
    atif_agent_name: str = "Kopi Agent"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Kopi profile; keeping the existing process-global configuration "
...
class _Runtime:
    def __init__(
        ...
        self._atof_subscriber_name = f"kopi.nemo_relay.atof.{self.host.runtime_id}"
        self._execution_consumer_name = (
            f"kopi.nemo_relay.rich_observability.{self.host.runtime_id}"
        )
        ...
        with self._sessions_lock:
            ...
            if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
                ...
                state.atif_subscriber_name = (
                    f"kopi.nemo_relay.atif.{self.host.runtime_id}.{session_id}"
                )
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Kopi core Relay session is unavailable")
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Kopi installs the managed execution
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("KOPI_NEMO_RELAY_ATIF_AGENT_NAME") or "Kopi Agent",
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Kopi cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Kopi-owned "
