"""OpenRouter-compatible image generation backend (OpenRouter + Kopi Official).

Both OpenRouter and the Kopi Official inference endpoint speak the same
...
Kopi Official proxies OpenRouter, so one implementation services both — we only
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Kopi Official). The two differ only
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Kopi Official).
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            ...
            setup_schema={
                "name": "Kopi Official (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Kopi Official (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Kopi Official image gen providers."""
