# A2A — Agent-to-Agent protocol for Kopi
...
peer (another Kopi, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
kopi gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Kopi serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.kopi/a2a_audit.jsonl`.
- Conversations persist to `~/.kopi/a2a_conversations/` — they survive context
