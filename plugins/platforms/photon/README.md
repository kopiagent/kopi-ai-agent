...
This plugin connects Kopi Agent to iMessage (and other Spectrum
...
TypeScript-only, Kopi runs it inside a small supervised Node sidecar and
...
2. **Find or create** the `Kopi Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Kopi channel,
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Kopi
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Kopi coalesces
...
- **Native polls are supported.** Kopi posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `kopi send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<kopi-home>/runtime/photon-sidecar.json` (written after the sidecar's
