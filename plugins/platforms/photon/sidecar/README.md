...
Small Node helper that bridges Kopi Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Kopi is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Kopi-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Kopi can receive inbound messages
...
The Kopi plugin's `kopi photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Kopi is Python, so replies go through this sidecar
