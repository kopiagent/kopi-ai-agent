// Kopi Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Kopi): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Kopi -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Kopi-Sidecar-Token: ${TOKEN}`):
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Kopi can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "kopi-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Kopi'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Kopi allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Kopi, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-kopi-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Kopi supplied them so a known-good
