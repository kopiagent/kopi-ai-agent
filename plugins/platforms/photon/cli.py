...
there is no standalone ``login`` verb (matching how every other Kopi
...
# Writable sidecar runtime dir (mirrors to KOPI_HOME on immutable
...
# wiring imports this module for `kopi --help`).
...
# when called later (gateway start, kopi status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Kopi Agent')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Kopi Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.kopi/.env,
    ...
    try:
        from kopi_cli.config import write_platform_config_field
