"""
Photon Spectrum (iMessage) platform adapter for Kopi Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Kopi dep
...
#
# Out-of-process senders (cron subprocesses, `kopi send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_kopi_home() honors profile overrides — never hardcode ~/.kopi.
    from kopi_constants import get_kopi_home

    return get_kopi_home() / "runtime" / _RUNTIME_RECORD_NAME
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Kopi defaults).
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Kopi checkout's sidecar entry point.
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `kopi send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`kopi send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Kopi gateway (which spawns the sidecar and "
                    "records its address under <kopi-home>/runtime/"
    ...
    headers = {"X-Kopi-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Kopi plugin loader at startup."""
