...
# identify KopiAgent traffic — matching other Kopi outbound surfaces
# that already set ``KopiAgent/<version>`` for platform-partner attribution.
try:
    from kopi_cli import __version__ as _KOPI_VERSION
except Exception:
    _KOPI_VERSION = "unknown"
_KOPI_SLACK_USER_AGENT_PREFIX = f"KopiAgent/{_KOPI_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `kopi gateway setup` "
                "or in the active profile's ~/.kopi/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `kopi gateway setup` "
                "or add it to your active profile's ~/.kopi/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `kopi gateway setup` "
                "or in the active profile's ~/.kopi/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `kopi gateway setup` "
                "or add it to your active profile's ~/.kopi/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_KOPI_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Kopi has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Kopi can "
            ...
            self._app.action("kopi_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^kopi_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("kopi_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'kopi slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Kopi handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    async def _handle_app_context_changed(
        ...
        # Kopi owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_kopi_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_kopi_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_kopi_no_thread_response"):
        ...
        force_process = bool(event.get("_kopi_force_process"))
        ...
        # the shape produced by peer Kopi agents in Socket Mode on some
    ...
    async def send_clarify(
        ...
        (unique ``kopi_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``kopi_clarify_other``).  A choice click resolves the clarify
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "kopi_clarify_other" or token == "other":
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_kopi_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    from kopi_cli.config import get_env_value, remove_env_value, save_env_value
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `kopi setup` to install Slack support.",
