...
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Kopi infra |
...
| File operations | Generic instructions | Kopi file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Kopi repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
