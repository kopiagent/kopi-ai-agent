...
### 1. Start order: editor first, Kopi second

Kopi probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Kopi session.
...
restart the editor, restart the Kopi session.
...
server fails to bind (Output Log) or Kopi' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.kopi/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Kopi

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Kopi' connection
lives in `~/.kopi/config.yaml` via `kopi mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Kopi — just don't
mistake it for the Kopi setup step.
...
explicitly warns against overlapping calls. Kopi executes same-turn tool
...
### 9. Timeouts: Kopi gives up before Unreal does

Kopi' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Kopi runs elsewhere (SSH backend,
