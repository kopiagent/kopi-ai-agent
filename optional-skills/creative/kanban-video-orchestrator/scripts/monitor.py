#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `kopi kanban list` and
...
def kopi_available() -> bool:
    return shutil.which("kopi") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed kopi CLI."""
    try:
        out = subprocess.run(
            ["kopi", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `kopi kanban list`
    out = subprocess.run(
        ["kopi", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not kopi_available():
        print("ERROR: 'kopi' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
