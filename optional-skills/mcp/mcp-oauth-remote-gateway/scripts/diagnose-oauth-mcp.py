...
Usage:
  ...
  <server_name> matches the files in $KOPI_HOME/mcp-tokens/<server>.json etc.
...
def _kopi_home():
    # Prefer Kopi' own resolver (profile-safe); fall back to env then ~/.kopi.
    try:
        from kopi_constants import get_kopi_home
        return str(get_kopi_home())
    except Exception:
        return os.environ.get("KOPI_HOME") or os.path.expanduser("~/.kopi")


def _tokens_dir():
    return os.path.join(_kopi_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "kopi-diag", "version": "1.0"}},
        },
        headers={
