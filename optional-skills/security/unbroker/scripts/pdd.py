...
The Kopi agent orchestrates scanning and opt-out submission with native tools
...
def cmd_doctor(args) -> None:
    ...
    if cfg["email_mode"] == "draft_only":
        ...
        L.append("  Email mode: browser (no password) - the agent sends opt-outs and opens verify "
                 "links via the operator's logged-in webmail. This needs Kopi pointed at the "
...
def cmd_cdp(args) -> None:
    ...
    Kopi at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({"running": True, "endpoint": f"127.0.0.1:{port}",
              ...
              "note": "a debuggable browser is already listening; point Kopi's browser tools at "
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Kopi's browser tools at 127.0.0.1:{port} (CDP)",
