...
into Kopi Agent, archives selected unmapped docs for manual review, and
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`kopi config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.kopi/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Kopi memory store (memories/MEMORY.md, USER.md).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Kopi Agent.")
