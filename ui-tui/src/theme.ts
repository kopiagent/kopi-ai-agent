import type { SkinBranding, SkinColors } from '@kopi/shared/skin'
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Kopi navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Kopi look was never hand-authored: for years the
 ...
 *  answer when known (cached in KOPI_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.KOPI_TUI_BACKGROUND ?? '').trim()
 ...
 * the OSC-11 background answer cached into KOPI_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into KOPI_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Kopi navy
  ...
  // the derived value already carries the identity seeds (e.g. Kopi navy).
  const activeRow =
