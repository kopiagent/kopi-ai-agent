...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@kopi/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.KOPI_TUI_THEME : undefined

  writeBootTheme(theme, process.env.KOPI_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to KOPI_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit KOPI_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the KOPI_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.KOPI_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.KOPI_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a KOPI_TUI_THEME the user
    ...
    delete process.env.KOPI_TUI_THEME
  }
 ...
 * is cached into KOPI_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * KOPI_TUI_LIGHT / KOPI_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.KOPI_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.KOPI_TUI_THEME || process.env.KOPI_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.KOPI_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.KOPI_TUI_BACKGROUND ||
      process.env.KOPI_TUI_THEME ||
      process.env.KOPI_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.KOPI_TUI_BACKGROUND || process.env.KOPI_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.KOPI_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Kopi" if this surface owns it (server gates on config).
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey Kopi": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: kopi -p ${wakeProfile} --tui`)
