...
import { terminalBackgroundHex } from '@kopi/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $KOPI_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['KOPI_TUI_BACKGROUND', process.env.KOPI_TUI_BACKGROUND ?? '(unset)'],
            ['KOPI_TUI_THEME', process.env.KOPI_TUI_THEME ?? '(unset)'],
