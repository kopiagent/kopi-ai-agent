 ...
 * `kopi-boot-background` / `kopi-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports KOPI_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.kopi matches get_kopi_home()'s default.
const bootFilePath = () => join(process.env.KOPI_HOME ?? join(homedir(), '.kopi'), 'tui-theme-boot.json')

// Never touch the user's real ~/.kopi from test runs (the TS suite has no
// KOPI_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into KOPI_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.KOPI_TUI_THEME || env.KOPI_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.KOPI_TUI_BACKGROUND
  ) {
    env.KOPI_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.KOPI_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.KOPI_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into KOPI_TUI_THEME.
