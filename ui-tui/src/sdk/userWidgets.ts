...
/**
 * User widget apps — Kopi authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$KOPI_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.kopi/plugins/`: files under KOPI_HOME execute
...
const widgetsDir = () => join(process.env.KOPI_HOME?.trim() || join(homedir(), '.kopi'), 'tui-widgets')
 ...
 *  every change, so a widget Kopi writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Kopi) ever writes must hot-load too — a 10s poll here read
