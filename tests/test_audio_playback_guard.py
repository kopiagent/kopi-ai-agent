...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``kopi_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_KOPI_BEHAVIORAL_VARS`` now blanks ``KOPI_VOICE`` /
    ``KOPI_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("KOPI_VOICE_TTS") == "1"
    ...
    # calls it (a late `from kopi_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``KOPI_VOICE_TTS=1`` set.
    Before ``KOPI_VOICE``/``KOPI_VOICE_TTS`` were added to
    ``_KOPI_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "KOPI_VOICE_TTS" not in os.environ
    assert "KOPI_VOICE" not in os.environ
