"""Live Fireworks smoke test — exercises the Kopi runtime, not a raw SDK client.

Opt-in only:
    KOPI_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives Kopi'
...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the Kopi runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "Kopi failed to build a Fireworks client"
    return client, model


def test_kopi_wires_fireworks_client():
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the Kopi-resolved client returns text."""
    client, model = _resolve_runtime_client()
