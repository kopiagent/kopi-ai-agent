...
def test_openrouter_base_url_applies_or_headers(mock_openai):
    ...
    assert headers["X-Title"] == "Kopi Agent"
...
def test_nvidia_cloud_base_url_applies_billing_origin_header(mock_openai):
    ...
    assert agent._client_kwargs["default_headers"]["X-BILLING-INVOKE-ORIGIN"] == "KopiAgent"
...
def test_routed_client_preserves_openai_sdk_custom_headers(mock_openai):
    ...
    routed_client = SimpleNamespace(
        api_key="test-key",
        base_url="https://integrate.api.nvidia.com/v1",
        _custom_headers={"X-BILLING-INVOKE-ORIGIN": "KopiAgent"},
    )
...
@patch("run_agent.OpenAI")
def test_xai_base_url_applies_kopi_user_agent(mock_openai):
    """Direct xAI chat must send Kopi-Agent/* instead of OpenAI/Python."""
    mock_openai.return_value = MagicMock()
    agent = AIAgent(
        api_key="test-key",
    ...
    headers = agent._client_kwargs["default_headers"]
    assert headers["User-Agent"].startswith("Kopi-Agent/")


@patch("run_agent.OpenAI")
def test_xai_oauth_base_url_applies_kopi_user_agent(mock_openai):
    """xai-oauth uses the same api.x.ai host and must get the Kopi UA."""
    mock_openai.return_value = MagicMock()
    agent = AIAgent(
        api_key="oauth-token",
        base_url="https://api.x.ai/v1",
