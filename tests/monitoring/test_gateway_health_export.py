...
def test_gateway_health_snapshot_maps_runtime_status_to_low_cardinality_metrics():
    ...
    assert {
        "kopi.gateway.up",
        "kopi.gateway.active_agents",
        "kopi.gateway.busy",
        "kopi.gateway.drainable",
        "kopi.gateway.restart_requested",
        "kopi.platform.up",
        "kopi.platform.degraded",
    } <= metric_names

    active = next(m for m in snapshot.metrics if m.name == "kopi.gateway.active_agents")
    ...
    busy = next(m for m in snapshot.metrics if m.name == "kopi.gateway.busy")
    drainable = next(m for m in snapshot.metrics if m.name == "kopi.gateway.drainable")
    assert busy.value == 1
    assert drainable.value == 1

    degraded = next(
        m for m in snapshot.metrics
        if m.name == "kopi.platform.degraded" and m.attributes["kopi.platform"] == "telegram"
    )
    assert degraded.value == 1
    assert degraded.attributes["kopi.error_code"] == "auth_failed"
...
def test_gateway_health_snapshot_preserves_real_bounded_platform_states():
    ...
    observed = {
        metric.attributes["kopi.platform.state"]
        for metric in snapshot.metrics
        if metric.name == "kopi.platform.up"
    }
...
def test_otlp_attrs_include_gateway_transition_fields():
    ...
    assert attrs["kopi.old_state"] == "starting"
    assert attrs["kopi.new_state"] == "running"
    assert attrs["kopi.exit_reason"] == "restart"
    assert attrs["kopi.restart_requested"] is True
...
def test_otlp_attrs_redact_strings_and_never_export_profile():
    ...
    assert "kopi.profile" not in attrs
...
def test_runtime_resource_attributes_include_stable_hashed_instance():
    ...
    assert attrs["service.name"] == "kopi-gateway"
    assert attrs["service.instance.id"].startswith("sha256:")
...
def test_diagnostic_log_attributes_are_allowlisted_redacted_and_profile_free():
    ...
    assert "kopi.profile" not in attrs
    assert "kopi.custom" not in attrs
    assert "top-secret-token" not in str(attrs)
...
def test_diagnostic_log_streamer_uses_validated_source_as_otel_scope():
    ...
    streamer._logger = provider.get_logger("kopi.gateway.diagnostics")
    ...
    fallback = provider.loggers["kopi.gateway.diagnostics"].records
