...
def test_cron_snapshot_projects_freshness_counts_and_overdue_without_content(monkeypatch):
    ...
    assert _metric(snapshot, "kopi.cron.scheduler.heartbeat_age_seconds").value == 4.5
    assert _metric(snapshot, "kopi.cron.scheduler.last_success_age_seconds").value == 9.0
    assert _metric(snapshot, "kopi.cron.jobs.enabled").value == 1
    assert _metric(snapshot, "kopi.cron.jobs.running").value == 1
    assert _metric(snapshot, "kopi.cron.jobs.overdue").value == 1
...
def test_cron_snapshot_omits_unknown_freshness_instead_of_inventing_values(monkeypatch):
    ...
    assert "kopi.cron.scheduler.heartbeat_age_seconds" not in names
    assert "kopi.cron.scheduler.last_success_age_seconds" not in names
...
def test_cron_snapshot_exports_catch_up_occurrence_counter(monkeypatch):
    ...
    assert _metric(snapshot, "kopi.cron.scheduler.catch_up_occurrences").value == 3
...
def test_gateway_export_includes_cron_metrics_and_only_accepted_event_planes(monkeypatch):
    ...
    cron_snapshot = type(
        "Snapshot",
        (),
        {"metrics": [type("Metric", (), {"name": "kopi.cron.jobs.enabled", "value": 2, "attributes": {}})()]},
    )()
    ...
    assert "kopi.cron.jobs.enabled" in names
    ...
    assert "kopi.gateway.background_work" in names
    assert "kopi.gateway.background_delegations" in names
...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("kopi.gateway.up"), _M("kopi.gateway.active_agents"),
        _M("kopi.gateway.busy"), _M("kopi.gateway.drainable"),
        _M("kopi.gateway.restart_requested"),
        _M("kopi.platform.up"), _M("kopi.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("kopi.cron.scheduler.heartbeat_age_seconds"),
        _M("kopi.cron.scheduler.last_success_age_seconds"),
        _M("kopi.cron.scheduler.catch_up_occurrences"),
        _M("kopi.cron.jobs.enabled"), _M("kopi.cron.jobs.running"),
        _M("kopi.cron.jobs.overdue"),
    ]})()
    monkeypatch.setattr(gateway_health_export, "_read_gateway_snapshot", lambda config: gateway_snapshot)
    monkeypatch.setattr(gateway_health_export, "_read_cron_snapshot", lambda: cron_snapshot)
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Kopi Agent-owned Relay transport health" in text
