...
Kopi must not *enable* multi-process WAL on SQLite builds that still contain
...
def _reset_wal_reset_bug_warnings():
    kopi_state._wal_reset_bug_warned_paths.clear()
    yield
    kopi_state._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            kopi_state, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(kopi_state, "sqlite_source_id", lambda: "testid-abc")
