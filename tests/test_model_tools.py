...
class TestHandleFunctionCall:
    ...
    def test_registry_exception_emits_terminal_tool_hook(self, monkeypatch):
        from kopi_cli import lifecycle

        hook_calls = []
        monkeypatch.setattr("kopi_cli.plugins.invoke_hook", lambda *_args, **_kwargs: [])
...
class TestPreToolCallBlocking:
    ...
    def test_relay_rewrite_is_visible_to_pre_tool_authorization(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kopi_cli.observability.relay_runtime.apply_tool_request_intercepts",
            rewrite,
        )
        monkeypatch.setattr("kopi_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("kopi_cli.plugins.has_hook", lambda name: True)
