...
"""
    ...
    assert "Tyler Williams > Active Projects: Kopi Agent" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``kopi_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_KOPI_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_KOPI_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.kopi/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: kopi-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "kopi-4-405b" in real.read_text(encoding="utf-8")
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_KOPI_CONFIG, encoding="utf-8")
