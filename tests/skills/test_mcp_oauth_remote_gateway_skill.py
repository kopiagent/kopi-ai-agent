...
SESSION_REVOKED / REFRESH_DEAD), the KOPI_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, KOPI_HOME=str(tokens_dir.parent))), \
         patch.object(mod.urllib.request, "urlopen", side_effect=fake_urlopen), \
         patch.object(sys, "argv", ["diagnose-oauth-mcp.py", *argv]):
        # Force the env-var fallback path (ignore any importable kopi_constants).
        with patch.object(mod, "_kopi_home", lambda: str(tokens_dir.parent)):
            buf = io.StringIO()
...
def test_kopi_home_env_fallback(tmp_path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "custom-home"))
    # Block the kopi_constants import so the env fallback is exercised
    with patch.dict(sys.modules, {"kopi_constants": None}):
        home = mod._kopi_home()
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "Kopi Agent"  # human credited first
