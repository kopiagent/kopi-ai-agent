...
def test_kopi_help_under_wrapped_init(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"kopi --help failed under `docker run --init` (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"kopi --help produced a traceback under --init: {r.stderr[-2000:]!r}"
    )
