...
Running a state-writing ``kopi cron`` CLI command as root (the default for
...
2. Zombie-ticker surfacing: a failing tick must persist the failure reason
   (``ticker_last_error``) where ``kopi cron status`` can show it, and a
...
class TestTickerLoopRecordsErrors:
    ...
    def test_failing_tick_persists_error(self, cron_store, monkeypatch):
        ...
        msg = jobs.get_ticker_last_error()
        assert msg is not None, (
            "a failing tick must persist its reason for `kopi cron status` "
            "to surface (#68483)"
        )
        assert "Permission denied" in msg
...
# =========================================================================
# 3. `kopi cron status` surfaces the failure reason
# =========================================================================
