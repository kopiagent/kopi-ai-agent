...
class _FakeCronAgent:
    ...
    def run_conversation(self, prompt):
        ...
        assert get_session_env("KOPI_CRON_SESSION") == "1"
        return {
            "completed": True,
            "failed": False,
...
def test_run_job_cron_execute_code_deny_does_not_pollute_later_gateway_execute_code(
    ...
    monkeypatch.setenv("KOPI_MODEL", "test-model")
    monkeypatch.setattr(approval_module, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setattr(approval_module, "_get_approval_mode", lambda: "manual")
    monkeypatch.setattr(approval_module, "_get_cron_approval_mode", lambda: "deny")
    monkeypatch.setattr("kopi_state.SessionDB", _DummySessionDB)
    monkeypatch.setattr("run_agent.AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "kopi_constants.resolve_reasoning_config", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cron_scheduler, "_get_kopi_home", lambda: tmp_path)
    ...
    assert os.environ.get("KOPI_CRON_SESSION") is None
    assert get_session_env("KOPI_CRON_SESSION") == ""
    ...
    assert get_session_env("KOPI_CRON_SESSION") == "1"
    monkeypatch.delenv("KOPI_CRON_SESSION")
