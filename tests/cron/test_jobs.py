...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED Kopi
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured Kopi timezone differs from the
...
class TestGetDueJobs:
    ...
    def test_cron_next_run_offset_migration_is_rescheduled_not_fired(self, tmp_cron_dir, monkeypatch):
        ...
        # A 21:00 cron was stored while Kopi/system local time was UTC+10.
...
class TestClaimDispatch:
    ...
    def test_get_due_jobs_stale_removal_writes_diagnostic(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.delenv("KOPI_CRON_TIMEOUT", raising=False)
        ttl = _oneshot_run_claim_ttl_seconds()
        stale = (_kopi_now() - timedelta(seconds=ttl + 300)).isoformat()
        save_jobs([{
            "id": "wedged1",
...
class TestLateEnvRepointScopesStore:
    """A KOPI_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_unchanged_home_returns_import_time_constants(self, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_HOME", str(jobs.KOPI_DIR))
        store = jobs._current_cron_store()
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_HOME", str(tmp_path / "env-home"))
    ...
    def test_patched_compatibility_constants_beat_env(self, tmp_path, monkeypatch):
        ...
        process-wide escape hatch — they win over a repointed KOPI_HOME."""
        import cron.jobs as jobs
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import KOPI_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "KOPI_DIR", sim_old_home)
        ...
        monkeypatch.setenv("KOPI_HOME", str(new_home))
