...
which is already bounded by KOPI_CRON_TIMEOUT — the worker thread submitted
...
timeout (KOPI_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
def _hanging_session_db(never_set: threading.Event):
    """Stand-in for kopi_state.SessionDB() that blocks until released —
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        try:
            with patch("cron.scheduler._kopi_home", tmp_path), \
                 ...
                 patch("kopi_state.SessionDB", side_effect=lambda: _hanging_session_db(never_set)), \
                 patch(
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed KOPI_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors KOPI_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("KOPI_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "KOPI_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KOPI_CRON_SESSION_DB_TIMEOUT", raising=False)
