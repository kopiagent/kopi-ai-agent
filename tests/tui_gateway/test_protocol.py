...
def server():
    ...
    # e.g. kopi_cli.active_sessions would bind the mocked get_kopi_home
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `kopi config set` does)
    ...
    import kopi_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_kopi_home", lambda: tmp_path)
    ...
    # Activate midnight, as `kopi config set display.skin midnight` would.
