...
def test_non_git_cwd_can_group_inside_a_junk_repo_subtree():
    ...
    workspace = _session("/home/test/.kopi/workspaces/notes")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_root=lambda path: path.startswith("/home/test/.kopi"),
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.kopi"},
    )

    assert [p["id"] for p in tree["projects"]] == ["/home/test/.kopi/workspaces/notes"]
    assert tree["scoped_session_ids"] == [workspace["id"]]


def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.kopi")
...
def test_deleted_sibling_worktree_subdir_folds_into_parent_home_checkout():
    ...
    sessions = [
        _session("/www/kopi-ai-agent", branch="main"),
        _session("/www/kopi-ai-agent-guiperf/apps/desktop"),
    ]
...
def test_deleted_unrelated_workspace_does_not_become_a_project():
    # A deleted dir the sibling probe can't reach by name (`kopi-salvage-drafts`
    # shares no prefix with `kopi-ai-agent`; `/tmp/scratch` was never a worktree)
    ...
    live, salvage, scratch = (
        _session("/www/kopi-ai-agent", branch="main"),
        _session("/www/kopi-salvage-drafts/apps/desktop"),
        _session("/tmp/scratch"),
    )
    on_disk = {"/www/kopi-ai-agent"}

    tree = pt.build_tree([], [live, salvage, scratch], [], resolve, hydrate=True, exists=lambda p: p in on_disk)

    assert _real_project_ids(tree) == ["/www/kopi-ai-agent"]
