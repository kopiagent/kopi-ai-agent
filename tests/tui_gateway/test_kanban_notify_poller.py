...
``platform="tui"`` / ``chat_id=KOPI_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        kopi_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_kopi_root()``, which resolves the process env and
        ...
        token = set_kopi_home_override(str(other_profile_home))
        try:
