...
Kopi's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Kopi root.  harden_import_path must relocate the
    Kopi root to the front so ``from utils import ...`` resolves to Kopi."""
    import sys
