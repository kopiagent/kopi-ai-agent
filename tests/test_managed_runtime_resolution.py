"""Guard: Kopi-owned subprocesses must not resolve managed runtimes by bare PATH.

Kopi installs runtimes for itself — ``uv`` at ``$KOPI_HOME/bin/uv``, Node at
``$KOPI_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Kopi's own
...
* when a system copy also exists, the one Kopi does not own wins — which is
...
``iter_kopi_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Kopi provisions into KOPI_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Kopi-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Kopi-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$KOPI_HOME/bin (uv) and $KOPI_HOME/node are not on an "
        ...
        "  node/npm -> kopi_constants.find_node_executable()\n"
        "  PATH env -> kopi_constants.iter_kopi_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(kopi_constants, helper))


def test_managed_uv_helpers_exist():
    from kopi_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
