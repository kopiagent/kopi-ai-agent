...
def test_built_sdist_ships_web_dist(tmp_path):
    """The sdist must carry kopi_cli/web_dist/ too.

    MANIFEST.in `graft kopi_cli/web_dist` is what puts the frontend assets
    in the source distribution tarball. This test builds the sdist and asserts
    that index.html exists inside it.
    """
    # Create a dummy index.html in kopi_cli/web_dist if it doesn't exist
    # so that the sdist build actually has files to bundle.
    web_dist_dir = REPO_ROOT / "kopi_cli" / "web_dist"
    ...
    try:
        ...
        with tarfile.open(tarballs[0]) as tf:
            names = tf.getnames()
            web_assets = [m for m in names if "kopi_cli/web_dist/" in m]

        assert any(m.endswith("/kopi_cli/web_dist/index.html") for m in web_assets), (
            f"sdist missing kopi_cli/web_dist/index.html; shipped files in web_dist: {web_assets}"
        )
    finally:
