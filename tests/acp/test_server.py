        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Kopi' interactive model/provider setup in a terminal. "
                    "Use this when Kopi has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Kopi provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_authenticated_cross_provider_model_state(self):
        ...
        with (
            patch("kopi_cli.inventory.load_picker_context", return_value=picker_context),
            patch("kopi_cli.inventory.build_models_payload", return_value=payload) as build_payload,
        ):
    ...
    async def test_new_session_bounds_models_and_keeps_current_selection(self):
        ...
        def bounded_payload(_context, **kwargs):
            ...
            # exercises the cap Kopi actually requests.
        ...
        with (
            patch("kopi_cli.inventory.load_picker_context", return_value=picker_context),
            patch("kopi_cli.inventory.build_models_payload", side_effect=bounded_payload),
        ):
            resp = await acp_agent.new_session(cwd="/tmp")
    ...
    async def test_new_session_keeps_current_model_missing_from_inventory(self):
        ...
        with (
            patch("kopi_cli.inventory.load_picker_context", return_value=picker_context),
            patch("kopi_cli.inventory.build_models_payload", return_value=payload),
        ):
            resp = await acp_agent.new_session(cwd="/tmp")
    ...
    async def test_load_session_flags_compaction_summary_on_replayed_user_chunk(self, agent):
        """A replayed compaction summary must carry _meta.kopi.compactionSummary.
        ...
        assert user_chunks[0].field_meta == {"kopi": {"compactionSummary": True}}
    ...
    async def test_load_session_flags_compaction_summary_on_replayed_assistant_chunk(self, agent):
        ...
        assert agent_chunks[0].field_meta == {"kopi": {"compactionSummary": True}}
    ...
    async def test_load_session_flags_merged_tail_summary_as_contains_not_standalone(self, agent):
        ...
        assert user_chunks[0].field_meta == {
            "kopi": {"containsCompactionSummary": True}
        }
...
class TestPrompt:
    ...
    async def test_prompt_pins_the_session_cwd_for_the_turn(self, agent, tmp_path):
        """The turn must resolve the ACP client's cwd, not the Kopi workspace.
