...
# boundary, blocking `kopi send` message bodies, multi-line
# `git commit -m` messages, and heredoc text that merely MENTION
# shutdown/reboot commands.
_QUOTED_NEWLINE_DATA_ALLOW = [
    # kopi send with a multi-line message body (the reported symptom)
    'kopi send -t telegram -s "spark1" "console output:\nsudo reboot\ndone"',
    'kopi send -t telegram "line1\nshutdown -h now\nline3"',
...
_QUOTED_NEWLINE_THREATS_BLOCK = [
    ...
    'kopi send -t telegram "$(sudo reboot)"',
    'echo "`shutdown -h now`"',
...
def test_quoted_newline_data_not_blocked_by_full_guard_chain(clean_session):
    ...
    command = 'kopi send -t telegram "status:\nsudo reboot happened at 3am"'
