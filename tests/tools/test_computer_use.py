...
class TestRegistration:
    ...
    def test_check_fn_finds_user_local_cua_driver_when_path_omits_it(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KOPI_CUA_DRIVER_CMD", raising=False)
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_CUA_DRIVER_CMD", str(driver))
...
class TestCuaDriverSessionReconnect:
    ...
    def test_reconnect_retry_can_revive_ended_session(self):
        ...
        ended = {
            "data": "session 'kopi-test' has ended; call start_session to revive it",
            "images": [],
            "structuredContent": None,
            "isError": True,
        }
        ...
        bridge = FakeBridge()
        session = self._make_session(bridge)
        session._declared_session_id = "kopi-test"

        result = session.call_tool(
            "list_windows", {"session": "kopi-test"}
        )

        assert result is windows
        assert session._reconnect_log == ["stop", "start"]
        assert [call[0] for call in bridge.calls] == [
            ("call", "list_windows", {"session": "kopi-test"}),
            ("call", "list_windows", {"session": "kopi-test"}),
            ("call", "start_session", {"session": "kopi-test"}),
            ("call", "list_windows", {"session": "kopi-test"}),
        ]
    ...
    def test_call_tool_revives_ended_session_then_retries_once(self):
        ...
        ended = {
            "data": (
                "session 'kopi-test' has ended; tool call 'list_windows' "
        ...
        result = session.call_tool(
            "list_windows", {"on_screen_only": True, "session": "kopi-test"}
        )

        assert result is windows
        assert [call[0] for call in bridge.calls] == [
            ("call", "list_windows", {"on_screen_only": True, "session": "kopi-test"}),
            ("call", "start_session", {"session": "kopi-test"}),
            ("call", "list_windows", {"on_screen_only": True, "session": "kopi-test"}),
        ]
    ...
    def test_call_tool_does_not_loop_when_retry_is_still_ended(self):
        ...
        result = session.call_tool("list_windows", {"session": "kopi-test"})
    ...
    def test_lifecycle_call_does_not_try_to_revive_itself(self):
        ...
        result = session.call_tool("start_session", {"session": "kopi-test"})
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Kopi-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (NousResearch/kopi-ai-agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (NousResearch/kopi-ai-agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (NousResearch/kopi-ai-agent#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Kopi patch.
    ...
    def test_future_renamed_subcommand_is_honored(self):
        ...
        instead of `mcp` keeps working without a Kopi patch."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (NousResearch/kopi-ai-agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (NousResearch/kopi-ai-agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (NousResearch/kopi-ai-agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Kopi never declared a cua-driver
