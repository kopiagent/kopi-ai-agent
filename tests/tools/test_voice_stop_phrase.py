...
class TestVoiceStopHint:
    ...
    def test_custom_phrase_uses_first_entry(self):
        with patch(
            ...
            return_value=("goodbye kopi", "stop"),
        ):
            assert voice_stop_hint() == 'Say "goodbye kopi" to end the voice chat.'
...
class TestIsVoiceStopPhrase:
    ...
    def test_custom_phrases(self):
        phrases = ("stop", "goodbye kopi")
        assert is_voice_stop_phrase("Goodbye Kopi!", phrases) is True
        assert is_voice_stop_phrase("goodbye kopi, one more thing", phrases) is False
...
class TestLoadVoiceStopPhrases:
    ...
    def test_custom_list(self):
        with self._with_cfg({"stop_phrases": ["Stop", "  Goodbye Kopi "]}):
            assert _load_voice_stop_phrases() == ("stop", "goodbye kopi")
    ...
    def test_config_error_falls_back(self):
        with patch("kopi_cli.config.load_config", side_effect=RuntimeError):
            assert _load_voice_stop_phrases() == DEFAULT_VOICE_STOP_PHRASES
