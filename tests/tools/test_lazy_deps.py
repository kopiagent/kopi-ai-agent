...
class TestIsSatisfiedVersionAware:
    ...
    def test_trace_upload_hub_at_core_locked_version_is_current(self, monkeypatch):
        ...
        core installs satisfies the trace-upload spec, so the `kopi update`
...
class TestActiveFeatures:
    ...
    def test_shared_dependency_does_not_activate_feature(self, monkeypatch):
        ...
        # reasons. It must not make kopi update try to refresh Matrix unless
...
class TestInstallSpecs:
    ...
    def test_sealed_venv_without_target_reports_immutable_reason(self, monkeypatch):
        # KOPI_DISABLE_LAZY_INSTALLS=1 with no durable target: never touch
        ...
        assert "KOPI_LAZY_INSTALL_TARGET" in result.reason
