...
def _make_backend(session: _FakeSession):
    ...
    backend._session_id = "kopi-session"
...
def test_backends_are_isolated_by_kopi_session_and_reused_within_it():
...
def test_concurrent_kopi_sessions_do_not_share_backend_state():
    from tools.computer_use import tool as computer_use
...
def _browser_route(driver: _BrowserDriver, session_id: str = "kopi-a"):
...
def test_exact_browser_binding_injects_kopi_session_capability():
    driver = _BrowserDriver()
    route = _browser_route(driver, session_id="kopi-owned-session")
    ...
    assert driver.calls == [
        (
            "get_browser_state",
            {"pid": 101, "window_id": 202, "session": "kopi-owned-session"},
        )
    ]
...
def test_typed_browser_refs_do_not_cross_route_sessions():
    driver = _BrowserDriver()
    first = _browser_route(driver, session_id="kopi-a")
    second = _browser_route(driver, session_id="kopi-b")
