...
def test_egress_node_options_overrides_conflicting_ca_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: ([], {"_KOPI_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca"}, []),
    )
    calls = _mock_subprocess_run(monkeypatch)
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Kopi image, or ``gosu``/``su``
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root Kopi home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Kopi process spawning a new container
    ...
    def _run(cmd, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                assert any(str(part).startswith("label=kopi-egress=") for part in cmd)
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
...
def test_forward_env_provider_key_collision_refuses_under_egress(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            {
                "HTTPS_PROXY": "http://host.docker.internal:9090",
                "OPENROUTER_API_KEY": "kopi-proxy-openrouter-token",
                "KOPI_PROXY_TOKEN_OPENROUTER_API_KEY": "kopi-proxy-openrouter-token",
            },
            [],
        ),
    )
...
def test_extra_args_proxy_override_refuses_under_egress(monkeypatch):
    """docker_extra_args are appended after Kopi args, so egress enforcement
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Kopi process uses it. Without this
...
def test_reuse_off_rejects_non_off_egress_container(monkeypatch):
    """When egress is off, a container that still has kopi-egress=on
    (e.g. from before ``kopi egress disable``) must be rejected and a
    ...
    def _run(cmd, **kwargs):
        if isinstance(cmd, list) and len(cmd) >= 2:
            if cmd[1] == "version":
                return subprocess.CompletedProcess(cmd, 0, stdout="ok", stderr="")
            if cmd[1] == "ps":
                # Return a container with kopi-egress=on.  With egress=off
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Kopi startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd Kopi processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a Kopi process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    ...
    assert "status=exited" in flat, (
        ...
        "belong to a sibling Kopi process and must NEVER be reaped"
    )
