"""Tests for tools.wake_word — the "Hey Kopi" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey kopi"}) == "hey kopi"
    assert ww.wake_phrase({}) == "hey kopi"
...
def test_looks_like_path():
    assert ww._looks_like_path("models/hey_kopi.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "kopi_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey kopi"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey kopi"
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_kopi.onnx"}}
    )
    assert calls["download"] == [["/models/hey_kopi.onnx"]]
    assert eng._labels == ["hey_kopi"]
...
def test_bundled_hey_kopi_model_ships_on_disk():
    # The "hey kopi" wake word works out of the box only if the model is
...
@pytest.mark.parametrize("model_value", [None, "", "hey_kopi", "hey kopi", "HEY_KOPI"])
def test_openwakeword_default_resolves_to_bundled_model(monkeypatch, model_value):
    # The default (and any "hey_kopi" alias) must load the bundled file, not be
...
def test_sherpa_engine_profile_routing_can_be_disabled(monkeypatch, tmp_path):
    ...
    eng = ww._SherpaKwsEngine({
        "provider": "sherpa", "phrase": "hey kopi", "profile_routing": False,
        "sherpa": {"model_dir": str(model_dir)},
    })
    assert eng._display_to_profile == {"HEY_KOPI": "default"}
    eng.close()
...
def test_sherpa_engine_match_maps_back_to_profile(monkeypatch, tmp_path):
    ...
    calls["results"].append("HEY_KOPI")
    assert eng.process(frame) is True
    assert eng.last_match == ("hey kopi", "default")
...
def test_enrolled_profile_phrases_reads_profile_configs(monkeypatch, tmp_path):
    ...
    import types as _types
    fake_profiles = _types.ModuleType("kopi_cli.profiles")
    ...
    monkeypatch.setitem(sys.modules, "kopi_cli.profiles", fake_profiles)
