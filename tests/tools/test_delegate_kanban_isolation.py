...
# cwd. Without an explicit PYTHONPATH the child resolves ``kopi_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("KOPI_PROFILE", "parent-worker")
    monkeypatch.setenv("KOPI_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("KOPI_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
    ...
    monkeypatch.setenv("KOPI_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with KOPI_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("KOPI_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KOPI_KANBAN_RUN_ID", "123")
    home = tmp_path / ".kopi"
    home.mkdir()
    monkeypatch.setenv("KOPI_HOME", str(home))
...
def test_delegate_child_terminal_env_scrubs_parent_kanban_keys(monkeypatch):
    monkeypatch.setenv("KOPI_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KOPI_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("KOPI_KANBAN_WORKSPACE", "/tmp/parent-workspace")
    monkeypatch.setenv("KOPI_KANBAN_CLAIM_LOCK", "lock")
    ...
    assert "KOPI_KANBAN_CLAIM_LOCK" not in env
...
def test_delegate_child_process_marker_scrubs_foreground_terminal_kanban_keys(monkeypatch):
    ...
    monkeypatch.setenv("KOPI_DELEGATED_CHILD_CONTEXT", "1")
    monkeypatch.setenv("KOPI_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KOPI_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("KOPI_KANBAN_DB", "/tmp/parent-kanban.db")
    monkeypatch.setenv("KOPI_KANBAN_WORKSPACE", "/tmp/parent-workspace")
    monkeypatch.setenv("KOPI_KANBAN_CLAIM_LOCK", "lock")
    ...
    env = _scrub_child_env(
        {
            ...
            "KOPI_KANBAN_TASK": "t_parent",
            "KOPI_KANBAN_RUN_ID": "123",
            "KOPI_KANBAN_DB": str(home / "kanban.db"),
            "KOPI_KANBAN_WORKSPACE": str(tmp_path / "parent-workspace"),
            "PATH": "/usr/bin",
        },
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote KOPI_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".kopi"
    home.mkdir()
    ...
    monkeypatch.setenv("KOPI_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("KOPI_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("KOPI_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("KOPI_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("KOPI_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("KOPI_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("KOPI_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("KOPI_DELEGATED_CHILD_CONTEXT") is None
    assert env["KOPI_HOME"] == str(home)
    assert env["KOPI_DELEGATED_CHILD_CONTEXT"] == "1"
    assert "KOPI_KANBAN_TASK" not in env
    assert "KOPI_KANBAN_RUN_ID" not in env
...
def test_delegate_child_execute_code_cannot_complete_parent_by_importing_kanban_db(
    ...
    code = "\n".join([
        ...
        "from kopi_cli import kanban_db as kb",
        "observed = {",
        "    'marker': os.environ.get('KOPI_DELEGATED_CHILD_CONTEXT'),",
        "    'is_child': is_delegated_child_process_context(),",
        "    'kanban_keys': sorted(k for k in os.environ if k.startswith('KOPI_KANBAN_')),",
        "}",
        "conn = sqlite3.connect(Path(os.environ['KOPI_HOME']) / 'kanban.db')",
