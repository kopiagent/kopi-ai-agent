...
Keys added via ``kopi auth add <provider>`` live in the credential pool /
auth store and used to be invisible to the voice tools, which only read
``os.environ`` + ``~/.kopi/.env`` via ``get_env_value``. The shared
...
def _no_dotenv(monkeypatch):
    """Keep the developer's real ~/.kopi/.env out of these tests."""
...
class TestToolWiring:
    ...
    def test_xai_env_fallback_consults_pool(self):
        ...
        def fake_load_pool(pid):
            ...
            # manual `kopi auth add xai` pool has the key.
