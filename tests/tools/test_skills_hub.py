...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  kopi:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["kopi"]["tags"] == ["a", "b"]
...
def _make_index_source(skills):
    """Build a KopiIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub import KopiIndexSource
    src = KopiIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestKopiIndexSearch:
...
class TestOptionalSkillSourceMetadata:
    def test_scan_all_emits_repo_root_relative_metadata(self, tmp_path):
        ...
        assert meta is not None
        assert meta.repo == "NousResearch/kopi-ai-agent"
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``kopi skills install … --name
