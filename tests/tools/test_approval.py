...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\kopi-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\kopi-victim", "Windows cmd destructive delete"),
...
class TestKopiConfigWriteProtection:
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.kopi/config.yaml",
            "echo '  mode: off' >> ~/.kopi/config.yaml",
            "echo x | tee ~/.kopi/config.yaml",
            "echo x | tee $KOPI_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.kopi/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Kopi absolute config.yaml is
        # handled by the project patterns, not the Kopi-home rule.
        for cmd in (
            "cat ~/.kopi/config.yaml",
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("KOPI_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        ...
        for variant in (
            ...
            "nohup python -m kopi_cli.main gateway run --replace",
        ):
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Kopi security config via IFS.
            "sed${IFS}-i ~/.kopi/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    ...
    def test_kill_pidof_expansion_detected(self):
        ...
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof kopi_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof kopi`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_against_kopi_label_detected(self):
        for cmd in (
            "launchctl stop ai.kopi.gateway",
            "launchctl kickstart -k system/ai.kopi.gateway",
            "launchctl bootout system/ai.kopi.gateway",
    ...
    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Kopi labels, are
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("kopi_cli.config.load_config_readonly", return_value=cfg):
