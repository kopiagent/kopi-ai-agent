...
class TestGating:
    ...
    def test_hidden_without_paid_service_access(self):
        ...
        with patch("kopi_cli.nous_account.get_nous_portal_account_info", return_value=account):
            assert flux3.check_bfl_requirements() is False
    ...
    def test_fails_closed_when_the_account_probe_raises(self):
        with patch("kopi_cli.nous_account.get_nous_portal_account_info", side_effect=RuntimeError("portal down")):
            assert flux3.check_bfl_requirements() is False
...
class TestPollTransport:
    ...
    def test_off_messaging_the_clip_stays_a_file_and_no_tag_is_offered(self, tmp_path, monkeypatch, platform):
        ...
        monkeypatch.setenv("KOPI_SESSION_PLATFORM", platform)
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
    ...
    def test_a_cli_session_is_recognised_by_its_source(self, tmp_path, monkeypatch):
        # The CLI, TUI, and desktop leave KOPI_SESSION_PLATFORM empty and
        # identify themselves on KOPI_SESSION_SOURCE instead, so keying only
        # on the platform would miss them.
        monkeypatch.delenv("KOPI_SESSION_PLATFORM", raising=False)
        monkeypatch.setenv("KOPI_SESSION_SOURCE", "tui")
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
