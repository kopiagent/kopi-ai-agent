...
def fake_kopi(tmp_path, monkeypatch):
    """Build a two-profile Kopi layout and point KOPI_HOME at
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_kopi):
        from tools.file_tools import write_file_tool
        target = fake_kopi["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_kopi):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_kopi["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_cross_profile_True_bypass(self, fake_kopi):
    ...
    def test_non_kopi_path_unaffected(self, fake_kopi, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_kopi):
    ...
    def test_cross_profile_patch_bypass(self, fake_kopi):
        from tools.file_tools import patch_tool
        target = fake_kopi["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_kopi):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_multiple_profiles(self, fake_kopi, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_kopi["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_kopi["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_kopi):
        ...
        assert "Active Kopi profile" in src
        assert "cross_profile=True" in src
        assert "~/.kopi/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Kopi profile: default" in src
        assert "Active Kopi profile: {active_profile}" in src
