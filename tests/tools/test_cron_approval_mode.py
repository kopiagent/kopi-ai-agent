...
class TestCronApprovalModeParsing:
    def test_default_is_deny(self):
        ...
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "deny"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "approve"}}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "off"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "allow"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "yes"}}):
            assert _get_cron_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "maybe"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_config_load_failure_defaults_to_deny(self):
        """If config loading fails entirely, default to deny (safe)."""
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", side_effect=RuntimeError("config broken")):
            assert _get_cron_approval_mode() == "deny"

    def test_yaml_boolean_false_maps_to_deny(self):
        """YAML 1.1 parses bare 'off' as False. Ensure it maps to deny."""
        from unittest.mock import patch as mock_patch
        with mock_patch("kopi_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": False}}):
...
class TestCronDenyModeAllGuards:
    ...
    def test_tirith_import_error_fail_closed_blocks_in_cron_deny(self, monkeypatch):
        ...
        with (
            ...
            mock_patch("tools.approval.detect_dangerous_command",
                       return_value=(False, None, None)),
            mock_patch("kopi_cli.config.load_config_readonly",
