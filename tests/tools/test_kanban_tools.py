...
def test_create_nested_default_scratch_children_each_get_own_workspace(
    ...
    monkeypatch.setenv("KOPI_KANBAN_TASK", child_result["task_id"])
...
def test_create_default_child_inherits_project_without_reusing_worktree(
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("KOPI_KANBAN_TASK", parent_id)
...
def test_create_cross_profile_project_children_keep_isolated_worktree_routing(
    ...
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
    monkeypatch.setenv("KOPI_KANBAN_DB", str(shared_db))
    monkeypatch.setenv("KOPI_HOME", str(profile_a))
    monkeypatch.setenv("KOPI_PROFILE", "creator")
    kb._INITIALIZED_PATHS.clear()
    kb.init_db()
    with pdb.connect_closing() as project_conn:
        project_id = pdb.create_project(
    ...
    monkeypatch.setenv("KOPI_PROFILE", "worker")
    monkeypatch.setenv("KOPI_KANBAN_TASK", parent_id)
    ...
    monkeypatch.setenv("KOPI_KANBAN_TASK", child_ids[0])
    grandchild_result = json.loads(kt._handle_create({
        "title": "nested review",
        "assignee": "reviewer",
        "parents": [child_ids[0]],
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("KOPI_HOME", str(home))
    monkeypatch.setenv("KOPI_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("KOPI_SESSION_CHAT_TYPE", "dm")
    monkeypatch.setenv("KOPI_SESSION_THREAD_ID", "20197")
    monkeypatch.setenv("KOPI_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("KOPI_SESSION_MESSAGE_ID", "msg-11")
...
def test_create_subscribes_gateway_session_with_active_profile_when_env_missing(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("KOPI_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("KOPI_SESSION_CHAT_ID", "chat-42")
    monkeypatch.delenv("KOPI_SESSION_PROFILE", raising=False)
    monkeypatch.delenv("KOPI_PROFILE", raising=False)
    monkeypatch.setattr("kopi_cli.profiles.get_active_profile_name", lambda: "spanorama")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting KOPI_ALLOW_PRIVATE_URLS on a private network.
...
def default_url_guard(monkeypatch):
    ...
    Clears KOPI_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
