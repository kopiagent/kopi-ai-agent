...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and kopi would run without zombie reaping.
    """
    ...
    assert "/opt/kopi/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/kopi/docker/main-wrapper.sh" in dispatcher, (
