...
#
# Installs a real, earlier Kopi the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `kopi`.
#
...
# Keep sandbox state out of the default .kopi-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".kopi-sandbox-e2e-$ROUTE"
export KOPI_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/kopi/.kopi/kopi-ai-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/kopi-ai-agent.git"
...
LOG_DIR="${KOPI_E2E_LOG_DIR:-$(mktemp -d -t kopi-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED kopi accept FLAG on `kopi update`?
#
...
# that silently rots. `kopi update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "kopi update --help 2>&1" | grep -qF -- "$flag"
}
...
require_kopi_works() {
  local when="$1" out
  out="$(in_sandbox "kopi --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "kopi --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "kopi runs $when"
}

# ── install the earlier Kopi ─────────────────────────────────────────────
...
require_kopi_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: kopi update'
    ...
    # kopi which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="kopi update --yes"
    else
      update_cmd="kopi update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "kopi update failed ($update_cmd)"
    fi
    require_landed_on_target 'kopi update'
    require_kopi_works 'after kopi update'
    ;;
  installer)
    ...
    require_kopi_works 'after installer re-run'
    ;;
esac
