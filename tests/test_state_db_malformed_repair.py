...
def test_fts_read_probe_returns_none_when_fts5_module_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kopi_state.sqlite3.connect", connect_no_fts5)
...
def test_fts_read_probe_returns_none_when_trigram_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kopi_state.sqlite3.connect", connect_no_trigram)
...
def test_repair_rebuilds_stale_btree_indexes(tmp_path):
    ...
    reason = kopi_state._db_opens_cleanly(db_path)
    assert reason is not None
    ...
    assert kopi_state._db_opens_cleanly(db_path) is None
    raw = sqlite3.connect(str(db_path))
