...
Kopi-side adapter behaves correctly against the documented response
shape.
"""
...
class TestJsonOutput:
    def test_json_output_is_parseable_round_trip(self):
        ...
        # health_report keys are preserved; Kopi adds kopi_identity.
        parsed = json.loads(out.getvalue())
        report = _ok_report()
        for key, value in report.items():
            assert parsed[key] == value
        assert "kopi_identity" in parsed
        assert parsed["kopi_identity"]["resolved_binary"]
...
class TestDriverCmdResolution:
    ...
    def test_env_var_used_when_no_arg_given(self, monkeypatch):
        ...
        with patch("shutil.which", return_value="/env/path/cua-driver") as which_mock, \
             patch("subprocess.Popen", return_value=proc), \
             patch("sys.stdout", new_callable=StringIO), \
             patch("kopi_cli.tools_config._cua_driver_cmd", side_effect=Exception("force env")):
...
class TestHealthReportFallback:
    ...
    def test_real_schema_version_1_preferred_over_fallback(self):
        ...
        parsed = json.loads(out.getvalue())
        # Upstream health_report keys pass through unchanged; Kopi adds
        # only the additive kopi_identity envelope.
...
class TestDoctorVersionIdentity:
    ...
    def test_json_includes_kopi_identity(self):
        from tools.computer_use import doctor
        ...
        assert "kopi_identity" in payload
        ident = payload["kopi_identity"]
    ...
    def test_matching_versions_no_mismatch_flag(self):
        ...
        assert code == 0
        payload = json.loads(out.getvalue())
        assert payload["kopi_identity"]["version_mismatch"] is False
