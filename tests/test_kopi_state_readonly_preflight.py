...
- files inside the Kopi home tree are repaired with ``chmod u+rw``
...
import kopi_state
from kopi_state import SessionDB, preflight_db_writability
...
def kopi_home(tmp_path, monkeypatch):
    """Isolated KOPI_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".kopi"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, kopi_home):
        db = kopi_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, kopi_home):
        db = kopi_home / "state.db"
        _make_wal_db(db)
    ...
    def test_wal_data_survives_repair(self, kopi_home):
    ...
    def test_repairs_readonly_parent_directory(self, kopi_home):
        sub = kopi_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, kopi_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, kopi_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    def test_memory_uri_skipped(self, kopi_home):
        preflight_db_writability(Path(":memory:"))

    def test_file_uri_skipped(self, kopi_home):
        preflight_db_writability(Path("file:whatever?mode=ro"))

    def test_missing_files_no_error(self, kopi_home):
        preflight_db_writability(kopi_home / "state.db")

    def test_healthy_db_untouched(self, kopi_home):
        db = kopi_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, kopi_home):
