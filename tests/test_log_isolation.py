"""The test suite must never write into the operator's real Kopi logs.

`kopi_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<KOPI_HOME>/logs/agent.log`.

If KOPI_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets KOPI_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(kopi_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_kopi_home_is_sandboxed_before_imports(self):
        # Deliberately NOT os.environ: by test time the per-test `_isolate_env`
        # fixture has sandboxed KOPI_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import KOPI_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set KOPI_HOME before test modules import"
        assert Path(home).resolve() != _real_kopi_home().resolve(), (
            f"KOPI_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("kopi_cli.main")

        real_logs = str(_real_kopi_home() / "logs")
        offenders = [p for p in _all_file_destinations() if p.startswith(real_logs)]

        assert offenders == [], (
            "the test session is writing into the operator's real Kopi logs:\n  "
            + "\n  ".join(offenders)
        )
