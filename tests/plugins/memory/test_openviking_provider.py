...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_kopi_openviking_config()
...
def test_post_setup_create_remote_user_profile_can_mirror_to_openviking_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            ...
            "Kopi peer ID in OpenViking": "kopi",
...
def test_post_setup_create_remote_user_can_keep_kopi_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            "OpenViking server URL": "https://openviking.example",
            "OpenViking user API key": "user-secret",
            "Kopi peer ID in OpenViking": "agent",
        }),
    )
    config = {"memory": {}}
...
def test_post_setup_user_key_path_can_route_detected_root_key_to_root_setup(tmp_path, monkeypatch):
    ...
    assert prompt_events.count("Kopi peer ID in OpenViking") == 1
    env_text = (kopi_home / ".env").read_text(encoding="utf-8")
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this Kopi run."
    ]
...
def test_sync_turn_chunks_structured_messages_to_openviking_limit(
    ...
    ] == provider._messages_to_openviking_batch(messages, assistant_peer_id="kopi")
...
def test_initialize_recovers_pending_session_from_previous_process(tmp_path, monkeypatch):
    ...
    later.initialize("third-sid", kopi_home=str(tmp_path))
...
def test_initialize_skips_pending_session_owned_by_live_same_profile_provider(tmp_path, monkeypatch):
    ...
    live_owner.initialize("owned-sid", kopi_home=str(tmp_path))
    ...
    other_provider.initialize("other-sid", kopi_home=str(tmp_path))
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._kopi_home = str(tmp_path)
