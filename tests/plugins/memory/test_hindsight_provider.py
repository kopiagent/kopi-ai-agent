...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Kopi Agent and the User"
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Kopi Agent and the User"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Kopi Agent and the User"
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kopi_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("kopi_cli.config.save_config", lambda c: None)
...
class TestClientAutoUpgradeRoutesThroughLazyDeps:
    ...
    def _init_with_outdated_client(self, tmp_path, monkeypatch, outcome):
        ...
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", kopi_home=str(tmp_path), platform="cli")
        return calls
