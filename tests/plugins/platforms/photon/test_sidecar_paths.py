...
``/opt/kopi``; ``resolve_sidecar_dir`` must run in place when the deps are
baked and current, and mirror the sidecar to the writable ``KOPI_HOME``
...
def test_readonly_source_missing_deps_mirrors_to_kopi_home(
    tmp_path, monkeypatch
) -> None:
...
def test_mirror_failure_falls_back_to_source(tmp_path, monkeypatch) -> None:
    """If KOPI_HOME is unusable too, return the source dir (fail-open)."""
    monkeypatch.delenv("PHOTON_SIDECAR_DIR", raising=False)
    # Point KOPI_HOME at a path under a file so mkdir fails.
    blocker = tmp_path / "blocker"
    blocker.write_text("", encoding="utf-8")
    monkeypatch.setenv("KOPI_HOME", str(blocker / "home"))
    source = tmp_path / "src"
    _seed_source(source)
    _freeze_writability(monkeypatch, writable=False)
...
def test_adapter_import_does_not_resolve_sidecar_dir(monkeypatch) -> None:
    ...
    files to KOPI_HOME; the adapter and CLI resolve lazily on first use so
    a bare import (plugin discovery, `kopi --help`, test collection) has
