...
def test_save_auth_never_world_readable(tmp_kopi_home: Path) -> None:
    """auth.json must be created 0o600 — no window at process umask."""
    photon_auth.store_photon_token("secret-token")
    mode = (tmp_kopi_home / "auth.json").stat().st_mode & 0o777
    assert mode == 0o600


def test_save_auth_leaves_no_temp_files(tmp_kopi_home: Path) -> None:
    photon_auth.store_photon_token("secret-token")
    leftovers = [
        p.name
        for p in tmp_kopi_home.iterdir()
        # auth.lock is the cross-process lock sentinel from
        # kopi_cli.auth._auth_store_lock — expected to persist.
        if p.name not in ("auth.json", "auth.lock")
    ]
    assert leftovers == []


def test_save_auth_uses_os_open_with_0o600_mode(tmp_kopi_home: Path) -> None:
...
def test_save_auth_closes_raw_fd_when_fdopen_fails(tmp_kopi_home: Path) -> None:
...
# cross-process lock kopi_cli/auth.py's ~15 other writers all use, so a
...
def _hold_auth_lock_then_release(hold_event: threading.Event, release_event: threading.Event) -> None:
    from kopi_cli.auth import _auth_store_lock
...
def test_store_functions_block_on_auth_store_lock(
    tmp_kopi_home: Path, call,
) -> None:
    """store_* writers must serialize against kopi_cli.auth's cross-process lock.
...
def test_list_projects_unwraps_list(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body=[{"id": "p1", "name": "Kopi Agent"}])
...
def test_find_project_by_name_case_insensitive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "get", fake_get)
    proj = photon_auth.find_project_by_name("tok", "Kopi Agent")
...
def test_create_project_omits_spectrum_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "post", fake_post)
    data = photon_auth.create_project("tok", name="Kopi Agent")
    ...
    assert captured["body"]["name"] == "Kopi Agent"
...
def test_create_project_unwraps_success_data(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    data = photon_auth.create_project("dashboard-token", name="Kopi Agent")
