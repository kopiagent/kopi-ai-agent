...
gateway process memory + sidecar child env — so cron/`kopi send`
standalone sends structurally could not authenticate. The adapter now
persists ``<kopi-home>/runtime/photon-sidecar.json`` after the sidecar
...
async def test_standalone_send_consumes_record_when_env_missing(
    ...
    assert ":9111/" in url
    assert headers["X-Kopi-Sidecar-Token"] == "record-token"
...
async def test_standalone_send_env_token_still_wins(
    ...
    assert headers["X-Kopi-Sidecar-Token"] == "env-token"
