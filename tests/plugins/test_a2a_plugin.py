...
class TestAgentCardV1:
    def test_card_shape(self):
        card = protocol.build_agent_card(
            name="kopi-test", url="http://localhost:9900/",
            description="test", skills=[], streaming=False, auth_required=False,
        )
        assert card["name"] == "kopi-test"
...
class TestV1SpecRegressionFixes:
    ...
    def test_forward_to_profile_first_contact_creates_then_resumes_fake_kopi(self, monkeypatch, tmp_path):
        from plugins.platforms.a2a.adapter import A2AAdapter
        from gateway.config import PlatformConfig
        ...
        kopi = fakebin / "kopi"
        kopi.write_text("""#!/usr/bin/env python3
import json, os, sqlite3, sys, time
calls = os.environ['FAKE_KOPI_CALLS']
with open(calls, 'a') as f:
    f.write(json.dumps(sys.argv[1:]) + '\\n')
home = os.environ['KOPI_HOME']
...
""")
        ...
        monkeypatch.setenv("FAKE_KOPI_CALLS", str(calls))
