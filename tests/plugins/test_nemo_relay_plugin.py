...
from kopi_cli import lifecycle, plugins as plugin_api
...
def test_shared_metrics_and_rich_plugin_share_one_core_session(
    ...
    monkeypatch.setenv("KOPI_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv(
        "KOPI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif")
    )
    monkeypatch.setattr(
    ...
    register_metrics = next(
        ...
        and item[1].startswith("kopi.nemo_relay.shared_metrics.")
    )
    register_atif = next(
    ...
    packages = list(
        (kopi_home / "telemetry" / "shared_metrics" / "outbox").glob("*.json")
    )
    assert len(packages) == 1
    package = json.loads(packages[0].read_text(encoding="utf-8"))
    assert package["metrics"][0]["name"] == "kopi.model_call.count"
...
def test_nemo_relay_plugin_reuses_core_parented_child_scope_for_embedded_atif(monkeypatch):
    ...
    stop_mark_index = next(
        index
        for index, event in enumerate(fake.events)
        if event[0] == "scope.event" and event[1] == "kopi.subagent.stop"
    )
...
""",
    ...
    assert "Use Kopi-owned [[dynamic_plugins]]" in caplog.text
...
def test_relay_tool_request_rewrite_precedes_kopi_authorization_boundary(
    tmp_path,
    monkeypatch,
):
...
def test_nemo_relay_plugin_activates_without_duplicate_execution_hooks(
    ...
    class _Context:
        ...
        def register_middleware(self, name, callback):
            del callback
            fake.events.append(("kopi.register_middleware", name))

    plugin.register(_Context())

    event_names = [event[0] for event in fake.events]
    assert "plugin.activate_dynamic" in event_names
    assert "kopi.register_middleware" not in event_names
    assert not {
        "pre_api_request",
        "post_api_request",
        "api_request_error",
