"""POSIX advisory locks must survive Kopi' own database inspection.
...
Kopi used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `kopi sessions
...
out across Kopi' inspection calls.
"""
...
from kopi_cli.sqlite_safe_read import (
...
def clean_registry():
    ...
    import kopi_cli.sqlite_safe_read as mod
...
def test_write_lock_survives_file_length_check(tmp_path, journal_mode, clean_registry):
    ...
    from kopi_cli.kanban_db import _check_file_length_invariant
...
def test_write_lock_survives_zeroed_state_db_probe(
    ...
    from kopi_state import is_zeroed_state_db
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import kopi_cli.sqlite_safe_read as ssr
