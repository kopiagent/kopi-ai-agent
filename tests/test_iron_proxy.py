...
def kopi_home(tmp_path, monkeypatch):
    """Point KOPI_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_discover_provider_mappings_from_env(kopi_home, monkeypatch):
...
def test_discover_provider_mappings_explicit_names(kopi_home):
    ms = ip.discover_provider_mappings(
...
def test_discover_provider_mappings_empty(kopi_home):
...
def test_wizard_rendered_yaml_contains_deny_list(kopi_home, tmp_path):
...
def test_write_and_load_mappings_roundtrip(kopi_home):
...
def test_load_mappings_handles_missing_file(kopi_home):
    assert ip.load_mappings() == []


def test_load_mappings_handles_corrupt_json(kopi_home):
    state = ip._proxy_state_dir()
    (state / "mappings.json").write_text("{not json", encoding="utf-8")
    assert ip.load_mappings() == []


def test_write_proxy_config_serializes_yaml(kopi_home, tmp_path):
    ca_crt = tmp_path / "ca.crt"
    ca_key = tmp_path / "ca.key"
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
...
def test_merge_mappings_preserves_existing_tokens():
    ...
    existing = [
        ip.TokenMapping(
            proxy_token="kopi-proxy-original-12345",
    ...
    assert by_name["OPENROUTER_API_KEY"].proxy_token == "kopi-proxy-original-12345"
    # New provider got a fresh token.
    assert by_name["OPENAI_API_KEY"].proxy_token != "kopi-proxy-original-12345"
...
def test_merge_mappings_rotate_mints_fresh_tokens():
    ...
    assert merged[0].proxy_token != "kopi-proxy-original-12345"
...
def test_uncovered_providers_detects_aws_gcp_appdefault(kopi_home, monkeypatch):
...
def test_uncovered_providers_skips_bearer_providers(kopi_home, monkeypatch):
...
def test_uncovered_providers_skips_header_auth_providers(kopi_home, monkeypatch):
...
def test_find_iron_proxy_returns_none_when_missing(kopi_home, monkeypatch):
    monkeypatch.setattr("shutil.which", lambda name: None)
    assert ip.find_iron_proxy(install_if_missing=False) is None


def test_find_iron_proxy_returns_managed_first(kopi_home, monkeypatch):
    managed = ip._kopi_bin_dir() / ip._platform_binary_name()
...
def test_install_iron_proxy_verifies_checksum_and_extracts(kopi_home, monkeypatch):
...
def test_install_iron_proxy_rejects_bad_checksum(kopi_home, monkeypatch):
    fake_payload = _make_fake_tar(ip._platform_binary_name())
    asset_name = ip._platform_asset_name()
...
def test_install_iron_proxy_rejects_missing_checksum_entry(kopi_home, monkeypatch):
    fake_payload = _make_fake_tar(ip._platform_binary_name())
...
def test_verify_checksums_signature_skips_without_gpg(kopi_home, monkeypatch, tmp_path):
...
def test_verify_checksums_signature_skips_when_sig_assets_missing(kopi_home, monkeypatch, tmp_path):
...
def test_verify_checksums_signature_raises_on_bad_signature(kopi_home, monkeypatch, tmp_path):
...
def test_verify_checksums_signature_passes_on_good_signature(kopi_home, monkeypatch, tmp_path):
...
def test_install_aborts_on_bad_release_signature(kopi_home, monkeypatch):
...
def test_get_status_when_nothing_configured(kopi_home):
...
def test_get_status_with_config_present(kopi_home, monkeypatch):
    # Materialize binary, config, and ca cert.
    bin_path = ip._kopi_bin_dir() / ip._platform_binary_name()
...
def test_stop_proxy_handles_missing_pidfile(kopi_home):
    # No pidfile → stop returns False, doesn't raise.
    assert ip.stop_proxy() is False


def test_stop_proxy_cleans_stale_pidfile(kopi_home, monkeypatch):
...
def test_start_proxy_refuses_without_binary(kopi_home, monkeypatch):
...
def test_start_proxy_refuses_without_config(kopi_home, monkeypatch):
    binary = ip._kopi_bin_dir() / ip._platform_binary_name()
...
def test_start_proxy_writes_pidfile_when_alive(kopi_home, monkeypatch):
    binary = ip._kopi_bin_dir() / ip._platform_binary_name()
    binary.parent.mkdir(parents=True, exist_ok=True)
    binary.write_bytes(b"")
    binary.chmod(0o755)
...
def test_start_proxy_raises_when_immediate_exit(kopi_home, monkeypatch):
    binary = ip._kopi_bin_dir() / ip._platform_binary_name()
    binary.parent.mkdir(parents=True, exist_ok=True)
    binary.write_bytes(b"")
    binary.chmod(0o755)
...
def test_start_proxy_idempotent_when_already_running(kopi_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_docker_egress_args_empty_when_disabled(kopi_home, monkeypatch):
...
def test_docker_egress_args_when_enabled_but_unconfigured_raises(kopi_home, monkeypatch):
    from tools.environments.docker import _egress_proxy_args_for_docker
...
def test_docker_egress_args_when_unconfigured_no_enforce(kopi_home, monkeypatch):
    from tools.environments.docker import _egress_proxy_args_for_docker
    from kopi_cli.config import load_config, save_config
...
def test_docker_egress_args_full_path(kopi_home, monkeypatch):
    ...
    assert "-v" in vol
    assert any("kopi-egress-ca.crt" in arg for arg in vol)
    ...
    assert env["REQUESTS_CA_BUNDLE"].endswith("kopi-egress-ca.crt")
    ...
    assert env["KOPI_PROXY_TOKEN_OPENROUTER_API_KEY"] == mapping.proxy_token
    # Linux host-gateway mapping
    assert host == ["--add-host", "host.docker.internal:host-gateway"]


def test_docker_egress_fingerprint_changes_with_tokens(kopi_home, monkeypatch):
    ...
    first = _egress_reuse_fingerprint(
        ["-v", "/tmp/ca:/etc/ssl/certs/kopi-egress-ca.crt:ro"],
...
def test_subprocess_env_strips_unrelated_secrets(kopi_home, monkeypatch):
...
def test_subprocess_env_strips_proxy_recursion_vars(kopi_home, monkeypatch):
...
def test_subprocess_env_keeps_infrastructure_vars(kopi_home, monkeypatch):
...
def test_ca_key_created_with_0o600(kopi_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(kopi_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(kopi_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_docker_egress_args_raises_on_empty_mappings(kopi_home, monkeypatch):
...
def test_docker_egress_args_raises_when_ca_vanishes(kopi_home, monkeypatch):
    """status.configured was True at check time but the CA file
    disappeared between then and now (e.g. operator manually deleted
    ~/.kopi/proxy/ca.crt).  enforce_on_docker=True must refuse."""
...
def test_docker_env_collision_with_proxy_raises_when_enforce(kopi_home, monkeypatch):
...
def test_header_auth_providers_discovered(kopi_home, monkeypatch):
...
def test_gemini_alias_collapses_to_single_mapping(kopi_home, monkeypatch):
...
def test_gemini_alias_only_still_mints_mapping(kopi_home, monkeypatch):
...
def test_mappings_roundtrip_preserves_headers_and_aliases(kopi_home):
    m = ip.TokenMapping(
...
def test_load_mappings_legacy_entries_default_to_bearer(kopi_home):
...
def test_subprocess_env_mirrors_alias_into_canonical(kopi_home, monkeypatch):
...
def test_subprocess_env_canonical_wins_over_alias(kopi_home, monkeypatch):
    m = ip.TokenMapping(
        proxy_token=ip.mint_proxy_token("gemini"),
        real_env_name="GEMINI_API_KEY",
        upstream_hosts=("generativelanguage.googleapis.com",),
...
def test_ensure_management_token_persists_and_is_stable(kopi_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("kopi-mgmt-")
    p = ip._proxy_state_dir() / "management.token"
    assert p.exists()
    assert (p.stat().st_mode & 0o777) == 0o600


def test_ensure_management_token_force_rotates(kopi_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token(force=True)
    assert t1 != t2


def test_reload_proxy_refuses_when_not_running(kopi_home, monkeypatch):
    monkeypatch.setattr(ip, "_read_pid", lambda: None)
    with pytest.raises(RuntimeError, match="not running"):
        ip.reload_proxy()


def test_reload_proxy_refuses_on_pre_management_config(kopi_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(kopi_home, monkeypatch):
    monkeypatch.setattr(ip, "_read_pid", lambda: 4242)
    monkeypatch.setattr(ip, "_pid_alive", lambda pid: True)
    monkeypatch.setattr(
...
def test_start_proxy_injects_management_key_env(kopi_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=kopi_home / "ca.crt",
        ca_key=kopi_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (kopi_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = kopi_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("kopi-mgmt-")
...
def test_stop_proxy_suppresses_sigkill_on_pid_recycle(kopi_home, monkeypatch):
...
def test_docker_egress_node_options_uses_sentinel(kopi_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_KOPI_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_KOPI_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(kopi_home, monkeypatch):
...
def test_persisted_nonce_returns_none_when_missing(kopi_home):
...
def test_read_http_listen_from_config_returns_host_and_port(kopi_home):
...
def test_read_http_listen_from_config_missing_file(kopi_home):
    assert ip._read_http_listen_from_config() is None


def test_get_status_probes_configured_bind_host(kopi_home, monkeypatch):
