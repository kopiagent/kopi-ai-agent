...
class TestStatusBarSegment:
    ...
    def test_snapshot_exposes_focus_label(self):
        ...
        snapshot = KopiCLI._get_status_bar_snapshot(host)
        assert snapshot["focus_label"] == FOCUS_STATUSBAR_LABEL

        host._focus_view_enabled = False
        assert KopiCLI._get_status_bar_snapshot(host)["focus_label"] == ""
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        with patch.object(KopiCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(KopiCLI, "_is_session_yolo_active", return_value=False):
            text = KopiCLI._build_status_bar_text(host, width=width)
    ...
    def test_fragment_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        with patch.object(KopiCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(KopiCLI, "_get_tui_terminal_width", return_value=width), \
             patch.object(KopiCLI, "_is_session_yolo_active", return_value=False):
            frags = KopiCLI._get_status_bar_fragments(host)
    ...
    def test_badge_absent_from_fragments_when_focus_is_off(self):
        ...
        with patch.object(KopiCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(KopiCLI, "_get_tui_terminal_width", return_value=120), \
             patch.object(KopiCLI, "_is_session_yolo_active", return_value=False):
            frags = KopiCLI._get_status_bar_fragments(host)
