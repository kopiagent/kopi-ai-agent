...
The recovery path lives in ``KopiCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A KopiCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(KopiCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise KopiCLI._recover_terminal_after_interrupt()."""
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(KopiCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(KopiCLI, "_recover_terminal_after_interrupt")
        assert callable(KopiCLI._recover_terminal_after_interrupt)
