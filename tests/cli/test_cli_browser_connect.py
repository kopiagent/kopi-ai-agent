...
class TestChromeDebugLaunch:
    ...
    def test_launch_tries_next_browser_when_first_candidate_fails(self):
        ...
        with patch("kopi_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
             patch("kopi_cli.browser_connect._wait_for_browser_debug_ready_or_exit", return_value="ready"), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert KopiCLI._try_launch_chrome_debug(9222, "Linux") is True
    ...
    def test_connect_context_note_allows_expected_browser_use(self, monkeypatch):
        ...
        with patch(
                 "kopi_cli.cli_commands_mixin.discover_local_cdp_url",
                 return_value="http://127.0.0.1:9222",
             ), \
             patch("kopi_cli.cli_commands_mixin.is_browser_debug_ready", return_value=True), \
