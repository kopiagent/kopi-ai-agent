...
class TestWidenedPruner:
    ...
    Previously only ``kopi-*`` directories were considered, so salvage/
    ...
    # -- named (non kopi-*) directories are now covered ------------------
    ...
    def test_squash_merged_tree_is_reaped(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "kopi-merged", commit=True, age_h=100)
    ...
    def test_partially_merged_tree_survives(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "kopi-partial", commit=True, age_h=100)
        self._merge_upstream(git_repo, sha)
    ...
    def test_merged_predicate_true_on_patch_equivalence(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "kopi-eq", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_merged_predicate_false_on_unique_work(self, git_repo):
        import cli
        wt, _ = self._mk(git_repo, "kopi-uniq", commit=True)
    ...
    def test_merged_predicate_fails_safe_on_stale_base(self, git_repo):
        import cli
        wt, _ = self._mk(git_repo, "kopi-manyahead", commit=True)
...
class TestMergeVerdictCache:
    ...
    dominates ``kopi -w`` startup (~0.2-1.0s per worktree, re-run on every
    ...
    def test_cache_hit_matches_uncached_verdict(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "kopi-cachehit", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_cache_records_negative_verdicts_too(self, git_repo):
        import cli
        wt, _ = self._mk(git_repo, "kopi-cacheneg", commit=True)
        cache = {}
    ...
    def test_new_commit_invalidates_cached_verdict(self, git_repo):
        ...
        """
        import cli
        wt, sha = self._mk(git_repo, "kopi-moves", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_cached_tree_with_new_work_is_still_preserved(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "kopi-warmsafe", commit=True)
        self._merge_upstream(git_repo, sha)
        ...
        wt2, _ = self._mk(git_repo, "kopi-warmsafe", commit=True)
    ...
    def test_corrupt_cache_file_is_ignored(self, git_repo, monkeypatch, tmp_path):
        ...
        wt, _ = self._mk(git_repo, "kopi-corrupt", commit=True)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestPruneParallelEquivalence:
    ...
    def test_pool_failure_falls_back_to_serial(self, git_repo, monkeypatch):
        ...
        wt, sha = self._mk(git_repo, "kopi-poolfail", commit=True)
        self._merge_upstream(git_repo, sha)
