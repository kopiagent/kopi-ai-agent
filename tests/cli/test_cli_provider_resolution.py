...
def test_cli_init_wires_moa_preset_model_to_moa_provider(monkeypatch):
    ...
    shell = cli.KopiCLI(model="moa:strategy", compact=True, max_turns=1)
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "kopi_cli.config.save_env_value",
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    _save_custom_provider(
        ...
        api_key="sk-secret",
        name="Ollama",
        key_env="KOPI_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "KOPI_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_save_custom_provider_migrates_an_existing_plaintext_entry(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("kopi_cli.config.load_config", lambda: existing)
    saved = {}
    monkeypatch.setattr("kopi_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from kopi_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
...
def test_custom_endpoint_key_env_separates_ports_on_one_host():
    ...
    from kopi_cli.config import custom_endpoint_key_env
