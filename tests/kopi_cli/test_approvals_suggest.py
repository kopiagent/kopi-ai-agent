"""Tests for ``kopi approvals suggest`` (kopi_cli/approvals_suggest.py).

Approval history in Kopi is implied, not ledgered: the session DB
...
from kopi_cli.approvals_suggest import (
    Proposal,
    apply_proposals,
    build_proposals,
    derive_glob,
...
class TestNormalizeAndGlob:
    ...
    def test_derive_glob_flag_second_token_falls_back_to_root(self):
        assert derive_glob("kopi --yolo update") == "kopi --yolo".split()[0] + " *"
...
class TestRankingAndSafety:
    ...
    def test_benign_classes_are_not_excluded(self):
        for desc in (
            ...
            "kopi update (restarts gateway, kills running agents)",
