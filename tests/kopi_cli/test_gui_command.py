...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Kopi.app" / "Contents" / "MacOS" / "Kopi"
    elif platform == "win32":
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Kopi-managed
    ...
    # A managed Node tree on disk so with_kopi_node_path() actually prepends it.
    home = tmp_path / "kopi-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["kopi"], 0)

    with patch("kopi_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("kopi_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("kopi_cli.main._desktop_build_needed", return_value=True), \
         patch("kopi_cli.main._write_desktop_build_stamp"), \
         patch("kopi_cli.main.subprocess.run", side_effect=[subprocess.CompletedProcess([], 0), launch_ok]), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_kopi_node_dirs() if p.is_dir()]
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    kopi_root = tmp_path / "custom-kopi"
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Kopi.app/Contents/MacOS/Kopi in place. Re-downloading Electron can't
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "Kopi.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "Kopi.exe"
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Kopi.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Kopi.app"
    _write_info_plist(app, "com.nousresearch.kopi")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Kopi").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Kopi Helper.app"
    _write_info_plist(helper, "com.nousresearch.kopi.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Kopi.app component, silently skipping every .node/.dylib/crashpad
...
def test_desktop_macos_local_codesign_stable_requirements_and_entitlements(tmp_path, monkeypatch):
    ...
    assert any(
        '=designated => identifier "com.nousresearch.kopi"' in c and str(ent_main) in c
        for c in sign_calls
    )
    assert any(
        '=designated => identifier "com.nousresearch.kopi.helper"' in c and str(ent_inherit) in c
        for c in sign_calls
    )
...
def test_desktop_macos_local_codesign_keychain_identity_skips_requirements(tmp_path, monkeypatch):
    ...
    assert cli_main._desktop_macos_local_codesign(
        app, desktop_dir=desktop_dir, identity="Kopi Local Signing"
    ) is True

    sign_calls = [c for c in calls if c[:3] == ["/usr/bin/codesign", "--force", "--sign"]]
    assert sign_calls, "expected sign invocations"
    assert all(c[3] == "Kopi Local Signing" for c in sign_calls)
...
def test_relaunchable_fixup_noop_when_publisher_signing_configured(tmp_path, monkeypatch):
    ...
    with patch("kopi_cli.main.subprocess.run") as run:
...
def test_desktop_macos_local_signing_identity_reads_config(monkeypatch):
    ...
    with patch(
        ...
        return_value={"desktop": {"macos_signing_identity": "  Kopi Local Signing  "}},
    ):
        assert cli_main._desktop_macos_local_signing_identity() == "Kopi Local Signing"
    with patch("kopi_cli.config.load_config", return_value={"desktop": {}}):
