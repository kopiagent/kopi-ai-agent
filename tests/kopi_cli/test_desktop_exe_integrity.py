...
Kopi.exe on the end user's machine. Before this gate, "build succeeded" was
...
def test_parse_pe_machine_reads_machine_field(tmp_path):
    exe = make_pe(tmp_path / "Kopi.exe", PE_AMD64)
...
def test_parse_pe_machine_rejects_truncated_sections(tmp_path):
    ...
    exe = make_pe(tmp_path / "Kopi.exe", PE_AMD64, truncate_to=0x300)
...
def test_expected_machines_per_host(host, loadable, not_loadable):
    with patch("kopi_cli.main._windows_native_machine", return_value=host):
...
def test_expected_machines_unknown_host_is_permissive():
    ...
    with patch("kopi_cli.main._windows_native_machine", return_value="RISCV64"):
        expected = cli_main._expected_windows_pe_machines()
...
# winnt.h PROCESSOR_ARCHITECTURE_* — mirrors kopi_cli.main constants.
_SYSINFO_AMD64 = 9
_SYSINFO_ARM64 = 12
...
def test_native_machine_system_info_fallback_when_iswow64_fails(monkeypatch):
    ...
    correctly-built ARM64 Kopi.exe."""
    import ctypes
...
def test_integrity_gate_accepts_arm64_exe_from_emulated_x64_process(monkeypatch, tmp_path):
    ...
    process, correctly-built ARM64 Kopi.exe. The gate must pass it."""
    import ctypes

    monkeypatch.setattr(cli_main.sys, "platform", "win32")
    exe = make_pe(tmp_path / "Kopi.exe", PE_ARM64)
    with patch.object(ctypes, "WinDLL", _fake_windll(PE_ARM64), create=True), \
         patch("platform.machine", return_value="AMD64"):
...
def test_integrity_gate_accepts_arm64_when_iswow64_fails_but_system_info_ok(
    ...
    GetNativeSystemInfo reports ARM64, ARM64 Kopi.exe must pass the gate."""
    import ctypes
...
def test_integrity_error_none_for_matching_arch(tmp_path):
    exe = make_pe(tmp_path / "Kopi.exe", PE_AMD64)
    with patch("kopi_cli.main._windows_native_machine", return_value="AMD64"):
        assert cli_main._desktop_exe_integrity_error(exe) is None
...
def test_packaged_executable_prefers_host_arch_over_mtime(tmp_path, monkeypatch):
    ...
    good = make_pe(desktop_dir / "release" / "win-unpacked" / "Kopi.exe", PE_AMD64)
    bad = make_pe(desktop_dir / "release" / "win-arm64-unpacked" / "Kopi.exe", PE_ARM64)
...
def test_packaged_executable_falls_back_to_mtime_when_unparseable(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main.sys, "platform", "win32")
    desktop_dir = tmp_path / "apps" / "desktop"
    a = desktop_dir / "release" / "win-unpacked" / "Kopi.exe"
    b = desktop_dir / "release" / "win-arm64-unpacked" / "Kopi.exe"
    for p in (a, b):
...
def _win_tree(tmp_path: Path) -> tuple[Path, Path]:
    desktop_dir = tmp_path / "apps" / "desktop"
    exe = desktop_dir / "release" / "win-unpacked" / "Kopi.exe"
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Kopi.exe"
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Kopi.exe").exists()
...
def test_gate_rolls_back_corrupt_exe_and_purges_cache(tmp_path, monkeypatch, capsys):
    ...
    desktop_dir, exe = _win_tree(tmp_path)
    make_pe(exe, PE_AMD64, truncate_to=0x300)
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "Kopi.exe", PE_AMD64)
    ...
    with patch("kopi_cli.main._windows_native_machine", return_value="AMD64"), \
         ...
         patch("kopi_cli.main._desktop_stamp_path", return_value=stamp):
        verified, rolled_back = cli_main._ensure_desktop_exe_launchable(desktop_dir, exe)
...
def test_gate_fails_clearly_without_backup(tmp_path, monkeypatch, capsys):
    ...
    with patch("kopi_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("kopi_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
        verified, rolled_back = cli_main._ensure_desktop_exe_launchable(desktop_dir, exe)
...
# ─── end-to-end: `kopi desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Kopi.exe cannot launch
    must exit nonzero (so kopi-setup's retry-once kicks in) and must restore
    ...
    with patch("kopi_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("kopi_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         patch("kopi_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("kopi_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("kopi_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("kopi_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("kopi_cli.main.subprocess.run", return_value=pack_ok), \
...
def test_build_only_succeeds_with_valid_exe(tmp_path, monkeypatch, capsys):
    ...
    make_pe(desktop_dir / "release" / "win-unpacked" / "Kopi.exe", PE_AMD64)
