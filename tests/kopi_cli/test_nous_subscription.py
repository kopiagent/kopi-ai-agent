...
def test_has_agent_browser_resolves_via_kopi_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Kopi Node
    ...
    monkeypatch.setattr(
        "kopi_constants.with_kopi_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "kopi_constants.agent_browser_runnable",
...
def test_has_agent_browser_false_when_nothing_runnable(monkeypatch):
    ...
    monkeypatch.setattr("kopi_constants.with_kopi_node_path", lambda: {"PATH": ""})
    monkeypatch.setattr("kopi_constants.agent_browser_runnable", lambda p: False)
