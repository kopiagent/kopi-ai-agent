...
class TestSystemUnitRefreshSyncsKopiHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.kopi."""

    def test_refresh_adopts_unit_kopi_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_kopi = root_home / ".kopi"
        alice_kopi = alice_home / ".kopi"
        root_kopi.mkdir(parents=True)
        alice_kopi.mkdir(parents=True)
        (root_kopi / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_kopi / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's KOPI_HOME + drain timeout).
        monkeypatch.setenv("KOPI_HOME", str(alice_kopi))
        ...
        # Simulate sudo without inherited KOPI_HOME (falls back to root).
        monkeypatch.setenv("KOPI_HOME", str(root_kopi))
        ...
        assert os.environ["KOPI_HOME"] == str(alice_kopi)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned KOPI_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_kopi_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_kopi_home_from_systemd_unit",
