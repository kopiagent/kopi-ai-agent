...
class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(kopi_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return kopi_root / f".npm_lock_hash_{cache_key}"
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `kopi update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".kopi"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(kopi_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            kopi_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("KOPI_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("KOPI_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_already_up_to_date(
        ...
        with patch("kopi_cli.managed_uv.update_managed_uv") as mock_uv_update, \
             patch(
                 "kopi_cli.managed_uv.ensure_uv",
                 return_value=None,
             ) as mock_uv_ensure:
            cmd_update(mock_args)
    ...
    def test_zero_commit_runtime_repair_requires_process_restart(
        ...
        with patch(
            "kopi_cli.managed_uv.update_managed_uv",
            side_effect=fake_update,
        ), patch(
            "kopi_cli.managed_uv.ensure_uv",
            return_value="/managed/uv",
        ), patch(
            "kopi_cli.main._is_windows",
            return_value=False,
        ):
            cmd_update(mock_args)
