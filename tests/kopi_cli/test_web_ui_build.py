...
(mirroring the desktop build), recorded in a stamp file under $KOPI_HOME,
NOT mtime comparison — so ``git pull`` / ``kopi update`` that rewrite
...
def _isolated_kopi_home(tmp_path, monkeypatch):
    """Keep web-build-stamp writes inside the test's tmp dir, never the real home."""
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "_kopi_home"))
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        ...
        wrong.write_text("{}")
        # Correct location (kopi_cli/web_dist/) is empty -> still needs build.
        assert _web_ui_build_needed(web_dir) is True
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``kopi update`` do) must NOT report stale."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        src = web_dir / "src" / "App.tsx"
        src.parent.mkdir(parents=True, exist_ok=True)
        src.write_text("export const A = 1\n")
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_npm_install_fallback_forces_include_dev_and_no_save(self, tmp_path):
        ...
        with patch("kopi_cli.main.subprocess.run", side_effect=[ci_fail, install_ok]) as mock_run:
            result = _run_npm_install_deterministic("/usr/bin/npm", web_dir)
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_with_dist_serves_stale_without_building(self, tmp_path):
        import fcntl
        from kopi_cli.main import _build_web_ui as build
        ...
        try:
            ...
            with patch("kopi_cli.main._do_build_web_ui") as mock_do:
    ...
    def test_uncontended_lock_builds_and_creates_lock_file(self, tmp_path):
        ...
        web_dir, _ = _make_web_dir(tmp_path)
        with patch("kopi_cli.main._do_build_web_ui", return_value=True) as mock_do:
            result = build(web_dir)
...
class TestBuildRecoversFromMissingToolchain:
    def test_reinstalls_and_retries_when_the_build_cannot_resolve_tsc(self, tmp_path):
        ...
        with patch("kopi_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             ...
             patch("kopi_cli.main._run_with_idle_timeout", side_effect=[build_fail, build_ok]) as mock_build, \
             patch("kopi_cli.main._web_ui_build_needed", return_value=True), \
             patch("kopi_cli.main._write_web_ui_build_stamp"), \
             patch("kopi_cli.main._time.sleep"):
            result = _build_web_ui(web_dir)
    ...
    def test_healthy_tree_builds_without_an_extra_install(self, tmp_path):
        ...
        with patch("kopi_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("kopi_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("kopi_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_build, \
             patch("kopi_cli.main._web_ui_build_needed", return_value=True), \
             patch("kopi_cli.main._write_web_ui_build_stamp"):
            result = _build_web_ui(web_dir)
    ...
    def test_unrelated_build_failure_takes_the_generic_retry_only(self, tmp_path):
        ...
        with patch("kopi_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("kopi_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("kopi_cli.main._run_with_idle_timeout", side_effect=[type_error, build_ok]), \
             patch("kopi_cli.main._web_ui_build_needed", return_value=True), \
             patch("kopi_cli.main._write_web_ui_build_stamp"), \
             patch("kopi_cli.main._time.sleep"):
            result = _build_web_ui(web_dir)
