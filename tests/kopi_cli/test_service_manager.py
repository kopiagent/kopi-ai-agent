...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "s6-setuidgid kopi" in run_text
...
def test_s6_log_run_creates_leaf_as_kopi_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as kopi.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    kopi). Restartable log/run must not pathname-chown or pathname-rm a
    kopi-writable tree from root — that is a symlink TOCTOU hole.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on kopi-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid kopi mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid kopi rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_kopi_idx = log_text.index('s6-setuidgid kopi mkdir -p "$log_dir"')
    rm_as_kopi_idx = log_text.index('s6-setuidgid kopi rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_kopi_idx < rm_as_kopi_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = kopi_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside KOPI_HOME the way
    # a real kopi uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'KOPI_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that kopi cannot unlink a foreign
...
def test_s6_log_run_mkdir_as_kopi_on_real_dirs(tmp_path) -> None:
    """Root-context setup creates ``$log_dir`` via ``s6-setuidgid kopi mkdir``."""
    import os
    import stat
    import subprocess
    ...
    kopi_home = tmp_path / "kopi"
    (kopi_home / "logs" / "gateways").mkdir(parents=True)
    ...
    assert any(
        c.split()[:3] == ["kopi", "mkdir", "-p"] and "gateways/coder" in c
        for c in setuid_calls
    ), setuid_calls
    assert any(
        c.split()[:3] == ["kopi", "rm", "-f"] and c.rstrip().endswith("/lock")
        for c in setuid_calls
    ), setuid_calls
    assert (kopi_home / "logs" / "gateways" / "coder").is_dir()
    assert (
