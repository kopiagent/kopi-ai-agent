...
def test_explicit_only_keeps_moa_when_raw_config_has_enabled_preset():
    ...
    with (
        _list_auth_returning(rows),
        patch("kopi_cli.config.load_config", return_value=raw_config),
        patch("kopi_cli.config.read_raw_config", return_value=raw_config),
        patch("kopi_cli.auth.is_provider_explicitly_configured", return_value=False),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_apply_featured_keeps_newest_n_per_lab():
    ...
    cut."""
    from kopi_cli.inventory import _FEATURED_PER_LAB
