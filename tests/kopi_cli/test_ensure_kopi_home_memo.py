"""ensure_kopi_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".kopi"
    second = tmp_path / "b" / ".kopi"

    monkeypatch.setenv("KOPI_HOME", str(first))
    cfg.ensure_kopi_home()

    # Profile switch: KOPI_HOME moves → the new path is ensured too.
    monkeypatch.setenv("KOPI_HOME", str(second))
    cfg.ensure_kopi_home()
