"""Tests for the Speech-to-Text category in `kopi tools` (tools_config).
...
from kopi_cli.tools_config import (  # noqa: E402
...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from kopi_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY
...
class TestModelPicker:
    ...
    def test_configure_stt_model_writes_model(self):
        config = {}
        with patch("kopi_cli.tools_config._prompt_choice", return_value=3):
    ...
    def test_configure_stt_model_skips_uncataloged_provider(self):
        config = {}
        with patch("kopi_cli.tools_config._prompt_choice") as pc:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "kopi_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
            _configure_stt_model("openai", config)
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from kopi_cli.tools_config import _POST_SETUP_READY
