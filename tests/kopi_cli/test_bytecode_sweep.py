...
Bug class: the checkout's ``.py`` files change (``kopi update``, manual
...
def _make_pycache(repo: Path, subdir: str = "kopi_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(kopi_main, "PROJECT_ROOT", repo)
    # Stamp records a different (older) fingerprint.
    (repo / kopi_main._BYTECODE_FINGERPRINT_FILE).write_text(
        "git:refs/heads/main:" + "a" * 40, encoding="utf-8"
    )

    kopi_main._sweep_stale_bytecode_if_checkout_changed()

    assert not cache.exists()
    # Stamp updated to the current fingerprint.
    recorded = (repo / kopi_main._BYTECODE_FINGERPRINT_FILE).read_text(encoding="utf-8")
...
def test_sweep_noop_when_fingerprint_matches(monkeypatch, tmp_path):
    ...
    cache = _make_pycache(repo)
    monkeypatch.setattr(kopi_main, "PROJECT_ROOT", repo)
    fingerprint = kopi_main._read_git_revision_fingerprint(repo)
    assert fingerprint is not None
    (repo / kopi_main._BYTECODE_FINGERPRINT_FILE).write_text(fingerprint, encoding="utf-8")
...
def test_sweep_first_launch_clears_and_records(monkeypatch, tmp_path):
    ...
    assert not cache.exists()
    assert (repo / kopi_main._BYTECODE_FINGERPRINT_FILE).exists()
...
def test_sweep_noop_on_non_git_install(monkeypatch, tmp_path):
    ...
    assert cache.exists()
    assert not (repo / kopi_main._BYTECODE_FINGERPRINT_FILE).exists()
...
def test_sweep_never_raises_on_unreadable_stamp(monkeypatch, tmp_path):
    ...
    cache = _make_pycache(repo)
    monkeypatch.setattr(kopi_main, "PROJECT_ROOT", repo)
    stamp = repo / kopi_main._BYTECODE_FINGERPRINT_FILE
    stamp.mkdir()  # a directory where a file is expected → OSError on read

    kopi_main._sweep_stale_bytecode_if_checkout_changed()  # must not raise
...
def test_record_bytecode_fingerprint_writes_atomically(monkeypatch, tmp_path):
    ...
    kopi_main._record_bytecode_fingerprint()
...
def test_record_bytecode_fingerprint_noop_without_git(monkeypatch, tmp_path):
    ...
    kopi_main._record_bytecode_fingerprint()  # must not raise
...
def test_sweep_skips_venv_and_git_dirs(monkeypatch, tmp_path):
    ...
    repo_cache = _make_pycache(repo, "kopi_cli")
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.kopi/plugins/<name>
# ---------------------------------------------------------------------------
