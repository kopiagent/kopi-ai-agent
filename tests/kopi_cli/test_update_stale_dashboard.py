...
class TestKillStaleDashboardPosix:
    ...
    def test_update_path_restarts_managed_dashboard_instead_of_killing(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "kopi-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            if args[:2] == ["systemctl", "list-unit-files"]:
                return MagicMock(returncode=0, stdout="kopi-dashboard.service enabled enabled\n", stderr="")
            if args[:2] == ["systemctl", "is-active"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args[:2] == ["systemctl", "is-enabled"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "restart", "kopi-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("kopi_cli.main._find_stale_dashboard_pids",
                   return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert ["systemctl", "restart", "kopi-dashboard.service"] in calls
        find_pids.assert_not_called()
        kill.assert_not_called()

        out = capsys.readouterr().out
        assert "Restarting managed dashboard service" in out
        assert "✓ restarted kopi-dashboard.service" in out
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "kopi-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="kopi-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "kopi-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "kopi-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "kopi-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("kopi_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "kopi-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "kopi-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "kopi-dashboard.service"],
            ["systemctl", "--user", "restart", "kopi-dashboard.service"],
        ]
        assert all(call[:1] != ["sudo"] and call[:2] != ["systemctl"] for call in calls)
        find_pids.assert_not_called()
        kill.assert_not_called()
        assert "✓ restarted kopi-dashboard.service" in capsys.readouterr().out
    ...
    def test_user_scope_restart_failure_does_not_try_system_or_sudo(self):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "kopi-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="kopi-dashboard.service enabled enabled\n", stderr="")
            if args[-2:] == ["is-active", "kopi-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args[-2:] == ["is-enabled", "kopi-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args[-2:] == ["restart", "kopi-dashboard.service"]:
                return MagicMock(returncode=1, stdout="", stderr="user manager unavailable")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("kopi_cli.main._find_stale_dashboard_pids") as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls[-1] == ["systemctl", "--user", "restart", "kopi-dashboard.service"]
        assert not any(call[:1] == ["sudo"] or call == ["systemctl", "restart", "kopi-dashboard.service"] for call in calls)
        find_pids.assert_not_called()
        kill.assert_not_called()
    ...
    def test_managed_dashboard_restart_failure_does_not_raw_kill(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["sudo", "-n", "systemctl", "restart", "kopi-dashboard.service"]:
        ...
        out = capsys.readouterr().out
        assert "failed to restart kopi-dashboard.service" in out
        assert "not raw-killing its PID" in out
        assert "sudo systemctl restart kopi-dashboard.service" in out
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "kopi_cli.main._kill_stale_dashboard_processes",
    ...
    def test_unrecovered_stop_prints_shared_update_notice(self, capsys):
        ...
        assert "kopi dashboard --port <port>" in out
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["kopi_cli.main"]
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(live, "_get_pid_cgroup_path",
                          return_value="/system.slice/kopi-serve.service"), \
             patch.object(live, "_get_systemd_service_for_pid",
                          return_value="kopi-serve.service"), \
        ...
        restart.assert_called_once_with(
            "kopi-serve.service", "/system.slice/kopi-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service kopi-serve.service" in out
    ...
    def test_supervised_restart_failure_prints_hint(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "⚠ kopi-serve.service" in out
    ...
    def test_stop_path_never_restarts(self, capsys):
        """`kopi dashboard --stop` (restart_managed=False) must stay a stop:
...
class TestManualBackendRespawn:
    ...
    def test_manual_pid_respawned_with_captured_argv(self, capsys):
        live = self._live()
        argv = ["/usr/bin/python3", "-m", "kopi_cli.main", "serve", "--port", "8300"]
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["kopi", "dashboard", "--port", "8300"],
                ["kopi", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["kopi", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["kopi", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["kopi", "serve"]])

        assert failed == [["kopi", "serve"]]
        out = capsys.readouterr().out
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00kopi_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "kopi_cli.main", "serve"]
