...
import kopi_state
from kopi_state import FTS_STORAGE_VERSION, SCHEMA_VERSION, SessionDB
from kopi_cli import session_recovery
...
def test_snapshot_blocks_connections_opened_during_the_copy(
    ...
    from kopi_cli import session_recovery as recovery_module
...
def test_recovery_refuses_to_snapshot_a_live_database(tmp_path: Path) -> None:
    ...
    kopi_cli.sqlite_safe_read). Recovery normally runs against an offline
    file, but the check must exist so this path cannot drift away from
    kopi_state._backup_db_file, which refuses the same situation.
    """
    from kopi_cli.sqlite_safe_read import connect_tracked
...
def test_recovery_refuses_before_writing_when_disk_space_is_short(
    ...
    with pytest.raises(SessionRecoverySafetyError, match="Not enough free disk space"):
        recover_session_database(source, output, work_dir=tmp_path)
    assert not output.exists()
    assert not list(tmp_path.glob("kopi-session-recovery-*"))
...
def test_cli_allow_partial_salvages_rows_across_a_corrupt_leaf(
    ...
    env = os.environ.copy()
    env["KOPI_HOME"] = str(tmp_path / "isolated-kopi-home")
...
def test_failed_repair_points_the_user_at_offline_recovery(tmp_path: Path) -> None:
    ...
    This is the reported dead end: `kopi sessions repair` exhausts its
    ...
    """
    kopi_home = tmp_path / "isolated-kopi-home"
    kopi_home.mkdir()
    # Not a database at all -> every in-place repair strategy fails.
    (kopi_home / "state.db").write_bytes(b"SQLite format 3\x00" + b"\x7f" * 2048)
    env = os.environ.copy()
    env["KOPI_HOME"] = str(kopi_home)

    result = subprocess.run(
        [sys.executable, "-m", "kopi_cli.main", "sessions", "repair"],
        cwd=Path(__file__).resolve().parents[2],
        env=env,
        capture_output=True,
        text=True,
