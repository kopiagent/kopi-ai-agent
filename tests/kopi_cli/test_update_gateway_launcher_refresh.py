...
2. ``kopi_cli.main._refresh_windows_gateway_launchers`` — ``kopi
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "kopi_cli.main", "gateway", "run"]
    ...
    ), mock.patch("kopi_cli.config.get_kopi_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: kopi update regenerates launchers
# ---------------------------------------------------------------------------
