...
Every ``CommandDef`` with ``execute`` set must:
  * name a key that exists in :data:`kopi_cli.slash_exec.EXECUTORS`, and
...
from kopi_cli.commands import COMMAND_REGISTRY, resolve_command
from kopi_cli.slash_exec import (
    EXECUTORS,
    CommandContext,
    CommandReply,
    execute_command,
...
def test_every_execute_key_resolves():
    for cmd in MIGRATED:
        assert cmd.execute in EXECUTORS, (
            ...
            f"kopi_cli.slash_exec.EXECUTORS"
        )
...
def test_profile_options_override_process_values():
    reply = run_execute(
        ...
        CommandContext(options={"profile_name": "work", "home_display": "~/.kopi-work"}),
    )
    assert reply.data == {"profile": "work", "home": "~/.kopi-work"}
