...
Runs a real subprocess with a temp KOPI_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (KOPI_MANAGED_DIR).
...
Subprocess (not in-process monkeypatching) so module-level ``_kopi_home``
globals, managed-scope caches, and ``_under_pytest`` guards behave like
production: ``KOPI_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._kopi_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        ...
        {
            "KOPI_HOME": str(home),
            "KOPI_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['KOPI_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"KOPI_HOME": str(home)}, tmp_path)
