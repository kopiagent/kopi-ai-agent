"""Tests for `kopi memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from kopi_cli.memory_setup import cmd_status
    ...
    with patch("kopi_cli.config.load_config", return_value=config):
        with patch("kopi_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
