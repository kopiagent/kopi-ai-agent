...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("kopi_cli.config.sanitize_env_file", return_value=2),
            patch(
                "kopi_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("kopi_cli.config.read_raw_config", return_value={}),
            patch("kopi_cli.config.get_missing_env_vars", return_value=[]),
            patch("kopi_cli.config.get_missing_config_fields", return_value=[]),
            patch("kopi_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `kopi setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_kopi_home(self):
        from kopi_cli.config_migrations import support_floor_message
        from kopi_constants import display_kopi_home

        msg = support_floor_message()
        assert f"{display_kopi_home()}/config.yaml" in msg
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from kopi_cli.config import _persist_migration, read_raw_config
