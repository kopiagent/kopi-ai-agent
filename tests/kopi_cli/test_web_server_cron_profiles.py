...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_kopi_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_kopi_home()
    ...
    outer_token = set_kopi_home_override(default_home)
    try:
        ...
        assert scheduler._get_kopi_home() == default_home
    finally:
        reset_kopi_home_override(outer_token)
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.kopi`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from kopi_cli import web_server

    monkeypatch.setenv(
        "KOPI_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """KOPI_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from kopi_cli import web_server

    monkeypatch.setenv("KOPI_HOME", str(isolated_profiles["default"]))
