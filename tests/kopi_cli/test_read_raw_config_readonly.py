...
@pytest.fixture()
def isolated_kopi_home():
    """Per-test KOPI_HOME dir (already redirected by the autouse conftest
...
def test_identity_invariant_from_first_call(isolated_kopi_home):
    from kopi_cli.config import read_raw_config_readonly

    _write_config(isolated_kopi_home, {"telemetry": {"shared_metrics": {"enabled": True}}})
...
def test_parity_with_mutable_read(isolated_kopi_home):
    from kopi_cli.config import read_raw_config, read_raw_config_readonly

    _write_config(isolated_kopi_home, {"gateway": {"max_inbound_media_bytes": 123}})
    assert read_raw_config_readonly() == read_raw_config()


def test_freshness_after_config_edit(isolated_kopi_home):
    from kopi_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_kopi_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_kopi_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_kopi_home):
    from kopi_cli.config import read_raw_config_readonly

    cfg = isolated_kopi_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
    assert read_raw_config_readonly() == {}


def test_mutable_variant_still_isolated(isolated_kopi_home):
    """read_raw_config() callers may mutate; that must not corrupt the
    readonly cache entry."""
    from kopi_cli.config import read_raw_config, read_raw_config_readonly

    _write_config(isolated_kopi_home, {"a": {"b": 1}})
