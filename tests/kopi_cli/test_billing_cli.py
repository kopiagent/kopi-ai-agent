...
def test_billing_overview_non_interactive_renders_text_not_modal(cli, monkeypatch, capsys):
    monkeypatch.setattr(KopiCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
    state = BillingState(
        logged_in=True,
        org_name="Acme",
        role="OWNER",
...
def test_billing_killswitch_off_blocks(cli, monkeypatch, capsys):
    ...
    assert (
        "A billing admin can turn it on from the portal's Kopi Agent page "
        "to add funds here."
    ) in out
...
def test_buy_flow_no_card_guides_then_continues_after_recheck(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(KopiCLI, "_prompt_text_input_modal", _scripted("recheck", "cancel"), raising=False)
