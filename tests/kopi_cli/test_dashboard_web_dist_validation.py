"""Regression tests: `kopi dashboard` validates KOPI_WEB_DIST before serving.

A custom KOPI_WEB_DIST without --skip-build previously skipped BOTH the
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """KOPI_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("KOPI_WEB_DIST", str(empty_dist))
    ...
    assert "KOPI_WEB_DIST" in out and str(empty_dist) in out


def test_env_dist_with_index_starts_server(main_mod, monkeypatch, tmp_path):
    """A valid KOPI_WEB_DIST (has index.html) proceeds to start_server
    without building."""
    _wire_common(main_mod, monkeypatch)
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<html></html>", encoding="utf-8")
    monkeypatch.setenv("KOPI_WEB_DIST", str(dist))
...
def test_env_dist_tilde_expanded_for_web_server(main_mod, monkeypatch, tmp_path):
    """A '~/...' KOPI_WEB_DIST must be written back expanded so
    ...
    monkeypatch.setenv("KOPI_WEB_DIST", "~/mydist")
    ...
    import os
    assert os.environ["KOPI_WEB_DIST"] == str(dist)
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    dist = project_root / "kopi_cli" / "web_dist"
    dist.mkdir(parents=True)
...
def test_skip_build_recovery_build_failure_preserves_fatal_exit(
    ...
    monkeypatch.delenv("KOPI_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    (project_root / "kopi_cli" / "web_dist").mkdir(parents=True)
    monkeypatch.setattr(main_mod, "PROJECT_ROOT", project_root)
...
def test_skip_build_custom_env_dist_missing_does_not_attempt_recovery(
    ...
    """A custom KOPI_WEB_DIST is caller-managed: the recovery build writes
...
@pytest.mark.parametrize(
    "path,expected",
    [
        ("/Applications/Kopi.app/Contents/Resources/app.asar/dist", True),
        ("/Applications/Kopi.app/Contents/Resources/app.asar.unpacked/dist", True),
        (r"C:\Users\u\AppData\Local\Programs\Kopi\resources\app.asar\dist", True),
...
def test_standalone_dashboard_drops_electron_packaged_web_dist(
    ...
    packaged = "/Applications/Kopi.app/Contents/Resources/app.asar/dist"
    monkeypatch.setenv("KOPI_WEB_DIST", packaged)
    ...
    assert "KOPI_WEB_DIST" not in os.environ
    assert len(builds) == 1
    assert len(started) == 1
...
def test_standalone_dashboard_keeps_caller_managed_web_dist(
    ...
    """A non-Electron custom KOPI_WEB_DIST override must survive."""
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("KOPI_DESKTOP", raising=False)
...
def test_desktop_spawned_backend_keeps_electron_web_dist(
    ...
    """KOPI_DESKTOP=1 legitimately points at the packaged dist — do not strip."""
    _wire_common(main_mod, monkeypatch)
    ...
    monkeypatch.setenv("KOPI_WEB_DIST", str(packaged_root))
    ...
    assert os.environ["KOPI_WEB_DIST"] == str(packaged_root)
    assert builds == []
    assert len(started) == 1
...
def test_standalone_dashboard_clears_inherited_serve_headless(
    ...
    """Inherited KOPI_SERVE_HEADLESS must not disable the SPA for dashboard."""
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("KOPI_DESKTOP", raising=False)
    monkeypatch.delenv("KOPI_WEB_DIST", raising=False)
    ...
    assert os.environ.get("KOPI_SERVE_HEADLESS") != "1"
    assert len(started) == 1


def test_headless_serve_reasserts_serve_headless(main_mod, monkeypatch):
    """`kopi serve` must still set KOPI_SERVE_HEADLESS after the clear."""
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("KOPI_DESKTOP", raising=False)
    monkeypatch.delenv("KOPI_WEB_DIST", raising=False)
