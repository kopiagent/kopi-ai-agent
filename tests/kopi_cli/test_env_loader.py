...
def test_utf16_le_bom_env_loads_and_rewrites_clean_utf8(tmp_path, monkeypatch):
    ...
    home.mkdir()
    env_file = home / ".env"
    content = "KOPI_TEST_KEY=hello_utf16\nSECOND_KEY=world\n"
    env_file.write_bytes(codecs.BOM_UTF16_LE + content.encode("utf-16-le"))

    monkeypatch.delenv("KOPI_TEST_KEY", raising=False)
    ...
    assert loaded == [env_file]
    assert os.getenv("KOPI_TEST_KEY") == "hello_utf16"
    assert os.getenv("SECOND_KEY") == "world"
    assert os.environ.get("\ufffd\ufffdKOPI_TEST_KEY") is None
    _assert_clean_utf8_env_on_disk(env_file, first_key="KOPI_TEST_KEY")
...
def test_leading_replacement_char_does_not_rewrite(tmp_path):
    ...
    env_file = tmp_path / ".env"
    raw = b"\xffKOPI_TEST_KEY=should-not-rewrite\nSECOND_KEY=ok\n"
    env_file.write_bytes(raw)
