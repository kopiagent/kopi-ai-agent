...
KOPI_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
def kopi_home(monkeypatch, tmp_path):
    """Fresh KOPI_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("KOPI_HOME", str(home))
    from kopi_cli.config import invalidate_env_cache
...
def test_delete_env_key_prunes_env_seeded_pool_entry(kopi_home):
    _write_env(kopi_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(kopi_home, _zai_pool_fixture())
...
def test_delete_env_key_removes_provider_pool_key_when_emptied(kopi_home):
...
def test_delete_survives_pool_reload(kopi_home):
    """#59761: the pool loader must not resurrect the entry after 'restart'."""
    _write_env(kopi_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(kopi_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
...
def test_delete_clears_provider_models_cache(kopi_home):
    _write_env(kopi_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(kopi_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = kopi_home / "provider_models_cache.json"
...
def test_delete_unknown_key_404s(kopi_home):
...
def test_delete_does_not_touch_other_providers(kopi_home):
    _write_env(kopi_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    other_key = "dk-" + "e" * 24
...
def test_update_rotates_config_yaml_model_mirror(kopi_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(kopi_home, OPENAI_API_KEY=old)
    _write_config(
        kopi_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = kopi_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
def test_update_rotates_custom_provider_mirror(kopi_home):
    old = "sk-cp-" + "h" * 24
    new = "sk-cp-" + "i" * 24
    _write_env(kopi_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_update_leaves_unrelated_config_keys_alone(kopi_home):
    """A DIFFERENT key configured inline must not be rewritten by value-match."""
    old = "sk-un-" + "j" * 24
    unrelated = "sk-un-" + "k" * 24
    _write_env(kopi_home, OPENAI_API_KEY=old)
    _write_config(kopi_home, f"model:\n  provider: custom\n  api_key: {unrelated}\n")
...
def test_delete_scrubs_config_yaml_mirror(kopi_home):
    old = "sk-dl-" + "m" * 24
    _write_env(kopi_home, OPENAI_API_KEY=old)
    _write_config(kopi_home, f"model:\n  provider: custom\n  api_key: {old}\n")
...
def test_delete_then_resave_round_trip(kopi_home):
    _write_env(kopi_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(kopi_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    ...
    assert not is_source_suppressed("zai", "env:ZAI_API_KEY"), (
        "an explicit re-save must lift the suppression (like `kopi auth add`)"
    )
