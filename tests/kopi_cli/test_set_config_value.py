...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_resolved_nested_value(self, _isolated_kopi_home, capsys):
    ...
    def test_config_unset_removes_yaml_key_and_synced_env(self, _isolated_kopi_home, capsys):
        set_config_value("terminal.backend", "docker")
        assert "TERMINAL_ENV=docker" in _read_env(_isolated_kopi_home)
        capsys.readouterr()
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_kopi_home)) or {}
        assert reloaded == {}
        assert "TERMINAL_ENV=" not in _read_env(_isolated_kopi_home)
        assert "Unset terminal.backend" in capsys.readouterr().out


    def test_config_unset_removes_dotted_token_yaml_key(self, _isolated_kopi_home, capsys):
        (_isolated_kopi_home / "config.yaml").write_text(
            "platforms:\n"
            "  teams:\n"
            "    extra:\n"
...
class TestStringTypedConfigValues:
    ...
    def test_string_typed_values_are_not_coerced(self, _isolated_kopi_home, value):
        """Values stay strings when DEFAULT_CONFIG declares the leaf as a string."""
        set_config_value("approvals.mode", value)

        import yaml
        saved = yaml.safe_load(_read_config(_isolated_kopi_home))
    ...
    def test_non_string_defaults_keep_existing_coercion(
        self, _isolated_kopi_home, key, value, expected
    ):
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_kopi_home):
...
class TestSchemaValidation:
    """#34067: ``kopi config set`` must not report bare success for
    ...
    a post-write notice warns that Kopi may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_kopi_home, capsys):
        """The documented TCC signing identity setting is part of the schema."""
        set_config_value("desktop.macos_signing_identity", "Kopi Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_kopi_home))
        assert saved["desktop"]["macos_signing_identity"] == "Kopi Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out



    def test_force_suppresses_notice(self, _isolated_kopi_home, capsys):
        ...
        content = _read_config(_isolated_kopi_home)
...
class TestValidateConfigKey:
    ...
    def test_known_keys_pass(self, key):
        from kopi_cli.config import _validate_config_key
...
class TestDisplaySkinTouch:
    ...
    (`kopi config set display.skin X` while it is already X — the recovery
    ...
    def test_reaffirming_same_skin_moves_the_watcher_signature(self, _isolated_kopi_home):
        import os as _os
        skins = _isolated_kopi_home / "skins"
        skins.mkdir()
    ...
    def test_builtin_or_missing_skin_file_is_fine(self, _isolated_kopi_home):
        """Built-ins have no user file — the set must still succeed cleanly."""
        set_config_value("display.skin", "mono")
        assert "skin: mono" in _read_config(_isolated_kopi_home)

    def test_touch_preserves_skin_file_contents(self, _isolated_kopi_home):
        skins = _isolated_kopi_home / "skins"
        skins.mkdir()
        body = "name: neon\ncolors:\n  ui_accent: '#ff33aa'\n"
...
class TestMappingGuard:
    """``kopi config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_kopi_home):
        """kopi config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_kopi_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
        ...
        config_text = _read_config(_isolated_kopi_home)
        import yaml as _yaml
    ...
    def test_bare_model_shorthand_creates_default_when_none(self, _isolated_kopi_home):
        """Bare model shorthand still works when config is empty (legacy behaviour)."""
        set_config_value("model", "gpt-5.6-sol")
        assert "gpt-5.6-sol" in _read_config(_isolated_kopi_home)

    def test_non_model_mapping_is_refused(self, _isolated_kopi_home):
        """kopi config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_kopi_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_kopi_home):
        """kopi config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_kopi_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
        ...
        import yaml as _yaml
        parsed = _yaml.safe_load(_read_config(_isolated_kopi_home))
        assert parsed["terminal"] == "zsh"

    def test_model_default_dotted_path_is_not_guarded(self, _isolated_kopi_home):
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_kopi_home):
        """kopi config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_kopi_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
...
class TestScalarModelSubKeyPreservation:
    ...
    def test_scalar_model_id_preserved_after_provider_write(self, _isolated_kopi_home):
        ...
        raw = _read_config(_isolated_kopi_home)
        parsed = yaml.safe_load(raw)
        model = parsed["model"]
        assert model["default"] == "gpt-4o", f"model.default lost: {model}"
        assert model["provider"] == "openai"

    def test_scalar_model_id_preserved_after_api_key_write(self, _isolated_kopi_home):
...
class TestMalformedYAMLConfigPreservation:
    ...
    def test_set_config_value_refuses_broken_yaml(self, _isolated_kopi_home, capsys):
        """set_config_value must exit with error, not overwrite the broken config."""
        self._write_broken_config(_isolated_kopi_home)
    ...
    def test_unset_config_value_refuses_broken_yaml(self, _isolated_kopi_home, capsys):
        """unset_config_value must exit with error, not overwrite the broken config."""
        from kopi_cli.config import unset_config_value
