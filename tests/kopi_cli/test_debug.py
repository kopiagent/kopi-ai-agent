...
class TestMissingLogNote:
    ...
    `kopi debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, kopi_home):
        from kopi_cli.debug import _capture_log_snapshot

        (kopi_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, kopi_home):
        from kopi_cli.debug import _capture_log_snapshot
        ...
        assert "Kopi Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(kopi_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, kopi_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, kopi_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from kopi_cli.debug import _capture_log_snapshot

        (kopi_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, kopi_home):
...
class TestDebugSlashCommand:
    ...
    ``KopiCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
