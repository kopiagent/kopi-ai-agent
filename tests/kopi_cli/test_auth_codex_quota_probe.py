...
reset).  Kopi must detect that and lift the stale local cooldown instead
...
def _write_auth_store(kopi_home, payload):
...
def test_clear_cooldowns_only_touches_quota_shaped_entries(tmp_path, monkeypatch):
    ...
    _write_auth_store(kopi_home, _exhausted_pool_store())
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))

    assert clear_codex_pool_quota_cooldowns() == 1

    store = json.loads((kopi_home / "auth.json").read_text())
...
def test_clear_cooldowns_scoped_to_access_token(tmp_path, monkeypatch):
    ...
    kopi_home = tmp_path / "kopi"
    _write_auth_store(kopi_home, store)
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))

    assert clear_codex_pool_quota_cooldowns("tok-other") == 1

    persisted = json.loads((kopi_home / "auth.json").read_text())
...
def test_clear_cooldowns_noop_without_pool(tmp_path, monkeypatch):
    kopi_home = tmp_path / "kopi"
    _write_auth_store(kopi_home, {"version": 1, "providers": {}})
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))
...
def test_resolver_recovers_when_probe_confirms_reset(tmp_path, monkeypatch):
    ...
    kopi_home = tmp_path / "kopi"
    _write_auth_store(kopi_home, _pool_only_rate_limited_store())
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))
...
def test_pool_entry_recovers_when_probe_confirms_reset(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path / "kopi", _pool_only_rate_limited_store())
...
def test_pool_probe_not_fired_for_non_quota_exhaustion(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "kopi"))
    _write_auth_store(tmp_path / "kopi", store)
