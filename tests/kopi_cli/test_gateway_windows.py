...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Kopi_Gateway"])
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    assert argv[:3] == [str(venv_python), "-m", "kopi_cli.main"]
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Kopi_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Kopi_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Kopi_Gateway_alice")
...
def test_gateway_cmd_script_uses_console_python_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe, r"C:\\Kopi\\kopi-ai-agent\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Kopi\\kopi-ai-agent\\venv\\Scripts\\python.exe",
        r"C:\\Kopi\\kopi-ai-agent",
        r"C:\\KopiHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Kopi\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Kopi\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Kopi_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Kopi_Gateway_alice")
    ...
    assert "Kopi_Gateway_alice.vbs" in xml_seen["text"]
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Kopi_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Kopi gateway install prompt" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Kopi gateway uninstall prompt" in out
