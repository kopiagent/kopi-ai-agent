...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/kopi-ai-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    """Regression: `kopi tools` → Configure all platforms → Web Search
    ...
    monkeypatch.setattr("kopi_cli.tools_config._prompt_choice", choose_by_label)
    monkeypatch.setattr(
        "kopi_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web"},
    )
    monkeypatch.setattr(
        "kopi_cli.tools_config._toolset_needs_configuration_prompt",
...
def test_configure_single_platform_configures_selected_tool_missing_provider(monkeypatch):
    """Regression (per-platform sibling of the global flow): `kopi tools` →
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/kopi-ai-agent/issues/6901
    """
    config = {
...
def test_computer_use_post_setup_respects_custom_driver_command_when_installed():
    ...
    # bare-command miss after KOPI_CUA_DRIVER_CMD was looked up via which().
...
def test_provider_readiness_env_vars_gate_keys(monkeypatch):
    ...
    monkeypatch.setattr("kopi_cli.tools_config.get_env_value", lambda key: None)
    assert provider_readiness_status(provider, {}) == "needs_keys"

    monkeypatch.setattr("kopi_cli.tools_config.get_env_value", lambda key: "sk-x")
...
def test_provider_readiness_local_install_rows_track_module_presence(monkeypatch):
    ...
    monkeypatch.setattr(
        "kopi_cli.tools_config._module_installed", lambda name: False
    )
    assert provider_readiness_status(kitten, {}) == "needs_setup"
    assert provider_readiness_status(piper, {}) == "needs_setup"

    monkeypatch.setattr(
        "kopi_cli.tools_config._module_installed", lambda name: True
    )
...
# `kopi tools post-setup <key>` child. On Windows each console child (npm,
...
def test_post_setup_no_window_flags_zero_on_posix(monkeypatch):
    ...
    from kopi_cli.tools_config import _post_setup_no_window_flags
...
def test_provider_readiness_agent_browser_tracks_local_install(monkeypatch):
    ...
    monkeypatch.setattr(
        "kopi_cli.nous_subscription._local_browser_runnable", lambda: False
    )
    assert provider_readiness_status(provider, {}) == "needs_setup"

    monkeypatch.setattr(
        "kopi_cli.nous_subscription._local_browser_runnable", lambda: True
    )
    assert provider_readiness_status(provider, {}) == "ready"
...
def test_provider_readiness_cloud_browser_hook_tracks_cli_only(monkeypatch):
    ...
    monkeypatch.setattr(
        "kopi_cli.nous_subscription._has_agent_browser", lambda: False
    )
    assert provider_readiness_status(provider, {}) == "needs_setup"

    monkeypatch.setattr(
        "kopi_cli.nous_subscription._has_agent_browser", lambda: True
    )
    assert provider_readiness_status(provider, {}) == "ready"
