...
from kopi_cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
_EXTRA_ENV_VARS = (
    ...
    "KOPI_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
...
class TestHasAnyProviderConfigured:
    ...
    def test_config_provider_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        """
        import yaml
        kopi_home = self._setup_home(monkeypatch, tmp_path)
        ...
        monkeypatch.setattr("kopi_cli.auth.get_auth_status", _trap)
        from kopi_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert sweep_calls == [], (
    ...
    def test_auth_json_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("kopi_cli.auth.get_auth_status", _guarded_status)
        from kopi_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert calls == ["nous"], (
...
class TestZaiParallelProbe:
    ...
    def test_candidate_model_fallback_within_endpoint(self, monkeypatch):
        ...
        from kopi_cli.auth import ZAI_ENDPOINTS, detect_zai_endpoint
    ...
    def test_priority_order_wins_over_completion_order(self, monkeypatch):
        ...
        monkeypatch.setattr("kopi_cli.auth.httpx.post", _slow_first)
        result = detect_zai_endpoint("test-key", timeout=1.0)
        assert result is not None
        assert result["id"] == first[0]

    def test_all_fail_returns_none(self, monkeypatch):
        from kopi_cli.auth import detect_zai_endpoint

        monkeypatch.setattr("kopi_cli.auth.httpx.post", self._mock_post({}))
    ...
    def test_early_exit_does_not_wait_for_slow_losers(self, monkeypatch):
        ...
        monkeypatch.setattr("kopi_cli.auth.httpx.post", _slow_losers)
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        import kopi_cli.models as models
        monkeypatch.setattr(
            models, "_urlopen_model_catalog_request", lambda *a, **kw: _Resp()
        )
        from kopi_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from kopi_cli.models import _fetch_deepinfra_models_by_tag
        import kopi_cli.models as _m
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from kopi_cli.auth import PROVIDER_REGISTRY, resolve_provider
