...
class TestWebServerEndpoints:
    ...
    def test_get_status_profile_scopes_gateway_state_reads(self, monkeypatch):
        ...
        home (~/.kopi/profiles/<name>/), not the process-level KOPI_HOME.

        Gateway PID/state readers resolve _get_process_kopi_home(), which
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        KOPI_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        monkeypatch.setattr(
            ...
            lambda specs, *, timeout=300: ld.InstallSpecsResult(
                ...
                reason=(
                    ...
                    "target is configured (KOPI_LAZY_INSTALL_TARGET)"
                ),
            ),
        )
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide KOPI_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from kopi_constants import get_kopi_home

        path = get_kopi_home() / "honcho.json"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert data["docs_url"] == "https://docs.honcho.dev/v3/guides/integrations/kopi"
        ...
        assert fields["workspace"]["placeholder"] == "kopi"
        assert fields["aiPeer"]["placeholder"] == "kopi"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_kopi_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["kopi"]["workspace"] == "myws"
        assert cfg["hosts"]["kopi"]["peerName"] == "eri"
        assert cfg["hosts"]["kopi"]["environment"] == "local"
        assert cfg["hosts"]["kopi"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["kopi"]["apiKey"] == "hch-test-key"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_kopi_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("kopi", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads((get_kopi_home() / "honcho.json").read_text(encoding="utf-8"))["hosts"]["kopi"]
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_kopi_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["kopi"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["kopi"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(self, monkeypatch, tmp_path):
        ...
        global_path.write_text(
            json.dumps({"baseUrl": "https://kept.example", "hosts": {"kopi": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        assert resp.status_code == 200
        assert not (get_kopi_home() / "honcho.json").exists()
        cfg = json.loads(global_path.read_text(encoding="utf-8"))
        assert cfg["baseUrl"] == "https://kept.example"
        assert cfg["hosts"]["kopi"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_updates_legacy_dot_form_host_block(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("KOPI_HONCHO_HOST", "kopi_work")

        path = self._seed_local_honcho({"hosts": {"kopi.work": {"workspace": "w", "peerName": "eri"}}})
        ...
        assert set(hosts) == {"kopi.work"}
        assert hosts["kopi.work"] == {"workspace": "w", "peerName": "eri", "sessionStrategy": "per-repo"}
    ...
    def test_put_honcho_api_key_never_overwrites_oauth_token(self, monkeypatch, tmp_path):
        ...
        path = self._seed_local_honcho({"hosts": {"kopi": {"apiKey": "hch-at-oauth-token"}}})
        ...
        assert cfg["hosts"]["kopi"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert json.loads(path.read_text(encoding="utf-8"))["hosts"]["kopi"]["workspace"] == "myws"
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        # The suite pins KOPI_HONCHO_HOST=kopi; this test exercises
        # profile-driven host resolution, so drop the override explicitly.
        monkeypatch.delenv("KOPI_HONCHO_HOST", raising=False)
        from kopi_constants import get_kopi_home
        ...
        worker_cfg.write_text(json.dumps({"hosts": {"kopi_worker": {"workspace": "kept"}}}), encoding="utf-8")
        ...
        own = json.loads((get_kopi_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_list_triggers_config_gated_auto_archive(self):
        ...
        sweeps stale ones (the Desktop `kopi serve` code path)."""
        import time as _time
        ...
        # The helper imports load_config lazily from kopi_cli.config; patch there.
    ...
    def test_update_kopi_returns_nix_guidance_without_spawning(self, monkeypatch):
        import kopi_cli.web_server as web_server

        def fail_spawn(*_args, **_kwargs):
            raise AssertionError("Nix update guard should not spawn kopi update")
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/kopi_cli/test_setup_hidden_env.py.
        """
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from kopi_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_migrates_a_legacy_plaintext_key(self):
        ...
        endpoint again.
        """
        from kopi_cli.config import custom_endpoint_key_env, get_env_value, load_config, save_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from kopi_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from kopi_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from kopi_cli.web_server import CONFIG_SCHEMA, _timezone_options
    ...
    def test_proxy_schema_warns_dashboard_users_about_lifecycle(self):
        ...
        assert "kopi egress setup" in entry["description"]
...
class TestNewEndpoints:
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "kopi"
        save_config(config)

        body = self.client.get("/api/tools/terminal/backends").json()
        ssh = next(r for r in body["backends"] if r["name"] == "ssh")
        assert ssh["status"] == "ready"
        assert "kopi@devbox.example.com" in ssh["detail"]
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_kopi_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
            reset_kopi_home_override(token)
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(kopi_home, name="ocean"):
        themes_dir = kopi_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="kopi-theme-bootstrap">')
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "kopi-theme-bootstrap" not in resp.text
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        KOPI_HOME override scopes a request to another profile."""
        from kopi_constants import (
            reset_kopi_home_override,
            set_kopi_home_override,
        )
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_sets_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.delenv("KOPI_PYTHON_SRC_ROOT", raising=False)
        monkeypatch.delenv("KOPI_PYTHON", raising=False)
        monkeypatch.delenv("KOPI_CWD", raising=False)
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
        ...
        assert env["KOPI_PYTHON_SRC_ROOT"] == str(main_mod.PROJECT_ROOT)
        assert env["KOPI_PYTHON"] == sys.executable
        assert env["KOPI_CWD"] == os.getcwd()
    ...
    def test_resolve_chat_argv_replaces_invalid_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_PYTHON_SRC_ROOT", "/definitely/missing/kopi-src")
        monkeypatch.setenv("KOPI_PYTHON", "/definitely/missing/python")
        monkeypatch.setenv("KOPI_CWD", "/definitely/missing/cwd")
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        monkeypatch.setenv("KOPI_CWD", str(tmp_path))
        monkeypatch.setenv("KOPI_PYTHON", str(relative_python))
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        assert env["KOPI_PYTHON"] == command
    ...
    def test_resolve_chat_argv_falls_back_when_getcwd_is_missing(self, monkeypatch, tmp_path):
        ...
        assert env is not None
        assert env["KOPI_CWD"] == str(tmp_path)
