...
Canonical owners:
  ...
  * ``kopi_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._kopi_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/kopi/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "kopi_cli/managed_scope.py",
