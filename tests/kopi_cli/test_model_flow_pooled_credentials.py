...
from kopi_cli.model_setup_flows import _existing_api_key_for_model_flow
...
def test_existing_key_precedence_is_dotenv_then_process_then_pool(tmp_path, monkeypatch):
    ...
    (kopi_home / ".env").write_text("DEEPSEEK_API_KEY=dotenv-secret\n", encoding="utf-8")
    ...
    (kopi_home / ".env").write_text("", encoding="utf-8")
    with patch("agent.credential_pool.load_pool", return_value=_AvailablePool()):
        assert _existing_api_key_for_model_flow("deepseek", pconfig) == (
            "process-secret",
            "DEEPSEEK_API_KEY",
...
def test_generic_api_key_flow_passes_pool_key_to_existing_key_prompt(monkeypatch):
    from kopi_cli.model_setup_flows import _model_flow_api_key_provider
    ...
    with (
        ...
        patch("kopi_cli.main._prompt_api_key", side_effect=capture_prompt),
    ):
...
def test_bedrock_flow_sees_pool_key_when_no_env(monkeypatch, capsys):
    ...
    from kopi_cli.model_setup_flows import _model_flow_bedrock_api_key
