...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git' into $KOPI_HOME keeps
...
def test_nix_store_path_detected_as_nix(tmp_path, monkeypatch):
    ...
    as 'nix' even without KOPI_MANAGED or a .install_method stamp."""
    ...
    fake_nix = fake_nix_store / "abc123-kopi-ai-agent-0.19.0"
    fake_nix.mkdir(parents=True)

    monkeypatch.setattr("kopi_cli.config._NIX_STORE", fake_nix_store)
