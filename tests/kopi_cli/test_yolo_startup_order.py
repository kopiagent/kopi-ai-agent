"""Regression tests for #60328: --yolo must set KOPI_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing KOPI_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("KOPI_YOLO_MODE")

    monkeypatch.setattr(
        "kopi_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("kopi_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("KOPI_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from kopi_cli.main import main as cli_main
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """kopi --yolo must set KOPI_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["kopi", "--yolo"]
    )
    assert result == "1", (
        "KOPI_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """kopi chat --yolo must also set KOPI_YOLO_MODE before
    _prepare_agent_startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["kopi", "chat", "--yolo"]
    )
    assert result == "1", (
        "KOPI_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_no_yolo_flag_leaves_env_unset_at_startup(monkeypatch):
    """Without --yolo, KOPI_YOLO_MODE must not be set at startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["kopi"]
    )
    assert result is None, (
        "KOPI_YOLO_MODE was unexpectedly set at startup without --yolo."
    )
