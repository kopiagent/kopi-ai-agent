...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(kopi_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(kopi_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_cmd_update_reloads_runtime_modules_before_lazy_refresh(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(kopi_main.subprocess, "run", fake_run)
    monkeypatch.setattr(kopi_main, "_reload_updated_runtime_modules", fake_reload_runtime_modules)
    monkeypatch.setattr(kopi_main, "_refresh_active_lazy_features", fake_refresh_lazy_features)
...
def test_reload_updated_runtime_modules_restores_new_kopi_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import kopi_constants

    monkeypatch.delattr(kopi_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(kopi_constants, "apply_subprocess_home_env")

    kopi_main._reload_updated_runtime_modules()

    assert callable(kopi_constants.apply_subprocess_home_env)
...
def test_install_method_marker_not_autostashed_by_update(tmp_path):
    ...
    metadata; it is a sibling of ``.kopi-bootstrap-complete`` /
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:4] == ["stash", "push", "--include-untracked"]:
            return SimpleNamespace(
                ...
                stderr=(
                    "warning: failed to remove packaging/homebrew/kopi-ai-agent.rb: "
                    "Permission denied\n"
                ),
                returncode=1,
            )
...
def test_stash_push_failure_without_stash_entry_still_raises(monkeypatch, tmp_path, capsys):
    ...
    with pytest.raises(CalledProcessError):
        kopi_main._stash_local_changes_if_needed(["git"], tmp_path)
    out = capsys.readouterr().out
...
def test_stash_apply_untracked_only_failure_detector():
    fn = kopi_main._stash_apply_failed_only_on_existing_untracked
    assert fn(
        "packaging/homebrew/kopi-ai-agent.rb already exists, no checkout\n"
...
def test_restore_treats_existing_untracked_only_failure_as_restored(
    ...
    restored = kopi_main._restore_stashed_changes(
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "kopi-ai-agent.rb").write_text("formula\n")
    ...
    try:
        ...
        assert (pkg / "kopi-ai-agent.rb").read_text() == "formula\n"
    finally:
        os.chmod(pkg, 0o755)
