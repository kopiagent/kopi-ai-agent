...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew kopi_cli.backup's datetime forward instead of sleeping.
    ...
    import kopi_cli.backup as _backup

    shim = getattr(_backup.datetime, "_kopi_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _kopi_test_shim = True
