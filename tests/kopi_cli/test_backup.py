...
def _make_kopi_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "kopi_state.db"):
...
class TestQuickSnapshot:
    ...
    def test_copies_discord_recovery_ledger(self, kopi_home):
        from kopi_cli.backup import create_quick_snapshot

        gateway_dir = kopi_home / "gateway"
        gateway_dir.mkdir()
        ...
        copied = kopi_home / "state-snapshots" / snap_id / "gateway" / ledger.name
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, kopi_home, capsys):
        ...
        first_id = create_quick_snapshot(label="complete", kopi_home=kopi_home)
        assert first_id is not None
        first_dir = kopi_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", kopi_home=kopi_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = kopi_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, kopi_home=kopi_home)
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_bom_live_file_still_counted(self, tmp_path):
        ...
        from kopi_cli.backup import _count_cron_jobs, restore_cron_jobs_if_emptied
        kopi_home = tmp_path / ".kopi"
        jobs_path = kopi_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(kopi_home)
