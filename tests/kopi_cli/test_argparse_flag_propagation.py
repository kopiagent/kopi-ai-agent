...
class TestAcceptHooksOnAgentSubparsers:
    ...
    # One driver subprocess parses ALL argvs: kopi_cli.main is a very heavy
    # import (previously 11 separate `python -m kopi_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `kopi <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `kopi -t web chat` silently dropped the
    ...
    Uses the real `kopi_cli._parser.build_top_level_parser()` rather than
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`kopi {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `kopi --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`kopi {flag} chat` lost the flag — got "
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `kopi <flag> <value> chat`:\n  "
