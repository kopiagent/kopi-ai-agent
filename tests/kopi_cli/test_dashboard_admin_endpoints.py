...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.kopi/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `kopi auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `kopi auth add`, which clears every suppression for the
    ...
    def test_delete_does_not_clobber_other_providers(self):
        ...
        from agent.credential_pool import load_pool
        from kopi_cli.auth import _load_auth_store, read_credential_pool
        from kopi_cli.config import save_env_value
...
class TestOpsEndpoints:
    ...
    def test_backup_output_uses_output_flag(self, monkeypatch):
        ...
        r = self.client.post(
            "/api/ops/backup",
            json={"output": "  /tmp/kopi-test.zip  "},
        )

        assert r.status_code == 200
        assert captured == {
            "subcommand": ["backup", "-o", "/tmp/kopi-test.zip"],
            "name": "backup",
        }
...
# ---------------------------------------------------------------------------
# _spawn_kopi_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_kopi_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _KOPI_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    ws._spawn_kopi_action(["gateway", "restart"], "gateway-restart")

    assert "_KOPI_GATEWAY" not in captured["env"]
    assert captured["env"]["KOPI_NONINTERACTIVE"] == "1"
