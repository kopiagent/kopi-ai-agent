...
from kopi_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
def test_default_max_bytes_bounds_the_pragma_by_size():
    ...
    default made `kopi update` peg a CPU for minutes on a multi-GB
    state.db with no output (read as a hang). Callers that omit max_bytes
    must inherit a finite ceiling.
    """
    import inspect

    from kopi_cli.backup import DEFAULT_INTEGRITY_CHECK_MAX_BYTES
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    KOPI_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def test_healthy_db_stays_quiet(self, kopi_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, kopi_home, capsys, monkeypatch):
