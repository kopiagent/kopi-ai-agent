...
def test_run_and_exit_oneshot_hard_exits_when_oneshot_import_fails(
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "kopi_cli.oneshot" and "run_oneshot" in (fromlist or ()):
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import kopi_cli.oneshot as oneshot
        from kopi_cli.main import _exit_after_oneshot
...
def test_main_oneshot_path_bypasses_late_atexit_abort():
    ...
    program = textwrap.dedent(
        ...
        sys.modules["kopi_cli.oneshot"] = types.SimpleNamespace(
...
def test_launch_tui_worktree_validates_relative_python_against_final_cwd(
    ...
    monkeypatch.setenv("KOPI_CWD", str(parent_cwd))
    ...
    assert captured["env"]["KOPI_CWD"] == str(worktree)
    assert captured["env"]["KOPI_PYTHON"] == str(relative_python)
