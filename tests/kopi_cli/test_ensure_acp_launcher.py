"""`kopi update` must self-heal the ``kopi-acp`` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``kopi-acp`` command name on the login-shell PATH. Fresh installs get the
launcher from ``scripts/install.sh``; existing installs get it from
``_ensure_acp_launcher()`` during ``kopi update``.
"""
...
from kopi_cli.main import _ensure_acp_launcher
...
def test_does_not_follow_symlink_into_venv(fake_home, tmp_path):
    """#21454 failure mode: never write through a symlinked kopi-acp."""
    ...
    (fake_home / "kopi-acp").symlink_to(console_script)
