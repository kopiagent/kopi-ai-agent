...
only when they still resolve into the Kopi-managed node dir.
"""
...
def test_removes_dangling_symlink_into_kopi_node(fake_home):
    """A link into the Kopi node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Kopi links that exist; ignores the ones that don't."""
    kopi_home = fake_home / ".kopi"
    node_bin = _make_kopi_node(kopi_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Kopi-managed; node is a real user binary.
