...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/kopi-ai-agent:latest" in hint
        assert "recreate all Kopi containers" in hint
        assert "kopi update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `kopi update`" in hint
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    Kopi-managed node bin (~/.kopi/node/bin), not on PATH or in
...
def test_run_doctor_detects_agent_browser_in_managed_node_bin(monkeypatch, tmp_path):
    # Regression for #53192: `kopi acp --setup-browser` installs into
    # ~/.kopi/node/bin/agent-browser, which isn't on PATH; doctor must still
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp KOPI_HOME.

        ``run_doctor`` reads the module-level ``KOPI_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("KOPI_HOME")`` alone
        leaves doctor probing the REAL ~/.kopi. On a dev machine with a
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    ...
    # left thinking a crashing command means a broken Kopi install.
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_collect_deprecated_env_vars(self):
        ...
        assert "KOPI_TOOL_PROGRESS" in names
        assert "TERMINAL_CWD" in names
        assert "QQ_HOME_CHANNEL" in names
        assert "OPENAI_API_KEY" not in names
        by_name = dict(findings)
        assert "display.tool_progress" in by_name["KOPI_TOOL_PROGRESS"]
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        kopi_home = tmp_path / ".kopi"
        kopi_home.mkdir(parents=True)
        (kopi_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (kopi_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, kopi_home = self._run_doctor_with_config(monkeypatch, tmp_path, config_yaml=cfg)
        ...
        on_disk = (kopi_home / "config.yaml").read_text(encoding="utf-8")
...
"""
        ...
        assert "Deprecated: KOPI_TOOL_PROGRESS" in out
...
"""
        ...
        assert "Deprecated: KOPI_TOOL_PROGRESS" not in out
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"KOPI_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: KOPI_TOOL_PROGRESS_MODE" in out
