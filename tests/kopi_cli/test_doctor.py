...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_kopi_tool_progress_warning_says_unsupported_since_floor(self):
        """KOPI_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"KOPI_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["KOPI_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"KOPI_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["KOPI_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"
