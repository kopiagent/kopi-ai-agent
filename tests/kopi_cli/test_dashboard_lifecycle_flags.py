...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("kopi_cli.main._scan_dashboard_processes", return_value=[]), \
    ...
    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "kopi dashboard --port 9119"),
            (12346, "python -m kopi_cli.main dashboard --host 0.0.0.0 --port 9120"),
        ]
        with patch("kopi_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("kopi_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
    ...
    def test_status_ignores_headless_serve_children_and_non_listeners(self, capsys):
        processes = [
            (11111, "kopi serve --port 0"),
            (22222, "kopi dashboard --port 9119"),
            (33333, "kopi dashboard --port 9120"),
        ]
        ...
        with patch("kopi_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("kopi_cli.main._dashboard_listening", side_effect=fake_listening), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))

        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "1 kopi dashboard process(es) running" in out
    ...
    def test_status_ignores_dead_pids(self, capsys):
        with patch(
            "kopi_cli.main._scan_dashboard_processes",
            return_value=[(12345, "kopi dashboard --port 9119")],
        ), \
