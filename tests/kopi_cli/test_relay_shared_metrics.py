"""Focused tests for the Kopi shared-metrics durable store."""
...
import pytest
from kopi_cli.observability import shared_metrics as shared_metrics_module
from kopi_cli.observability.shared_metrics import SharedMetricsStore
from kopi_cli.observability.shared_metrics_contract import (
    COUNT_BUCKETS,
    DURATION_BUCKETS,
    EXECUTION_SURFACES,
    MODEL_FAMILIES,
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "kopi.shared_metrics.v1"
    assert first_package["resource"] == {"kopi_version": "test-version"}
...
def test_due_export_runs_once_per_utc_day_and_catches_up_pending_deltas(
    ...
    assert store.counter_snapshot()[0] == {
        ...
        "metric_name": "kopi.model_call.count",
        "kopi_version": "test-version",
        "dimensions": _dimensions(),
        "value": 2,
        "packaged_value": 1,
    }
...
def test_provider_family_does_not_resolve_live_provider_metadata(monkeypatch):
    ...
    monkeypatch.setattr("kopi_cli.providers.get_provider", fail_live_lookup)
...
def test_model_family_covers_families_evidenced_by_the_kopi_catalog(model, expected):
...
def test_execution_surface_uses_the_kopi_platform_registry(platform, expected):
...
def test_subscriber_contract_rejects_unknown_fields_and_dimension_values():
    event = SimpleNamespace(
        ...
        name="kopi.model_call",
        scope_category="end",
        metadata={"kopi.metrics.schema_version": "kopi.metrics.event.v1"},
        data={
            "call_role": "primary",
            "locality": "remote",
            "model_family": "gpt",
...
def test_pending_metrics_keep_the_version_recorded_at_event_time(tmp_path):
    ...
    assert {package["resource"]["kopi_version"] for package in packages} == {
        "version-a",
        "version-b",
    }
...
def test_store_exports_task_started_and_terminal_counters(tmp_path):
    ...
    store.record_counter("kopi.task_run.finished", terminal, "test-version")
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE kopi_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT kopi_version FROM counter_aggregates"
            ).fetchall()
        }
...
def test_package_export_does_not_chase_concurrent_updates(tmp_path, monkeypatch):
    ...
    assert counter["metric_name"] == "kopi.model_call.count"
