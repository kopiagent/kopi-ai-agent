...
from kopi_cli.slack_cli import _build_full_manifest, slack_manifest_command
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from kopi_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from kopi_cli import main as main_module
    from kopi_cli import slack_cli
...
class TestSlackManifestArgparse:
    ...
    def test_long_description_file_reports_tilde_expansion_errors(
        ...
        source = "~kopi-user-that-does-not-exist-20260716/AGENTS.md"
