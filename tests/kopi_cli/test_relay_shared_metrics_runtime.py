"""Tests for the direct Kopi-to-Relay shared-metrics runtime."""
...
from kopi_cli import lifecycle, plugins
from kopi_cli.observability import relay_runtime, relay_shared_metrics
...
def direct_runtime(tmp_path, monkeypatch):
    fake = _Relay()
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "kopi-home"))
    monkeypatch.setattr(relay_runtime, "_load_nemo_relay", lambda: fake)
    monkeypatch.setattr(
        "kopi_cli.config.read_raw_config_readonly",
...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert scope_starts[1][1] == "kopi.task_run"
    ...
    root = tmp_path / "kopi-home" / "telemetry" / "shared_metrics"
    ...
    assert set(metrics) == {
        "kopi.model_call.count",
        "kopi.task_run.finished",
        "kopi.task_run.started",
    }
    assert metrics["kopi.model_call.count"]["dimensions"]["model_family"] == "claude"
    assert metrics["kopi.model_call.count"]["value"] == 1
    assert metrics["kopi.task_run.started"] == {
        "name": "kopi.task_run.started",
        "type": "counter",
        "dimensions": {
            "entrypoint": "interactive",
            "execution_surface": "cli",
    ...
    terminal = metrics["kopi.task_run.finished"]["dimensions"]
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from kopi_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "kopi-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "kopi_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
    ...
    assert len(by_metric["kopi.task_run.started"]) == 1
    assert by_metric["kopi.task_run.started"][0]["value"] == 3
    assert {
        counter["dimensions"]["outcome"]
        for counter in by_metric["kopi.model_call.count"]
    } == {"success", "failed", "cancelled"}
    terminal_by_outcome = {
        counter["dimensions"]["outcome"]: counter
        for counter in by_metric["kopi.task_run.finished"]
    }
    ...
    for package in package_payloads:
        assert package["schema_version"] == "kopi.shared_metrics.v1"
...
def test_direct_runtime_is_disabled_by_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "kopi-home"))
    monkeypatch.setattr(relay_runtime, "_load_nemo_relay", lambda: fake)
    monkeypatch.setattr("kopi_cli.config.read_raw_config_readonly", lambda: {})
    relay_shared_metrics._reset_for_tests()
    relay_runtime._reset_for_tests()
    monkeypatch.setattr(plugins, "_plugin_manager", PluginManager())
    ...
    assert fake.events == []
    assert not (tmp_path / "kopi-home" / "telemetry").exists()
    relay_shared_metrics._reset_for_tests()
    relay_runtime._reset_for_tests()
...
def test_profile_key_caches_absolute_path_resolution(monkeypatch):
    ...
    home = Home()
    monkeypatch.setattr(relay_runtime, "get_kopi_home", lambda: home)
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert not relay_runtime.emit_mark("kopi.probe", session_id="s1")
    assert "Kopi Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_core_mark_uses_the_shared_session_handle_without_a_plugin(direct_runtime):
    ...
    assert relay_runtime.emit_mark(
        ...
        metadata={"data_schema": "kopi.skill.lifecycle.v1"},
    )

    [mark] = [event for event in direct_runtime.events if event[0] == "scope.event"]
    assert mark[1] == "kopi.skill.created"
...
def test_session_initializer_failure_does_not_block_conversation(
    ...
    assert "Kopi Relay session initializer failed" in caplog.text
    coordinator.finalize_conversation(
        profile_key=relay_runtime.current_profile_key(),
        session_id=lease.session_id,
    )
...
def test_core_runtime_isolates_same_session_id_by_profile(direct_runtime, tmp_path):
    ...
    token = set_kopi_home_override(profile_a)
    try:
    ...
    token = set_kopi_home_override(profile_b)
    try:
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from kopi_cli import config, managed_scope
    from kopi_constants import (
        reset_kopi_home_override,
        set_kopi_home_override,
    )
...
def test_shared_metrics_policy_and_store_are_profile_scoped(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "telemetry": {
                "shared_metrics": {"enabled": get_kopi_home() == profile_a}
            }
        },
    )
    relay_shared_metrics._reset_for_tests()
...
def test_shared_metrics_subscribers_isolate_two_enabled_profiles(tmp_path, monkeypatch):
    ...
    for profile in (profile_a, profile_b):
        ...
        package = json.loads(packages[0].read_text(encoding="utf-8"))
        metrics = {metric["name"]: metric for metric in package["metrics"]}
        assert metrics["kopi.task_run.started"]["value"] == 1
        assert metrics["kopi.task_run.finished"]["value"] == 1
...
def test_shared_metrics_isolates_same_task_id_across_sessions(direct_runtime):
    ...
    task_starts = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.push" and event[1] == "kopi.task_run"
    ]
    task_ends = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.pop" and event[1][1] == "kopi.task_run"
    ]
...
def test_disabling_shared_metrics_stops_collection_and_shutdown_export(
    ...
    runtime.subscriber(SimpleNamespace(
        ...
        scope_category="start",
        metadata={
            "kopi.metrics.schema_version": "kopi.metrics.event.v1",
...
def test_session_runners_preserve_caller_context_and_profile_override(
    ...
    profile_token = set_kopi_home_override(profile)
    ...
    try:
        ...
        reset_kopi_home_override(profile_token)
...
def test_active_turn_requires_matching_session_and_profile(
    ...
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "other-profile"))
...
def test_core_runtime_parents_subagent_session_without_exposing_ids(
    ...
    assert child_kwargs["metadata"] == {
        "kopi.execution_surface": "subagent",
...
def test_desktop_task_completion_exports_once_per_utc_day(
    ...
    outbox = tmp_path / "kopi-home" / "telemetry" / "shared_metrics" / "outbox"
    ...
    assert first_metrics["kopi.task_run.started"]["value"] == 1
    assert first_metrics["kopi.task_run.started"]["dimensions"] == {
        "entrypoint": "interactive",
        "execution_surface": "desktop",
    }
    assert first_metrics["kopi.task_run.finished"]["value"] == 1
    ...
    assert totals["kopi.task_run.started"] == 3
    assert totals["kopi.task_run.finished"] == 3
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    metrics = {metric["name"]: metric for metric in package["metrics"]}
    assert metrics["kopi.task_run.started"]["value"] == 2
    assert metrics["kopi.task_run.finished"]["value"] == 2
    assert flush_attempts == 2
    assert "Kopi shared-metrics task flush failed" in caplog.text
...
def test_persistence_failure_does_not_escape_the_hook(
    ...
    assert "Unable to persist the Kopi shared metric" in caplog.text
