...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert tool_starts[0][1] == "kopi.tool_call"
    ...
    assert set(metrics) == {
        "kopi.model_route.count",
        "kopi.task_run.finished",
        "kopi.task_run.started",
        "kopi.tool_approval.count",
        "kopi.tool_call.count",
    }
    assert metrics["kopi.model_route.count"]["dimensions"] == {
        "model": "claude-sonnet",
        "provider": "anthropic",
    }
    assert metrics["kopi.model_route.count"]["value"] == 1
    assert metrics["kopi.tool_call.count"] == {
        "name": "kopi.tool_call.count",
        "type": "counter",
        "dimensions": {
            "approval_outcome": "approved",
            "latency_bucket": "250ms_to_500ms",
    ...
    assert metrics["kopi.tool_approval.count"] == {
        "name": "kopi.tool_approval.count",
        "type": "counter",
        "dimensions": {
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    assert len(by_metric["kopi.model_route.count"]) == 1
    model_counter = by_metric["kopi.model_route.count"][0]
    ...
    assert {
        ...
        for counter in by_metric["kopi.tool_call.count"]
    ...
    assert len(by_metric["kopi.tool_approval.count"]) == 1
    approval_counter = by_metric["kopi.tool_approval.count"][0]
    ...
    for package in package_payloads:
        assert package["schema_version"] == "kopi.shared_metrics.v2"
...
def test_real_binding_correlates_plugin_approval_denial_to_tool_metric(
    ...
    tool_metrics = [
        counter
        for counter in snapshot
        if counter["metric_name"] == "kopi.tool_call.count"
    ]
    ...
    approval_metrics = [
        counter
        for counter in snapshot
        if counter["metric_name"] == "kopi.tool_approval.count"
    ]
...
def test_approval_without_tool_context_is_counted_as_unattributed(direct_runtime):
    ...
    [approval] = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.event" and event[1] == "kopi.tool_approval"
    ]
