"""Tests for kopi_cli.agent_import — ``kopi import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Kopi stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/kopi_cli/test_profiles.py:
Path.home() and KOPI_HOME are redirected to tmp_path so nothing touches
the real ~/.kopi.
"""
...
from kopi_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and KOPI_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, KOPI_HOME -> tmp/.kopi."""
...
@pytest.fixture()
def kopi_home(profile_env):
    return profile_env / ".kopi"
...
def run_import(agent, source, kopi_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=kopi_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, kopi_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, kopi_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, kopi_home):
        before = snapshot_tree(kopi_home)
        report = run_import("claude-code", claude_tree, kopi_home, execute=False)
        assert snapshot_tree(kopi_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0

    def test_codex_dry_run_writes_nothing(self, codex_tree, kopi_home):
        before = snapshot_tree(kopi_home)
        report = run_import("codex", codex_tree, kopi_home, execute=False)
        assert snapshot_tree(kopi_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0

    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, kopi_home):
        preview = run_import("claude-code", claude_tree, kopi_home, execute=False)
        applied = run_import("claude-code", claude_tree, kopi_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, kopi_home):
        return run_import("claude-code", claude_tree, kopi_home, execute=True)

    def test_claude_md_becomes_memory_entries(self, report, kopi_home):
        memory = (kopi_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        assert "type hints" in memory
        assert "§" in memory  # entry-delimited store format

    def test_allowlist_lands_in_config_yaml(self, report, kopi_home):
        config = yaml.safe_load((kopi_home / "config.yaml").read_text())
    ...
    def test_denylist_lands_in_approvals_deny(self, report, kopi_home):
        config = yaml.safe_load((kopi_home / "config.yaml").read_text())
        assert "rm -rf *" in config["approvals"]["deny"]

    def test_mcp_servers_from_claude_json_and_settings(self, report, kopi_home):
        config = yaml.safe_load((kopi_home / "config.yaml").read_text())
    ...
    def test_skill_copied_into_category_dir(self, report, kopi_home):
        dest = kopi_home / "skills" / "claude-code-imports" / "deploy-helper" / "SKILL.md"
        assert dest.exists()
        assert "Deploy things" in dest.read_text(encoding="utf-8")

    def test_dir_without_skill_md_not_copied(self, report, kopi_home):
        assert not (kopi_home / "skills" / "claude-code-imports" / "not-a-skill").exists()
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, kopi_home):
        return run_import("codex", codex_tree, kopi_home, execute=True)

    def test_agents_md_becomes_memory_entries(self, report, kopi_home):
        memory = (kopi_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        assert "force-push" in memory

    def test_memories_dir_merged(self, report, kopi_home):
        memory = (kopi_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        assert "tabs over spaces" in memory
        assert "PostgreSQL" in memory

    def test_mcp_servers_from_config_toml(self, report, kopi_home):
        config = yaml.safe_load((kopi_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, kopi_home):
        assert (kopi_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, kopi_home):
        run_import("claude-code", claude_tree, kopi_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in kopi_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, kopi_home):
        run_import("codex", codex_tree, kopi_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in kopi_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, kopi_home):
        report = run_import("claude-code", claude_tree, kopi_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, kopi_home):
        run_import("claude-code", claude_tree, kopi_home, execute=True)
        config = yaml.safe_load((kopi_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, kopi_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, kopi_home, execute=True)
        ...
        assert "still importable" in (
            kopi_home / "memories" / "MEMORY.md").read_text()

    def test_bad_claude_json_reports_error(self, profile_env, kopi_home):
        root = profile_env / ".claude"
        root.mkdir()
    ...
    def test_bad_config_toml_reports_error(self, profile_env, kopi_home):
        root = profile_env / ".codex"
        root.mkdir()
        (root / "config.toml").write_text("[[[[not toml", encoding="utf-8")
        (root / "AGENTS.md").write_text("- rule one\n", encoding="utf-8")
        report = run_import("codex", root, kopi_home, execute=True)
        errors = [i for i in report["items"] if i["status"] == "error"]
        assert any(i["kind"] == "config" for i in errors)
        assert "rule one" in (kopi_home / "memories" / "MEMORY.md").read_text()

    def test_missing_source_dir_is_error_not_crash(self, profile_env, kopi_home):
        report = run_import(
            "claude-code", profile_env / "nope", kopi_home, execute=True)
        assert report["summary"]["error"] == 1
        assert report["summary"]["imported"] == 0

    def test_empty_tree_all_skipped(self, profile_env, kopi_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, kopi_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    def test_existing_allowlist_preserved(self, claude_tree, kopi_home):
    ...
    def test_overwrite_replaces_mcp_server(self, claude_tree, kopi_home):
        (kopi_home / "config.yaml").write_text(
            yaml.safe_dump({"mcp_servers": {"github": {"command": "mine"}}}),
            encoding="utf-8")
        run_import("claude-code", claude_tree, kopi_home, execute=True,
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, kopi_home):
        dest = kopi_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, kopi_home):
        run_import("claude-code", claude_tree, kopi_home, execute=True)
        first = (kopi_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, kopi_home, execute=True)
        assert (kopi_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, kopi_home):
        memory = kopi_home / "memories" / "MEMORY.md"
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, kopi_home, capsys):
        ...
        assert not (kopi_home / "memories" / "MEMORY.md").exists()
        assert not (kopi_home / "skills" / "claude-code-imports").exists()
        config_text = (kopi_home / "config.yaml").read_text(encoding="utf-8") \
            if (kopi_home / "config.yaml").exists() else ""
