...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Kopi' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Kopi' DB/metadata/logs on task completion.
    """
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # KOPI_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is KOPI_SESSION_SOURCE, which the dispatcher
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import kopi_state as _kopi_state
    monkeypatch.setattr(
        _kopi_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _kopi_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("kopi_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="kopi_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        kopi_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "kopi_cli.kanban_db.sqlite3.connect",
...
def test_resolve_kopi_argv_module_actually_runs():
    ...
    assert "Kopi Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
