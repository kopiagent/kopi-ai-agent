...
config.yaml, memories/*.md) because the host Kopi process never reads
...
class TestClassifySandboxMirrorTarget:
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        """A plain Kopi path is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_sandboxes_segment_without_home_kopi_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to Kopi-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_kopi_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-kopi`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
