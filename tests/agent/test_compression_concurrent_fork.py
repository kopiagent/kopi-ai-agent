...
Damien's incident (Discord, 2026-05-28): a long Kopi session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``kopi_state`` module, whose ``SessionDB``
...
_NominalSessionDBImpostor.__module__ = "kopi_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``kopi_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = kopi_state.SessionDB
    monkeypatch.setattr(kopi_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(kopi_state, "SessionDB", real_session_db_type)
