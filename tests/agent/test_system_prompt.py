...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "KOPI_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_kopi_home", lambda: Path("/kopi"))

    expected_profile = (
        "Active Kopi profile: default. Other profiles (if any) live "
        "under /kopi/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("kopi_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("kopi_cli.config.load_config_readonly") as mock_cfg:
