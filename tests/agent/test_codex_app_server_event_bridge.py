"""Regression tests for the codex_app_server → Kopi UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_kopi_tools_mcp_server_emits_bare_tool_name(self):
        """The kopi-tools MCP server wraps Kopi' own tools for codex;
        ...
        mcp.kopi-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "kopi-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "kopi-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "kopi agent docs"
        assert calls[0].args[3] == {"query": "kopi agent docs"}
