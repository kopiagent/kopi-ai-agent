...
def test_pending_response_records_kanban_timeout(monkeypatch):
    monkeypatch.setattr("kopi_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("KOPI_KANBAN_TASK", "task-123")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("kopi_cli.kanban_db.connect", lambda: conn)
    monkeypatch.setattr("kopi_cli.kanban_db._record_task_failure", record)
    agent = _LimitAgent()
