...
def test_stream_uses_rewritten_request_and_post_intercept_chunks(relay_turn):
    ...
    try:
        ...
        relay.intercepts.deregister_llm_stream_execution("kopi-test-stream")
        relay.intercepts.deregister_llm_request("kopi-test-request")
...
def test_stream_does_not_swallow_kopi_finalizer_failure(relay_turn, monkeypatch):
    relay, _turn = relay_turn
    finalizer_error = RuntimeError("Kopi finalizer failed")
...
def test_chat_codec_preserves_provider_message_extensions_after_rewrite(relay_turn):
    ...
    relay.intercepts.register_llm_request(
        "kopi-provider-extension-request",
        1,
        False,
        rewrite_request,
    )
