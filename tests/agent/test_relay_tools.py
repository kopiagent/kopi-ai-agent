"""Tests for the core Relay-managed Kopi tool adapter."""
...
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "profile"))
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "kopi-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "kopi-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("kopi-test-tool-execution")
        relay.intercepts.deregister_tool_request("kopi-test-tool-request")
...
def test_tool_adapter_returns_dispatch_result_after_relay_post_processing_failure(
    ...
    assert "returning the Kopi tool result" in caplog.text
