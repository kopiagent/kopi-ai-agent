...
class TestApiModeRouting:
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the Kopi
        ...
        assert (
            determine_api_mode("nous", PORTAL_URL, model="kopi-4-405b")
            == "chat_completions"
        )
...
class TestRuntimeResolution:
    ...
    def test_non_anthropic_model_stays_on_chat_completions(self, monkeypatch):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="nous", target_model="kopi-4-405b"
        )
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "nous", "default": "kopi-4-405b"},
        )
...
class TestClientShape:
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. Kopi loads that env from ~/.kopi/.env, so
...
class TestAuxiliaryDualWire:
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                "agent.auxiliary_client._try_nous",
                return_value=(plain, "kopi-4-405b"),
            ),
            patch(
                "agent.anthropic_adapter.build_anthropic_client",
            ...
            client, model = resolve_provider_client("nous", "kopi-4-405b")
