...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "kopi" / "auth.json").read_text())
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Kopi PKCE
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``kopi auth remove copilot gh_cli``.  The gate must run
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / "kopi"))
    from kopi_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("kopi_cli.copilot_auth.resolve_copilot_token", _boom)
