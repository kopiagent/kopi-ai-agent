...
class TestGatewayRuntimeStatus:
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders KOPI_HOME with backslashes while the
        ...
        cmdline = r"kopi_home=c:\opt\data\profiles\coder kopi gateway run --replace"
...
class TestTakeoverMarker:
    ...
    def test_consume_accepts_legacy_marker_without_kopi_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Kopi versions have no
...
class TestScopedLockTakeover:
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        replacer_home.mkdir()
        monkeypatch.setenv("KOPI_HOME", str(replacer_home))
        ...
        monkeypatch.setattr(
            ...
            lambda _pid: "python -m kopi_cli.main gateway run",
        )
        calls = []
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_kopi_home"] == str(target_home)
            assert payload["replacer_kopi_home"] == str(replacer_home)
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KOPI_HOME", str(tmp_path))
        from kopi_cli.gateway import generate_launchd_plist
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the KOPI_HOME contextvar override (#56986), so the profile
