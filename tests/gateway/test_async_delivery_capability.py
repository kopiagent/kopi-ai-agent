...
class TestAsyncDeliverySupported:
    ...
    def test_dispatcher_spawned_kanban_worker_is_unsupported(self, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_KANBAN_TASK", "t_review")
...
class TestDeclareStatelessChannel:
    """``kopi -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``KOPI_SESSION_*`` routing keys, so an async delegation's
