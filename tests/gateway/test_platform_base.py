...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    kopi_home.mkdir()
    monkeypatch.setattr(base, "_KOPI_HOME", kopi_home)
    monkeypatch.setattr(base, "_KOPI_ROOT", kopi_home)
    path = kopi_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        self._patch_roots(monkeypatch)
        monkeypatch.setenv("KOPI_KANBAN_HOME", str(tmp_path / "kopi"))
        monkeypatch.setenv("KOPI_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "kopi" / "kanban" / "boards" / "research"
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_shared_kopi_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Kopi root config."""
        self._patch_roots(monkeypatch)
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Kopi
