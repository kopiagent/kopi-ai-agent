...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``kopi gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "kopi_feedback" in created_apps[0].registered_actions
        assert "kopi_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("kopi_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("kopi_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestMissingCredentials:
    ...
    async def test_missing_bot_token_sets_fatal_error(self):
        ...
        assert "kopi gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_kopi_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/kopi",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/kopi-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        a.handle_message.assert_awaited_once()
        msg_event = a.handle_message.call_args[0][0]
        assert msg_event.media_urls == ["/tmp/kopi-cached.png"]
    ...
    async def test_collect_thread_root_images_resolves_connect_stub(
        ...
        assert urls == ["/tmp/kopi-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_KOPI_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_kopi_slack_user_agent_prefix_format(self):
        """Module constant matches the KopiAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._KOPI_SLACK_USER_AGENT_PREFIX.startswith("KopiAgent/")

    @pytest.mark.asyncio
    async def test_async_web_client_constructed_with_kopi_user_agent_prefix(self):
        ...
        expected_prefix = _slack_mod._KOPI_SLACK_USER_AGENT_PREFIX
