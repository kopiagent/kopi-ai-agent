...
class TestHomeChannelRoundtrip:
    ...
    def test_persisted_relay_home_survives_real_config_reload(self, tmp_path, monkeypatch):
        ...
        home_token = set_kopi_home_override(str(tmp_path))
        try:
            persist_home_channel(
                HomeChannel(
                    platform=Platform.SLACK,
...
class TestLoadGatewayConfig:
    def test_shipped_template_does_not_enable_auto_reset(self, tmp_path, monkeypatch):
        ...
        docker/stage2-hook.sh, kopi doctor) copy the template verbatim to
        ~/.kopi/config.yaml, so whatever ``session_reset.mode`` the template
    ...
    def test_typing_status_text_from_toplevel_platform_block(self, tmp_path, monkeypatch):
        ...
        fallback) — the route a bare ``kopi config set``-style YAML uses."""
        kopi_home = tmp_path / ".kopi"
        kopi_home.mkdir()
        (kopi_home / "config.yaml").write_text(
...
class TestMultiplexProfilesConfig:
    ...
    def test_multiplex_profiles_nested_under_gateway(self, tmp_path, monkeypatch):
        """gateway.multiplex_profiles (the form written by `kopi config set
