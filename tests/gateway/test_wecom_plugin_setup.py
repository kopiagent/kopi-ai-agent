...
``kopi_cli.config`` (get_env_value / save_env_value / remove_env_value),
``kopi_cli.cli_output`` (prompt / prompt_yes_no / print_*), and
``kopi_cli.setup`` (prompt_choice); we patch each at its source module so
