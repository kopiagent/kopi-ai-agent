...
def test_addressed_pipe_form_bot_is_not_other():
    assert _addressed(f"<@{BOT_USER_ID}|kopi> hello") is False
...
def test_mentions_self_pipe_form():
    assert _mentions_self(f"hello <@{BOT_USER_ID}|kopi>") is True
...
async def test_free_response_replies_when_bot_mentioned_in_pipe_form(adapter):
    ...
    await _run(
        adapter,
        _event(
            f"<@{OTHER_USER_ID}|rasha> and <@{BOT_USER_ID}|kopi> please compare",
