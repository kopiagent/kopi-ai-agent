...
async def test_gateway_handler_uses_shared_persistent_logic_without_cache_eviction():
    ...
    with patch("kopi_cli.approval_mode.run_approval_mode_command", return_value=result) as run:
...
async def test_gateway_rejects_non_admin_persistent_approval_change():
    ...
    with patch("kopi_cli.approval_mode.run_approval_mode_command") as run:
...
async def test_gateway_live_dispatch_routes_and_persists_approvals_command(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_MANAGED_DIR", str(tmp_path / "missing-managed"))
    from kopi_cli import managed_scope
    from kopi_cli.config import _LOAD_CONFIG_CACHE, _RAW_CONFIG_CACHE
