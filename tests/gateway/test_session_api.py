    ...
    with patch.object(auth_adapter, "_run_agent", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"Authorization": "Bearer sk-test", "X-Kopi-Session-Key": "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers["X-Kopi-Session-Id"] == session_id
    assert resp.headers["X-Kopi-Session-Key"] == "client-42"
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={"X-Kopi-Session-Key": "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert "X-Kopi-Session-Key requires API key" in data["error"]["message"]
...
async def test_run_agent_returns_controlled_response_on_provider_auth_failure(adapter, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(
            RuntimeError("No credentials found for provider 'nous' — run `kopi auth add nous`")
        ),
    )
    ...
    assert result == {
        "final_response": "⚠️ Provider authentication failed: No credentials found for provider 'nous' — run `kopi auth add nous`",
        "messages": [],
        "api_calls": 0,
        "tools": [],
    }
...
async def test_session_model_lock_endpoint_persists_and_invalidates_prompt(adapter, session_db):
    ...
    assert payload["object"] == "kopi.session.model_lock"
