...
* api_server subscriptions wake the creator's REAL session via the
  ...
  that never matches the raw X-Kopi-Session-Id session real turns use).
"""
...
def test_sendresult_failure_rewinds_cursor(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_KANBAN_DB", str(tmp_path / "softfail.db"))
    kb.init_db()
...
def test_apiserver_sub_wakes_real_session_via_self_post(tmp_path, monkeypatch):
    ...
    match the raw X-Kopi-Session-Id session)."""
    monkeypatch.setenv("KOPI_KANBAN_DB", str(tmp_path / "apiserver.db"))
    kb.init_db()
...
def test_apiserver_failed_self_post_rewinds_cursor(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_KANBAN_DB", str(tmp_path / "apiserver_fail.db"))
    kb.init_db()
    tid = _create_completed_subscription(
...
def test_apiserver_self_post_succeeds_after_earlier_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KOPI_KANBAN_DB", str(tmp_path / "apiserver_retry.db"))
    kb.init_db()
    tid = _create_completed_subscription(
