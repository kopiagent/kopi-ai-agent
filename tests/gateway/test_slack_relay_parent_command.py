...
@pytest.mark.parametrize(
    ("wire_text", "expected"),
    [
        ("/kopi sethome", "/sethome"),
        ("/kopi\tsethome", "/sethome"),
        (
            "/kopi model gpt-5.6 --provider openai",
...
def test_slack_relay_parent_freeform_text_matches_native_adapter():
    event = _event_from_wire(_wire("/kopi explain this"))
...
def test_non_slack_relay_message_is_not_rewritten():
    event = _event_from_wire(_wire("/kopi sethome", platform="discord"))

    assert event.text == "/kopi sethome"
    assert event.message_type == MessageType.COMMAND
