...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("kopi_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("kopi_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/kopi/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/kopi/profiles/from-source")
    ...
    def test_routing_wins_over_active_profile(self, mock_runner, discord_source):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("kopi_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("kopi_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/kopi/profiles/routed")
                    ...
                    assert result == Path("/kopi/profiles/routed")
    ...
    def test_active_profile_fallback(self, mock_runner, discord_source):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("kopi_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/kopi/profiles/active")
                ...
                assert result == Path("/kopi/profiles/active")
    ...
    def test_default_fallback_when_no_active(self, mock_runner, discord_source):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value=None):
            with patch("kopi_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/kopi")
                
                mock_runner._profile_name_for_source = MagicMock(return_value=None)
                
                result = mock_runner._resolve_profile_home_for_source(discord_source)
                
                assert result == Path("/kopi")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("kopi_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/kopi/profiles/nonexistent")
                with patch("kopi_cli.profiles.profile_exists", return_value=False):
                    with patch("kopi_constants.get_kopi_home", return_value=Path("/kopi")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global KOPI_HOME
                            assert result == Path("/kopi")
    ...
    def test_existing_profile_no_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("kopi_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/kopi/profiles/existing")
                with patch("kopi_cli.profiles.profile_exists", return_value=True):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
                        
                        assert result == Path("/kopi/profiles/existing")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("kopi_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("kopi_constants.get_kopi_home", return_value=Path("/kopi")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
    ...
    def test_exception_with_no_profile_name(self, mock_runner, discord_source, caplog):
        ...
        with patch("kopi_cli.profiles.get_active_profile_name", return_value=None):
            with patch("kopi_cli.profiles.get_profile_dir", side_effect=RuntimeError("Filesystem error")):
                with patch("kopi_constants.get_kopi_home", return_value=Path("/kopi")):
                    mock_runner._profile_name_for_source = MagicMock(return_value=None)
