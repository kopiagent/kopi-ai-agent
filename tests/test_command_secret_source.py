...
KOPI_HOME with a config.yaml routing ``secrets.provider: command`` through
``kopi_cli.env_loader._apply_external_secret_sources``.

Security invariants under test (ported from the desktop TS provider):

* the requested key travels ONLY via the ``KOPI_SECRET_KEY`` env var —
...
def test_dotenv_blob_helper_resolves_multiple_keys(tmp_path):
    ...
    # And the list path (KOPI_SECRET_KEY="") sees the full map.
...
def test_helper_receives_key_via_env_var(tmp_path):
    # The helper echoes KOPI_SECRET_KEY back — proving the key arrives
    # as env DATA through the real /bin/sh path.
    helper = _write_helper(tmp_path, 'printf \'%s\' "$KOPI_SECRET_KEY"')
    assert (
...
def test_hostile_key_name_is_inert_data(tmp_path):
    ...
    only inside KOPI_SECRET_KEY, never interpolated into the shell string."""
...
# ---------------------------------------------------------------------------
# Dispatch E2E through env_loader against a real temp KOPI_HOME
# ---------------------------------------------------------------------------
