"""The conftest WAL gate must agree with kopi_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``kopi_state``. That is deliberate: importing
``kopi_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.kopi`` before the per-test ``KOPI_HOME`` redirect, which makes tests
...
from kopi_state import is_sqlite_wal_reset_vulnerable
...
def test_conftest_gate_agrees_with_kopi_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and kopi_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_kopi_state_at_collection():
    """The gate must stay import-free of kopi_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.kopi, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``kopi_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "kopi_state" or name.startswith("kopi_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported kopi_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.kopi during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("kopi_state", None)
