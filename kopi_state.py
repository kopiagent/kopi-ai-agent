#!/usr/bin/env python3
"""
SQLite State Store for Kopi Agent.
...
from kopi_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from kopi_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from kopi_state_portability import SessionPortabilityMixin
from kopi_state_schema import SessionSchemaMixin
from kopi_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``kopi -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.kopi`` even when a test fixture later
    redirects ``KOPI_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_kopi_home()`` fresh so a runtime
       ``KOPI_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_kopi_home() / "state.db"
...
def _wal_reset_repair_hint() -> str:
    ...
    ``kopi update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Kopi-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Kopi"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Kopi-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `kopi doctor`. This warning fires once per "
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with kopi_cli.config.
        from kopi_cli.config import cfg_get, load_config_readonly
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``kopi_cli.sqlite_safe_read``).
    ...
    try:
        from kopi_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Kopi home tree (``get_kopi_home()``) — the safe repair scope:
      Kopi owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``kopi_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_kopi_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Kopi needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (kopi_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_kopi_state.py:600-632)
...
# (``~/.kopi/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``kopi sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("KOPI_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_kopi_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("KOPI_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship kopi_state without kopi_cli). A
    ...
    """
    try:
        from kopi_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "kopi_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch kopi_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``kopi_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from kopi_cli.backup import is_zeroed_sqlite_file
    ...
    if size <= 0:
        return False
    from kopi_cli.sqlite_safe_read import read_header_bytes_preopen
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `kopi snapshot list` / "
                "`kopi snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Kopi
    ...
    # `kopi update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `kopi snapshot list` / "
                    f"`kopi snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`kopi db optimize`),
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `kopi sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Kopi process held the "
    ...
    #
    # `optimize_fts_storage()` (the `kopi sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`kopi sessions optimize-storage`) ──
    #
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``kopi --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: kopi_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``kopi -c``/``--resume`` so the "last" session is the last one in
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``KOPI_SESSION_SOURCE``, so their runs
