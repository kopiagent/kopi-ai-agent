...
# optimized via ``kopi sessions optimize-storage``. A legacy DB sits at
...
# `kopi sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `kopi db optimize`. This is
...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `kopi sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `kopi db optimize`) must keep its EXISTING
    ...
    def search_messages(
        ...
        Threshold: KOPI_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("KOPI_SEARCH_SLOW_MS", "1000"))
            except (TypeError, ValueError):
                threshold = 1000.0
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Kopi surface and jump
