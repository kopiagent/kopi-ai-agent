...
storage discipline as the other indexes). On a populated database, run
    ...
    kopi sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.kopi/config.yaml` to disable. Override
the .so location with `KOPI_FTS5_CJK_SO`.
