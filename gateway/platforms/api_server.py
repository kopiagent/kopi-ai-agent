...
- GET  /api/sessions               — list client-visible Kopi sessions
- POST /api/sessions               — create an empty Kopi session
...
def _kopi_version() -> str:
    """Return the canonical Kopi Agent version string.

    ``kopi_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``kopi_agent-*.dist-info`` after a source update until the environment is
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from kopi_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("KOPI_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``kopi-ai-agent``)
    ...
    ``direct_model_requests`` config value here, while Kopi-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Kopi-aware and is
    always honored.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Kopi-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`kopi_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from kopi_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "kopi-ai-agent")
    ...
    def _check_auth(self, request: "web.Request") -> Optional["web.Response"]:
        ...
        if auth_header.startswith("Bearer "):
            ...
            # KOPI: X-Kopi-Api-Key fallback — used when reached through the K8s
            # API gateway, which cannot set an Authorization header.
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_kopi_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _ensure_session_db(self):
        ...
        redirects ``get_kopi_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_kopi_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "kopi_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "kopi_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        # kopi_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Kopi provider/model inventory.
        ...
        Kopi provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Kopi sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Kopi session row.
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Kopi-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Kopi-Session-Key"] = gateway_session_key
        runtime = {}
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "kopi.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Kopi-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Kopi-Error"] = _redact_api_error_text(err_msg, limit=200)
        ...
        try:
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="kopi.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Kopi-Session-Id header.  Without this,
        ...
        response_headers = {"X-Kopi-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Kopi-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_kopi_api_runtime", {}) or {})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Kopi-Relay
