...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_kopi_home() / "gateway-starts.log"
...
def _canonical_kopi_home(path: Path | str) -> Path:
    """Return a stable absolute KOPI_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_kopi_home(left: Path | str, right: Path | str) -> bool:
    """Compare KOPI_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_kopi_home(left))) == os.path.normcase(
        str(_canonical_kopi_home(right))
    )
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Kopi' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Kopi gateway lifecycle subcommand from a command line.
    ...
    ``--profile``/``-p`` selectors from anywhere in argv -- Kopi's
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Kopi-owned runtime records
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    # str(Path) renders backslashes while a KOPI_HOME= value on the argv
...
def _build_pid_record() -> dict:
    return {
        ...
        # KOPI_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "kopi_home": str(_canonical_kopi_home(_get_process_kopi_home())),
    }
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``kopi_cli.web_server`` bindings so the long-standing
...
def _get_takeover_marker_path(kopi_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``kopi_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = kopi_home or _get_process_kopi_home()
    return _canonical_kopi_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-KOPI_HOME --replace while
    ...
    our_home = _get_process_kopi_home()
    target_home = record.get("target_kopi_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_kopi_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_kopi_home")
        if replacer_home is not None and not _same_kopi_home(
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's KOPI_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_kopi_home(
            target_home or _get_process_kopi_home()
        )
        ...
        record = {
            ...
            "target_kopi_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_kopi_home": str(
                _canonical_kopi_home(_get_process_kopi_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target KOPI_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("kopi_home")
    if not isinstance(pid_record_home, str) or not _same_kopi_home(
