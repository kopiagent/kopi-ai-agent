...
2. An event-loop heartbeat file at ``<KOPI_HOME>/state/gateway.heartbeat`` so
...
def _process_kopi_home() -> Path:
    """KOPI_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<KOPI_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_kopi_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from kopi_logging import drain_log_queue
