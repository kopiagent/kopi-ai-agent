...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``kopi_cli.config.save_env_value`` / ``remove_env_value``, which target
...
class PairingStore:
    ...
    profile's own KOPI_HOME using the same legacy/consolidated layout rules
    as ``kopi -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current KOPI_HOME.
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            # ``kopi -p <profile> pairing ...`` process does. Merge the
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``kopi
