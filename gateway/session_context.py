...
_SESSION_CHAT_TYPE: ContextVar = ContextVar("KOPI_SESSION_CHAT_TYPE", default=_UNSET)
...
_CRON_SESSION: ContextVar = ContextVar("KOPI_CRON_SESSION", default=_UNSET)
...
_VAR_MAP = {
    ...
    "KOPI_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    ...
    "KOPI_CRON_SESSION": _CRON_SESSION,
...
def set_current_session_id(session_id: str) -> None:
    ...
    ``KOPI_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    # parent's session identity. See KopiPRDelegationSessionContext task.
    try:
...
def set_session_vars(
    ...
    ``os.environ["KOPI_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
# value (``telegram``) to KOPI_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind KOPI_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# KOPI_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``KOPI_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("KOPI_PLATFORM") or get_session_env("KOPI_SESSION_PLATFORM", "")
...
def declare_stateless_channel() -> None:
    ...
    See NousResearch/kopi-ai-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``kopi -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``KOPI_KANBAN_TASK``), which are one-shot
