...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Kopi commands.
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Kopi Agent version."""
        from kopi_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _kopi_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from kopi_cli.route_identity import should_clear_context_pin_async
                        ...
                        from kopi_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Kopi default runtime
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Kopi itself changed. ``--stat`` limits output to the summary.
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Kopi Agent to the latest version.
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Kopi Agent')}"
