...
`kopi config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.kopi/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `KOPI_HOME`, secret scope, and
