...
Kopi gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Kopi-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `kopi.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `kopi.platform.up/degraded` with bounded `error_code` attributes |
...
`kopi.gateway.active_agents`, `kopi.gateway.background_work`, and
`kopi.gateway.background_delegations` are complementary. `active_agents`
...
```bash
kopi monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'kopi-ai-agent[otlp]'`),
...
a plain `kopi gateway run`: the exporter lives in the gateway process, so
...
`kopi.gateway.up`, `kopi.platform.up`, and `kopi.platform.degraded`.
...
absent_over_time(kopi_gateway_up[5m])

# Locally owned bridge is explicitly down.
kopi_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
kopi_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
kopi_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
kopi_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(kopi_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `kopi.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
kopi.status = failed|unknown
kopi.delivery_outcome = failed|not_configured
kopi.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `kopi.job_key`;
...
Kopi Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/kopi_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/kopi_otel_capture.jsonl --wait 8
...
`kopi.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
kopi monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `kopi monitoring` CLI intentionally exposes `status` only. This first
release covers only Kopi Agent-owned service-health and operational-diagnostic
signals, including Kopi Agent-owned Relay transport health. Team Gateway's
