...
Kopi observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Kopi' planner, model providers, memory, tool registry,
...
Kopi also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Kopi catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Kopi only builds
