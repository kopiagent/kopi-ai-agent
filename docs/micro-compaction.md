...
has to be thrown away or summarized. Kopi has always done this in one batch:
...
Kopi folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Kopi's normal session flush is
...
With `micro_compact` unset or `false` Kopi behaves exactly as it always has:
...
Defaults to `$KOPI_HOME/logs/agent.log`. It reports passes, outcome mix, net
