...
<url>``) is spawned as a direct child of the Kopi process. Kopi's own
...
Kopi process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Kopi startup's opt-in
