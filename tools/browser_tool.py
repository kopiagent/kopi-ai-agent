...
# means a compromised transitive dependency could read every Kopi secret
...
def _build_browser_env() -> dict:
    ...
    Strips Kopi-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``kopi doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Kopi
