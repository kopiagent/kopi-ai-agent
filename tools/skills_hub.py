...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/kopi-ai-agent"
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``kopi_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _load_kopi_index() -> Optional[dict]:
    ...
    if kopi_index_cache_file.exists():
        try:
            ...
            if age < KOPI_INDEX_TTL:
                return json.loads(kopi_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Kopi index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    try:
        ...
        kopi_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
