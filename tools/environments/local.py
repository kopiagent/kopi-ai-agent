...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Kopi was already launched from (for example ``kopi-ai-agent``
    while the process cwd is ``~/.kopi/kopi-ai-agent``), passing it through
    unchanged makes the wrapper run ``cd kopi-ai-agent`` *inside* the project
    ...
    # Common recovery for config values like ``kopi-ai-agent`` when Kopi was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./kopi-ai-agent``; use the current directory instead.
...
# Kopi-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Kopi inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Kopi-managed inference credential — Claude subscription auth is not a
    # working Kopi provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Kopi venv path instead of the project's own
# ``.venv`` — silently clobbering the Kopi environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Kopi venv stays reachable via PATH (its bin dir is first), so stripping
...
def _is_kopi_internal_secret(key: str) -> bool:
    """Return True for Kopi-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Kopi-internal dynamic secret"
...
def _inject_context_kopi_home(env: dict) -> None:
    """Bridge the context-local Kopi home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        ...
        if key in _KOPI_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
...
# legitimate child Kopi spawns needs them, and they are the highest-value
...
def kopi_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Kopi spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_KOPI_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Kopi-internal dynamic secrets
    ...
    # still see the parent's KOPI_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`kopi_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``KOPI_HOME`` / subprocess ``HOME`` contract) and the
    Kopi secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_kopi_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``KOPI_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Kopi home override into ``KOPI_HOME`` and apply the
      subprocess HOME contract (``kopi_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``KOPI_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs KOPI_HOME override
...
def _find_bash() -> str:
    ...
    # system Git (or a stale KOPI_GIT_BASH_PATH pointing at one) must not
    ...
    # KOPI_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\kopi\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "KOPI_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Kopi Agent requires Git for Windows on Windows.\n"
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Kopi. If the override is blocked or later re-applied, "
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Kopi-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Kopi installs for
    itself, so on a machine where Kopi provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$KOPI_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$KOPI_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_kopi_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_kopi_managed_node``, a first browser install).
    """
    try:
        from kopi_constants import get_kopi_home, iter_kopi_node_dirs

        candidates = [*iter_kopi_node_dirs(), get_kopi_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Kopi-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Kopi
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_KOPI_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if k in _KOPI_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
