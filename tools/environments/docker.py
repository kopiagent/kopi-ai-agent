...
def _get_active_profile_name() -> str:
    """Return the active Kopi profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Kopi process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Kopi
    process leaves its container behind permanently because there's no
    subsequent Kopi process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``KOPI_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `kopi egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `kopi egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `kopi egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `kopi egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/kopi-egress-ca.crt"
    ...
    env_overrides: dict[str, str] = {
        ...
        "_KOPI_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca",
    }
    ...
    # through the proxy.  Keep the KOPI_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"KOPI_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if egress_env_overrides:
            try:
                from kopi_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from kopi_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_KOPI_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_KOPI_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # _sanitize_label_value(); the active Kopi profile is captured at
        ...
        # container shared across sessions").  If a prior Kopi process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # keys are filtered. Also strip Kopi-internal dynamic secrets
        ...
        for key in sorted(forward_keys):
            value = os.getenv(key) or kopi_env.get(key)
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # kopi-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "kopi egress disable",
        ...
        # container) but can happen if a previous Kopi process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Kopi exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Kopi process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Kopi startup. That covers the
        ...
        # outlives Kopi processes, processes inside it stay alive, and
