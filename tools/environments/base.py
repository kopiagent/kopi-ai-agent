...
# the FIRST session's KOPI_SESSION_ID into the snapshot makes every LATER
...
# KOPI_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Kopi' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is KOPI_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (KOPI_SESSION_|KOPI_UI_SESSION_ID|KOPI_CRON_AUTO_DELIVER_|KOPI_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``KOPI_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!KOPI_SESSION_*} ${!KOPI_CRON_AUTO_DELIVER_*} "
        f"KOPI_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__kopi_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__kopi_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_KOPI_RUNTIME_PASSTHROUGH_{name}"
        ...
        # Restrict Kopi metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__kopi_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_kopi_home() / "cache" / "terminal-output"
