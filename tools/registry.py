...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (kopi_constants itself is stdlib-only, so no cycle).
        from kopi_constants import get_kopi_home

        return Path(get_kopi_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Kopi-home override for every
