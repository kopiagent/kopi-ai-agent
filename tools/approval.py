...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("KOPI_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("KOPI_CRON_SESSION")
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_kopi_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Kopi: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bkopi\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart kopi gateway (kills running agents)"),
    ...
    # a lifecycle command past the guard — same treatment as the `kopi ...
    ...
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(kopi|ai\.kopi)\b', "stop/restart kopi launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Kopi file.
    ...
    # In-place edit of a Kopi-managed security file (~/.kopi/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_KOPI_CONFIG_PATH}|{_KOPI_ENV_PATH})', "in-place edit of Kopi config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_KOPI_CONFIG_PATH}|{_KOPI_ENV_PATH})', "in-place edit of Kopi config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_KOPI_CONFIG_PATH}|{_KOPI_ENV_PATH})', "in-place edit of Kopi config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    #
    # Fold the (more specific) Kopi home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\kopi), so folding the user home
    # first would eat the prefix the Kopi-home fold needs.
...
def _rewrite_resolved_kopi_home(command: str) -> str:
    """Rewrite the resolved absolute Kopi home prefix to ``~/.kopi/``.
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``kopi send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Kopi ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"kopi-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Kopi YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Kopi approval prompts.
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from kopi_cli.config import load_config_readonly as _load_cfg
