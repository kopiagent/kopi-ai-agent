...
Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    ...
    connection. Kopi-owned direct httpx request paths should use
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for Kopi-owned URL tools.
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("kopi_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("kopi_ssrf_origin_schemes")
...
def _install_ssrf_guard_on_async_transport(transport: Any, schemes_by_origin_var: Any) -> None:
    ...
    if transport is None or state.get("_kopi_ssrf_guarded", False):
        return
    ...
    transport._kopi_ssrf_guarded = True
