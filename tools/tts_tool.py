...
# Base URL now resolved via kopi_cli.models.deepinfra_base_url (shared).
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``kopi tools``); otherwise the historical Edge backend remains active.
    """
...
# file, so retired models disappear from kopi the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from kopi_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import kopi_cli as _kopi_cli

            _kopi_version = str(_kopi_cli.__version__)
        except Exception:
            _kopi_version = "0.0.0"
        # Include Kopi client context following Gemini's partner
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `kopi setup` to install Mistral support."
                }, ensure_ascii=False)
