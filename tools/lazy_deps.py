"""
Lazy dependency installer for opt-in Kopi Agent backends.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Kopi, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # presence, the `kopi update` lazy-refresh pass re-asserts THIS pin on
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Kopi under WSL to use Matrix on Windows."
        )
    return None
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv Kopi installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Kopi
    ...
    # deployment sets KOPI_MANAGED=true *and* KOPI_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from kopi_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Kopi cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Kopi instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``KOPI_DISABLE_LAZY_INSTALLS=1``) and sets
      ``KOPI_LAZY_INSTALL_TARGET``, installs are redirected to the writable
