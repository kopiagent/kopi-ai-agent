...
# Per-Kopi-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/kopi-ai-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Kopi's explicit approval bypass onto Cua's immutable mode.

    Kopi has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown KOPI_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Kopi YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Kopi sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Kopi process that spawned it
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/kopi-ai-agent#67052).
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Kopi sessions.
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/kopi-ai-agent#47072 — trycua/cua#1961
