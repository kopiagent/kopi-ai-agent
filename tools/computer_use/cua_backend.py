...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/kopi-ai-agent#67052.
    """
...
_CUA_DRIVER_CMD_ENV = "KOPI_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Kopi should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Kopi should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Kopi runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Kopi process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Kopi session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\kopi-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/kopi-ai-agent#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Kopi an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``KOPI_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Kopi Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `kopi computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``KOPI_CUA_DRIVER_CMD``) is never silently
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/kopi-ai-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/kopi-ai-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Kopi-managed secrets out of the child env.
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/kopi-ai-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/kopi-ai-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Kopi run, with
        ...
        #     concurrent Kopi runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Kopi run = one backend = one session — and pass it as
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Kopi' minimal core.
    ...
    def capture(
        ...
        # Surface 3 of NousResearch/kopi-ai-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/kopi-ai-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/kopi-ai-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/kopi-ai-agent#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Kopi adapter, and uses cua-driver's
