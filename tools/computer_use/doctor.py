...
flow through here without code changes on the Kopi side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Kopi provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Kopi-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `KOPI_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Kopi identity under kopi_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["kopi_identity"] = identity
