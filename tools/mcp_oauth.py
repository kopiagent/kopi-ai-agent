    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Kopi Agent"
"""
...
def _get_token_dir(kopi_home: str | Path | None = None) -> Path:
    ...
    base = Path(kopi_home) if kopi_home is not None else Path(get_kopi_home())
...
def _cached_redirect_port(storage: "KopiTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Kopi restarts and chooses a
...
def _cached_redirect_uri(storage: "KopiTokenStorage | None") -> str | None:
...
class KopiTokenStorage:
    ...
    def __init__(self, server_name: str, *, kopi_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._kopi_home = Path(kopi_home) if kopi_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._kopi_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._kopi_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._kopi_home) / f"{self._server_name}.meta.json"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._kopi_home)
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = KopiTokenStorage(server_name, kopi_home=kopi_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "KopiTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Kopi Agent" / "Kopi" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Kopi Agent")
    scope = cfg.get("scope")
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Kopi auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Kopi is running.
    """
    msg = str(exc)
    lowered = msg.lower()
