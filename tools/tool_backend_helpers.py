...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``kopi auth add
    ...
    2. The environment / ``~/.kopi/.env``. Under a multiplexed gateway turn
       ...
       ``kopi_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``kopi auth
    ...
    when omitted, ``kopi_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `kopi auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``kopi auth add openai-api``) via
