...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``kopi-cli`` because those also contain
...
def _build_child_agent(
    ...
    # Blocked tools also live inside mixed platform bundles (kopi-cli,
    # kopi-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Kopi/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from kopi_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # KOPI_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Kopi-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Kopi-Session-Id once the batch completes.
            ...
            # KOPI_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `kopi -z`, a cron job, a Kanban "
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and KOPI_SESSION_KEY is not in the CLI
