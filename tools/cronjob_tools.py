...
# uses 30s) — comfortably below the 1800s default KOPI_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (KOPI_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with KOPI_CRON_TIMEOUT=0
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `kopi cron create/edit --model`, or hand-edited jobs). The agent
