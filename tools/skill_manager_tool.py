...
def _background_review_write_guard(
    ...
    try:
        ...
        # shapes; `kopi curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `kopi curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `kopi sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `kopi sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `kopi sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `kopi sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
