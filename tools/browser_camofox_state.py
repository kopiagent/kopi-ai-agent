"""Kopi-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Kopi
...
def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Kopi-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Kopi profile = same userId).
