...
409), and is driven by:
  ...
  * the ``kopi sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``kopi_cli/``) so the
...
is toggled via ``kopi sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.kopi/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``kopi sync enable`` — it is
...
# namespace from kopi-ai-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the KOPI_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: KOPI_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("KOPI_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Kopi Cloud instance can be set
...
# precedence as base_url: the KOPI_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   KOPI_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   KOPI_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   KOPI_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``KOPI_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Kopi Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("KOPI_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``KOPI_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``kopi sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "KOPI_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``KOPI_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``kopi sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Kopi Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("KOPI_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.kopi/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.kopi/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Kopi Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``KOPI_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.kopi/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``kopi sync device --name`` to set an explicit
    label."""
    ...
    # Kopi Cloud (and any templated deployment) can seed the label
    # declaratively via KOPI_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `kopi sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("KOPI_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.kopi/skills/.sync_device_id, overwriting
...
# ~/.kopi/skills/.sync_state as JSON.
#
...
# path under ~/.kopi/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.kopi/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Kopi speaks "
            f"{WIRE_VERSION} — update Kopi to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (kopi sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.kopi/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "kopi skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (KOPI_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``kopi sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.kopi/skills/_org/
...
#
# `kopi sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.kopi/skills/_org/<org_id>/``.
