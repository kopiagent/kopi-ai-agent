"""
Configuration management for Kopi Agent.
...
- kopi config get      - Print a resolved configuration value
- kopi config set      - Set a specific value
- kopi config unset    - Remove a user configuration value
...
from kopi_cli.route_identity import normalize_route_base_url
...
#   the next ``subprocess.run([...])`` Kopi makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Kopi itself starts
...
#   ``KOPI_ENV`` — Kopi runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Kopi runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Kopi runtime location "
...
# from `nix run` / `nix profile install` (which don't set KOPI_MANAGED).
...
_NIX_UPDATE_MSG = (
    "Update Kopi through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/kopi`` inside the published image. It is a property of
    the running interpreter, NOT of ``$KOPI_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Kopi was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
    ...
    3. KOPI_MANAGED env / .managed marker (NixOS managed mode)
    ...
    # detect Nix installs that don't set KOPI_MANAGED (e.g. ``nix run``,
...
#     — that script installs a *new* host-side Kopi, it doesn't update
...
Kopi Agent runs as a published image (nousresearch/kopi-ai-agent), not a
...
from kopi_constants import get_kopi_home, get_process_kopi_home  # noqa: F811,E402
...
def _resolve_kopi_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Kopi commonly set these to map the in-container
...
# ensure_kopi_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_KOPI_HOME_ENSURED: set = set()
...
def ensure_kopi_home():
    ...
    After the first successful pass for a given ``KOPI_HOME`` we only re-run
    ...
    ``get_kopi_home()`` and therefore re-run for the new path.
    """
    home = get_kopi_home()
    key = str(home)

    if key in _KOPI_HOME_ENSURED and home.is_dir():
        return
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # prompt's environment-hints block. Lets a host that wraps Kopi
        ...
        # app, ACP) in a code workspace, Kopi adds a coding operating brief
        ...
        # ``KOPI_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bundled Kopi image, which drops to the `kopi` user via
    ...
    "browser": {
        ...
        "camofox": {
            ...
            # app owns the visible browser and Kopi should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    "checkpoints": {
        ...
        # ``kopi checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .kopi.md, or .cursorrules before Kopi applies
    ...
    "display": {
        ...
        # model — see kopi_cli/focus_view.py.
        "focus_view": False,
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Kopi appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # "Hey Kopi" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey kopi",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_kopi" (the bundled, works-out-of-the-box default) OR a
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "☕ *Kopi Agent*" header.
    ...
    # replace Kopi' built-in platform hint for a single messaging platform
    ...
    "cron": {
        ...
        # KOPI_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    #
    # Core Kopi tools (terminal, read_file, write_file, patch,
    ...
    "gateway": {
        ...
        # ``KOPI_GATEWAY_MAX_STARTS`` / ``KOPI_GATEWAY_START_WINDOW_S``
        ...
        # behavior: files must live under the Kopi cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Kopi
    ...
    "sessions": {
        ...
        # the default of ``kopi sessions prune``.
        "retention_days": 90,
        ...
        # user runs `kopi sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `kopi update` prints a one-line notice with
        ...
        # ~/.kopi/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to KOPI_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # KOPI_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <KOPI_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``kopi backup``-style zip
        #     of KOPI_HOME into <KOPI_HOME>/backups/, restorable with
        #     ``kopi import``. Can add minutes on large homes. This is the
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # material under ~/.kopi/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Kopi may use this encrypted
    ...
    "computer_use": {
        ...
        # by default upstream. Kopi disables it for our users unless they
        # explicitly opt in here. When false (default), Kopi sets
    ...
    #
    # Configure with `kopi egress setup`.  Disabled by default — the rest of
    # Kopi works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.kopi/bin/ on
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Kopi mints short-lived tokens from. Falls back to "
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Kopi, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `kopi config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Kopi' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Kopi versions) may write
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (kopi_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (kopi_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (kopi_cli/setup.py)
    "known_plugin_toolsets", # written/read by kopi_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, kopi send) so
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.kopi/.env); "
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Kopi writes itself."""
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``KOPI_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"KOPI_CUSTOM_{slug}_API_KEY" if slug else "KOPI_CUSTOM_API_KEY"
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for Kopi-managed credentials where a deliberate edit to ``.env``
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `kopi cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``kopi config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``kopi config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``kopi config set --force``.
    """
    if is_managed():
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_kopi_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `kopi config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: kopi config get <key> [--json]")
            print()
            print("Examples:")
            print("  kopi config get model")
            print("  kopi config get terminal.backend")
            print("  kopi config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: kopi config set [--force] <key> <value>")
            print()
            print("Examples:")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: kopi config unset <key>")
            print()
            print("Examples:")
            print("  kopi config unset model")
            print("  kopi config unset terminal.backend")
            print("  kopi config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        print("  kopi config get <key>          Print a resolved config value")
        print("  kopi config set <key> <value>   Set a config value")
        print("  kopi config unset <key>        Remove a config value")
