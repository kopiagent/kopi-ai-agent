...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # KOPI_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # KOPI_TOOL_PROGRESS variant is fully unsupported since the v12 config
    ...
    "KOPI_ACP_AUTH_METHOD",
    "KOPI_ACP_AUTO_APPROVE",
    "KOPI_COPILOT_ACP_COMMAND",
    "KOPI_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from kopi_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from kopi_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ kopi config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in kopi_cli.config_migrations as a
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_kopi_home``, tui profile override, multi-profile probes).
    """
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.kopi/.env``, scope-aware.
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``kopi config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (kopi config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``kopi config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``kopi config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    kopi config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    kopi config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
