"""On-demand supply-chain audit for Kopi Agent installs.

Scans three surfaces a Kopi user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Kopi venv (every PyPI dist via ``importlib.metadata``).
