"""Kopi update pipeline — extracted from ``kopi_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``kopi_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``kopi_cli.main`` reference — so existing call sites and test monkeypatches
that target ``kopi_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``kopi_cli.main``.
...
Imports are one-way: ``kopi_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``kopi_cli.main`` reference.

    Lets callers keep patching ``kopi_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``kopi_cli.main`` -> ``kopi_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``kopi update`` can be driven by a
    ...
    """
    from kopi_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from kopi_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('kopi_'):\n"
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``kopi_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `kopi update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``kopi_constants.py``
...
def _update_via_zip(args):
    ...
    try:
        ...
        print(
            "  Re-run `kopi update` to retry; if the agent won't start, "
            "reinstall from https://kopi-ai-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    if not import_ok:
        ...
        print("  Re-run `kopi update` to complete it.")
        _m().sys.exit(1)
...
def _update_node_dependencies() -> list[str]:
    ...
    if not _m()._npm_lockfile_changed(shared_kopi_root):
    ...
    nixos_env = with_kopi_node_path(_m()._nixos_build_env())
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".kopi-bootstrap-complete",
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``kopi update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from kopi_cli._scan_venv_blockers import _is_pausable_gateway
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``kopi update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_kopi_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        if not import_ok:
            ...
            print("    Run `kopi update` again — if it persists, reinstall:")
            print("    https://kopi-ai-agent.nousresearch.com")
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `kopi desktop --build-only` subprocess re-imports the whole
        ...
        try:
            ...
            try:
                from kopi_cli.gateway import _get_restart_exit_wait_budget
