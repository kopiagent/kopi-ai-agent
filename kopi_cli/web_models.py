"""Pydantic request/response models for the Kopi dashboard web server.

Extracted verbatim from ``kopi_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from kopi_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under KOPI_HOME.
    output: str = ""
