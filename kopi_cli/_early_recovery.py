"""Dependency-light venv recovery that runs BEFORE kopi_cli.main's imports.

The ``kopi`` console entry point is ``kopi_cli.main:main``.  Importing
``kopi_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``kopi_cli.env_loader``, ``yaml`` via ``kopi_cli.config``, ...).  In
...
a corrupted venv.  ``kopi_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``kopi_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``kopi acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``kopi_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``kopi_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``kopi update`` exists AND an import probe
