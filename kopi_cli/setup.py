"""
Interactive setup wizard for Kopi Agent.
...
def _print_setup_summary(config: dict, kopi_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Kopi cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    kopi model            (pick any provider/model)")
        print_info("    kopi setup --portal   (Kopi Official OAuth, no API key)")
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'kopi tools' → Speech-to-Text")
            )
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Kopi auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `kopi egress setup` then `kopi egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `kopi egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'kopi-ai-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $KOPI_HOME/bin is never on PATH, so a bare
            # which() misses the uv Kopi installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'kopi-ai-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Kopi profile and are not uploaded.")
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Kopi Official setup — OAuth + model pick + provider + Tool Gateway.
    ...
    Kopi session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ☕ Kopi Setup — Kopi Official (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Kopi Official sub.")
    print()
    ...
    try:
        ...
        print()
        print_error(f"  Kopi Official setup encountered an error: {exc}")
        print_info("  You can retry later with `kopi portal`.")
        return
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ...
      kopi setup telemetry — just local shared metrics
    ...
    # --portal: one-shot Kopi Official setup. Skips the rest of the wizard.
    ...
    print(
        color(
            "│  Let's configure your Kopi Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Kopi?",
            [
                "Quick Setup (Kopi Official) — free OAuth login, no API keys, model + tools (recommended)",
...
def _run_first_time_quick_setup(config: dict, kopi_home, is_existing: bool):
    """Streamlined first-time setup via Kopi Official: OAuth, model, terminal & messaging.

    Routes straight to the Kopi Official provider — runs the device-code OAuth
    ...
    # Step 1: Kopi Official — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nous model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("Kopi Official")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.nousresearch.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Kopi Official setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"Kopi Official setup encountered an error: {exc}")
        print_info("You can try again later with: kopi model")
