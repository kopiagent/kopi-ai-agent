...
from kopi_constants import get_kopi_home, with_kopi_node_path
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Kopi only upgrades npm inside its own managed Node install, so this\n"
...
def maybe_repair_npm_engine(
    ...
    *,
    ...
    """Repair an ``EBADENGINE`` failure when Kopi owns the npm involved.
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    not an engine failure, a Node (not npm) mismatch, an npm Kopi does not
