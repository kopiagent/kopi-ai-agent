...
from kopi_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``kopi_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``kopi_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("wake", "Toggle the 'Hey Kopi' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    ...
    CommandDef("update", "Update Kopi Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Kopi Agent version", "Info", aliases=("v",),
...
# Telegram allows up to 100 BotCommands. Kopi ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Kopi
...
#   - topup: the billing/balance surface; reached via /kopi topup on Slack.
...
#     rare from Slack; reachable as /kopi init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /kopi version on
...
#   - diff: git working-tree diff; reached via /kopi diff on Slack so it
...
#     /kopi update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_KOPI_ONLY = frozenset({"topup", "moa", "debug", "egress", "balance", "init", "version", "diff", "update"})
