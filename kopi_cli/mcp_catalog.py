...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.kopi/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Kopi install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Kopi understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Kopi is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Kopi
        understands. Update Kopi to install this entry.
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from kopi_cli.mcp_config import _bearer_auth_headers
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    print(color(
        ...
        f"Start a new Kopi session to load its tools.",
        Colors.GREEN,
    ))
