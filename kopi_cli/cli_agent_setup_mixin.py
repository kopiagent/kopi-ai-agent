"""Agent-construction and session-resume display methods for ``KopiCLI``.
...
Behavior-neutral: every method is lifted verbatim from ``KopiCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``KopiCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'kopi model' to choose a provider, or "
                      "'kopi setup' for first-time setup.")
                return False
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``kopi model`` (which fronts Quick Setup / Nous
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'kopi model' or 'kopi setup' any time.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'kopi model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        from kopi_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
