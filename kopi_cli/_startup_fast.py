...
This module is imported by ``kopi_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no kopi_cli.config,
...
``kopi_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(kopi_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect KOPI_HOME?"""
    active_profile = os.path.join(kopi_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    kopi_home = os.environ.get("KOPI_HOME", "").strip()
    if kopi_home:
        return kopi_home
    return os.path.join(os.path.expanduser("~"), ".kopi")
...
def container_mode_may_be_active() -> bool:
    ...
    kopi_home = os.environ.get("KOPI_HOME", "").strip()
    if kopi_home:
        if os.path.exists(os.path.join(kopi_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(kopi_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(kopi_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".kopi")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect KOPI_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    print("Run 'kopi version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``kopi --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the KOPI_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("KOPI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
