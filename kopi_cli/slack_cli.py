...
def _build_full_manifest(
    ...
    By default, this keeps Kopi on Slack's older Assistant messaging
...
def slack_manifest_command(args) -> int:
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "kopi slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"kopi slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "kopi slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "kopi slack manifest: long description must be at most "
