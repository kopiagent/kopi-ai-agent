...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Kopi backup slot."""
...
@contextmanager
def _backup_operation_lock(kopi_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = kopi_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Kopi backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
# to the user as a hung `kopi update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # kopi_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    ...
    try:
        with _backup_operation_lock(kopi_root):
            _run_backup_locked(args, kopi_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, kopi_root: Path) -> None:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: kopi profile list  (after installing kopi)")
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: kopi snapshot list"
        )
        logger.error(
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        kopi_home: Override for the Kopi home directory (tests).
...
def _write_full_zip_backup(out_path: Path, kopi_root: Path) -> Optional[Path]:
    ...
    try:
        with _backup_operation_lock(kopi_root):
            return _write_full_zip_backup_locked(out_path, kopi_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, kopi_root: Path) -> Optional[Path]:
