...
# Two lines of inline path math so ``python kopi_cli/main.py`` (script
# mode — sys.path[0] is kopi_cli/, not the repo root) can import the
# canonical helpers; everything else lives in kopi_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from kopi_cli import _startup_fast  # noqa: E402
...
def _active_profile_may_override_home_fast(kopi_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(kopi_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``kopi --version`` before config/logging imports."""
...
from kopi_cli.sessions_cmd import cmd_sessions  # noqa: F401
...
from kopi_cli.subcommands.sync import build_sync_parser
...
def _relative_time(ts) -> str:
    ...
    in :mod:`kopi_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from kopi_cli.timefmt import relative_time
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            ...
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $KOPI_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Kopi installed,
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_kopi_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Kopi-managed
            ...
            from kopi_cli.npm_engine import maybe_repair_npm_engine
...
def _launch_tui(
    ...
    # TUI child is a kopi process: propagate the profile-home contract via
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.kopi/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `kopi sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set KOPI_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "KOPI_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="kopi sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="kopi sync now")
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from kopi_cli.slash_exec import CommandContext, execute_command
...
# Update pipeline extracted to kopi_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (kopi_cli.main._cmd_update_impl,
# kopi_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from kopi_cli.update_cmd import (  # noqa: F401
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Kopi-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_kopi_node_path(run_env)["PATH"]
...
def cmd_gui(args: argparse.Namespace):
    ...
    env = with_kopi_node_path()
    # KOPI: ELECTRON_RUN_AS_NODE is useful for Electron helper scripts but toxic
    # for launching the app: it makes the Electron binary execute main.ts as
    # plain Node, so the main process cannot access the electron runtime module.
    ...
    if getattr(args, "kopi_root", None):
        ...
        # KOPI: `kopi desktop` from a checkout builds a local packaged app whose
        # stamp may point at an unpublished/dirty commit. Keep the backend on the
        # same checkout instead of bootstrapping the packaged app from GitHub raw.
...
# Dashboard process-hygiene helpers extracted to kopi_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on kopi_cli.main.<name> keep resolving unchanged.
from kopi_cli.dashboard_procs import (  # noqa: F401
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from kopi_constants import venv_bin_dir
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from kopi_constants import venv_python_path
...
def cmd_update(args):
    ...
    from kopi_cli.update_lock import (
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under $HOME/.kopi/desktop-ssh: a
    # literal "~/.kopi/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of KOPI_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_kopi_home(): a non-default sticky profile (or any KOPI_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_kopi_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".kopi" / "desktop-ssh"
    try:
...
def cmd_dashboard(args):
    ...
    if (
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: KOPI_HOME is explicitly pinned to the
...
def main():
    ...
    # cmd_sessions lives in kopi_cli/sessions_cmd.py (main.py decomposition).
