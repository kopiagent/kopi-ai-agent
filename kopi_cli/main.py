...
from kopi_cli._subprocess_compat import suppress_platform_ver_console
...
# a prior ``kopi update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from kopi_cli.env_loader import ...`` / ``from kopi_cli.config import
...
# package directory, so if IT can't import, nothing else in kopi_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from kopi_cli import _early_recovery as _early_recovery_mod
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``KOPI_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``kopi … chat -q`` with stdio on a pipe. This
...
def _print_fast_version_info() -> None:
    ...
    print(f"Kopi Agent v{__version__} ({__release_date__})")
...
from kopi_cli.subcommands.approvals import build_approvals_parser
...
from kopi_cli.subcommands.import_agent import build_import_agent_parser
...
from kopi_cli.subcommands.monitoring import build_monitoring_parser
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Kopi' own profile selector.
        """
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``kopi -c`` from repo A continues repo A's last session rather than the
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Kopi checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `kopi update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Kopi checkout, run `git restore -- ui-tui`\n"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # kopi_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("KOPI_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["KOPI_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("KOPI_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["KOPI_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("KOPI_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["KOPI_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["KOPI_PYTHON"] = sys.executable
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Kopi — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Kopi still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``kopi … chat -q`` with stdout
...
def select_provider_and_model(args=None):
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``kopi model``
    ...
    # matching against kopi_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``kopi model`` flow, then route aux tasks
    to them here.
    """
    from kopi_cli.config import load_config
    from kopi_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Kopi URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def cmd_approvals(args):
    """Dispatch `kopi approvals <subcmd>`."""
    from kopi_cli.approvals_suggest import approvals_command
...
def cmd_uninstall(args):
    """Uninstall Kopi Agent (or just the Chat GUI with --gui)."""
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``kopi update`` keeps running in the pre-pull Python process. After a
...
# validated against. Lives next to the checkout (NOT in KOPI_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``kopi update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``kopi update``
    ...
    ``kopi`` entry point compares the checkout fingerprint (cheap file
...
# Critical files that Kopi must be able to import immediately after an
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``kopi update``
    ...
    ``kopi_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $KOPI_HOME."""
    from kopi_constants import get_kopi_home
    return get_kopi_home() / "web-ui-build-stamp.json"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "kopi_cli" / "web_dist" / "index.html"
    try:
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Kopi.app/Contents/MacOS/Kopi"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Kopi.exe",
            release_dir / "win-ia32-unpacked" / "Kopi.exe",
            release_dir / "win-arm64-unpacked" / "Kopi.exe",
        ]
    else:
        candidates = [
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Kopi.exe
...
#
# The desktop self-update chain (Desktop → kopi-setup --update →
# `kopi update` → `kopi desktop --build-only` → relaunch) rebuilds
# Kopi.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Kopi.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Kopi.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 kopi-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Kopi.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Kopi.exe from backup.")
        print("    Your existing version was kept and still works. Run `kopi desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `kopi desktop --force-build` to rebuild, or re-run the Kopi")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Kopi'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Kopi.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Kopi.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Kopi.exe: Access is
    ...
    an unrelated "Kopi" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Kopi.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Kopi is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Kopi.app/Contents/MacOS/Kopi  ->  app bundle = .../Kopi.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Kopi Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Kopi Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Kopi-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → kopi-setup →
            # kopi update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `kopi update` path).
            ...
            if not source_mode:
                ...
                # holds Kopi.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Kopi.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Kopi.exe, close any")
                    print("  running Kopi desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Kopi is
                ...
                # success on a Kopi.exe Windows cannot load (truncated PE from
    ...
    if source_mode:
        print("→ Launching Kopi Desktop from source build...")
    ...
    print(f"→ Launching packaged Kopi Desktop: {' '.join(launch_command)}")
...
def _scan_dashboard_processes(
    *,
    ...
    ``kopi-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Kopi Desktop Electron app to protect its own
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `kopi update`.
    ...
    try:
        ...
        return
    db_path = get_kopi_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `kopi sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    kopi sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  kopi sessions optimize-storage")
    print(
...
_DASHBOARD_SYSTEMD_UNIT = "kopi-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Kopi installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``kopi-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/kopi-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but kopi launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``kopi update``.
    ...
    log_path = get_kopi_home() / "logs" / "dashboard-restart.log"
    try:
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``kopi dashboard`` / ``kopi serve`` processes.
    ...
    When ``restart_managed`` is true (the ``kopi update`` path), a detected
    ``kopi-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``kopi-serve.service``) has its owning unit
    ...
    # When the Kopi Desktop Electron app spawns this dashboard as a
    ...
    # caller asked for restarts (the `kopi update` path) — `--stop` must
    ...
    #    remote backend (kopi serve) under Restart=on-failure never comes
...
def _update_via_zip(args):
    """Update Kopi Agent by downloading a ZIP archive.
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Kopi couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Kopi couldn't drop "
...
# Install-scoped breadcrumbs live next to the venv (not under $KOPI_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``kopi update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``kopi.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_kopi_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from kopi.exe; applying package-only first aid, "
    ...
    try:
        ...
        if self_locked:
            print(
                "  Kopi is still running from the launcher that needs "
                "replacing. Close other Kopi windows, restart from a "
...
def _windows_running_kopi_launcher_locked() -> bool:
    """True when a venv ``kopi*.exe`` shim is this process or an ancestor.
...
def _detect_concurrent_kopi_instances(
    ...
    The Kopi Desktop Electron app spawns ``kopi.EXE`` as a backend child,
    ...
    #      kopi.exe sitting *under* a non-Kopi parent (e.g. a Kopi
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Kopi Desktop, exit any open `kopi` REPLs, and")
...
def _quarantine_running_kopi_exe(
    ...
    handles (recovers in <1s), or the Kopi Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Kopi Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _kopi_exe_shims(scripts_dir):
        ...
        print(
            "    Close Kopi Desktop, exit other `kopi` REPLs, stop the "
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``kopi.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``kopi.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `kopi update`:")
    print(
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `kopi update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `kopi update` once the upstream issue is resolved.")
        return True
    if status == "indeterminate":
        print(
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Kopi' editable-install extras or
    ...
    try:
        from kopi_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `kopi update` is exactly the step
...
def _npm_lockfile_changed(kopi_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `kopi
    ...
    try:
        ...
        cache_file = kopi_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(kopi_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `kopi update`.")
            failed = ["repo root"]
            if any(
    ...
    # Kopi profile using this checkout. Keep one per-checkout cache under the
    # shared Kopi root rather than rerunning npm once per named profile.
    shared_kopi_root = get_default_kopi_root()
    if not _npm_lockfile_changed(shared_kopi_root):
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_kopi_root)
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from kopi_cli.config import detect_install_method, recommended_update_command_for_method
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Kopi Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``kopi-acp`` launcher next to the ``kopi`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``kopi update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``kopi-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Kopi as not installed even when it is.

    The shim simply delegates to the sibling ``kopi`` launcher with the
    ...
    ``kopi-acp.exe`` already resolves) and wherever a ``kopi-acp`` is
    already present next to the ``kopi`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        kopi_cmd = bin_dir / "kopi"
        acp_cmd = bin_dir / "kopi-acp"
        try:
            if not (kopi_cmd.is_file() or kopi_cmd.is_symlink()):
                continue
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Kopi Agent — ACP launcher (written by `kopi update`).\n"
            ...
            continue
        print(f"  ✓ Installed kopi-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of KOPI_HOME under
      ``backups/`` (restorable via ``kopi import``; the #48200 wrong-path
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    calling process and its ancestors are always excluded (a CLI ``kopi
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Kopi processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Kopi Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Kopi desktop app / other Kopi terminals, then re-run:"
    )
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Kopi...")
    try:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``kopi-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("kopi-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``kopi update`` never touched them, so installs created before the
...
def cmd_update(args):
    """Update Kopi Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Kopi Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``kopi
        ...
        # marker survives and the next ``kopi`` launch finishes the install
        ...
        # symbols in kopi_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `kopi` again "
        ...
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            ...
            # debuggable.
            #
            # Start the build subprocess with the Kopi-managed Node on PATH:
            # when `kopi update` runs inside the desktop updater chain
            # (Desktop → kopi-setup → kopi update), the shell PATH
            # customizations are lost, so a bare-PATH child would fail with
            # `node: not found` before cmd_gui can self-heal.
            from kopi_constants import with_kopi_node_path

            _build_env = with_kopi_node_path()
        ...
        # Self-heal the kopi-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Kopi on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("kopi-acp launcher self-heal failed: %s", e)
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `kopi update`
                # must stay fast; `kopi computer-use install --upgrade`
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} kopi dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from kopi_cli.config import load_config, save_config
        from kopi_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from kopi_cli.windows_ssh_runtime import read_token
    ...
    token_root = _get_kopi_home() / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``KOPI_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``kopi dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with kopi serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with KOPI_DESKTOP=1 plus
    # KOPI_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # KOPI_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `kopi dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited KOPI_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # KOPI_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (KOPI_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets KOPI_SERVE_HEADLESS below.
    if os.environ.get("KOPI_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("KOPI_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("KOPI_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("KOPI_SERVE_HEADLESS", None)
    ...
    # the same logs directory as every other Kopi surface.
    try:
        from kopi_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom KOPI_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "KOPI_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # KOPI_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["KOPI_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ KOPI_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset KOPI_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads KOPI_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["KOPI_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from KOPI_WEB_DIST: {_dist_root}")
...
def cmd_console(args):
    """Open the safe Kopi command console."""
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``kopi <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``kopi photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that KOPI_YOLO_MODE is set before ANY
...
def cmd_acp(args):
    """Launch Kopi Agent as an ACP server."""
    try:
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: kopi-ai-agent[otlp])")
...
def main():
    ...
    # If the checkout changed since the last launch (kopi update, manual
    ...
    # `kopi proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://kopi-ai-agent.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from kopi_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``kopi proxy`` subparser (dest='proxy_command').
    ...
    # =========================================================================
    # approvals command  (parser built in kopi_cli/subcommands/approvals.py)
    # =========================================================================
    ...
    # =========================================================================
    # import-agent command  (parser: kopi_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from kopi_cli.agent_import import import_agent_command
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Kopi shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Kopi app. `status` reports the driver's grant\n"
    ...
    def cmd_sessions(args):
        ...
        if action == "repair":
            ...
            if report.get("repaired"):
                ...
                print(f"    kopi sessions recover --source {source_hint} \\")
                print("        --inspect-only")
        ...
        if action == "recover":
            ...
            from kopi_cli.session_recovery import (
        ...
        if action == "list":
            from kopi_state import workspace_key as _ws_key
            ...
            if result.get("vacuumed") is False:
                print("  (VACUUM was skipped or failed — run "
                      "`kopi sessions optimize` later to reclaim freed space.)")
    ...
    # for one release. The Kopi-Setup.exe success screen tells users to
    ...
    # --yolo: set KOPI_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``kopi egress start`` refusing when credential_source=bitwarden
