"""Direct NeMo Relay integration for Kopi shared client metrics."""
...
class _Runtime:
    """Own shared-metrics state layered on the Kopi core Relay host."""

    def __init__(self, host: relay_runtime.RelayRuntime | None = None) -> None:
        resolved_host = host or relay_runtime.get_runtime()
        if resolved_host is None:
            raise RuntimeError("Kopi core Relay runtime is unavailable")
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Kopi task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # physical attempt. Provider fallback resets Kopi's
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Kopi retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Kopi shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Kopi shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Kopi shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_tool_call(
        ...
        try:
            ...
            logger.warning(
                "Kopi shared-metrics tool call close failed",
                exc_info=True,
            )
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Kopi shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Kopi shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Kopi shared metrics operation failed", exc_info=True)
            return None
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Kopi lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning(
            "Kopi shared metrics hook failed: %s", hook_name, exc_info=True
        )


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Kopi's runtime registry."""
...
def start_task_run(
    *,
    ...
    """Start task metrics at the outer Kopi execution boundary."""
    if not enabled():
        return
    ...
    *,
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Kopi shared metrics initialization failed", exc_info=True)
