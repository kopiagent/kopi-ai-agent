"""Bounded product contract for the first Kopi shared-metrics slice."""
...
SCHEMA_KEY = "kopi.metrics.schema_version"
SCHEMA_VERSION = "kopi.metrics.event.v2"
MODEL_CALL_SCOPE = "kopi.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "kopi.task_run"
TOOL_CALL_SCOPE = "kopi.tool_call"
TOOL_APPROVAL_MARK = "kopi.tool_approval"
SUBSCRIBER_NAME = "kopi.nemo_relay.shared_metrics"
LEGACY_MODEL_CALL_METRIC = "kopi.model_call.count"
MODEL_ROUTE_METRIC = "kopi.model_route.count"
TASK_STARTED_METRIC = "kopi.task_run.started"
TASK_FINISHED_METRIC = "kopi.task_run.finished"
TOOL_CALL_METRIC = "kopi.tool_call.count"
TOOL_APPROVAL_METRIC = "kopi.tool_approval.count"
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Kopi logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("kopi.call_role")
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Kopi terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Kopi registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "kopi-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Kopi tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Kopi."""
