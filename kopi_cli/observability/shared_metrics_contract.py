...
SCHEMA_VERSION = "kopi.metrics.event.v1"
...
MODEL_CALL_METRIC = "kopi.model_call.count"
...
# in Kopi's execution metadata because that flag has narrower routing/catalog
# semantics there.
...
# Kopi intentionally resolves these local runtimes through the generic custom
...
def provider_family(kwargs: dict[str, Any]) -> str:
    """Map a Kopi provider to a bounded product category."""
...
def _provider_metadata(provider: str) -> tuple[str, bool, bool]:
    ...
    try:
        ...
        from kopi_cli.providers import KOPI_OVERLAYS, normalize_provider
...
def _known_provider_ids() -> frozenset[str]:
    """Cache Kopi's static provider catalog for the process lifetime."""
    try:
        from kopi_cli.provider_catalog import provider_catalog_by_slug
