...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Kopi'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Kopi' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Kopi for
        ...
        try:
            ...
            # Permissions + Kopi tool callback are always-on production
            ...
            if "kopi-tools" in mig_report.migrated:
                msg_lines.append(
                    "Kopi tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Kopi runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Kopi tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Kopi runtime.")
