"""
Kopi Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Kopi.app`` or ``~/Applications/Kopi.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Kopi`` (NSIS per-user)
...
the app name ("Kopi"), separate from ``$KOPI_HOME``:
  - macOS:   ``~/Library/Application Support/Kopi``
  - Windows: ``%APPDATA%\\Kopi``
  - Linux:   ``$XDG_CONFIG_HOME/Kopi`` (default ``~/.config/Kopi``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Kopi"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Kopi"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Kopi" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Kopi.app"),
            home / "Applications" / "Kopi.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Kopi).
            local_base / "Programs" / "Kopi",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Kopi")
    else:
