"""Offline, non-destructive recovery for a damaged Kopi session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``kopi_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``kopi_state._backup_db_file``.
    """
    from kopi_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Kopi process using this profile and retry."
            )
