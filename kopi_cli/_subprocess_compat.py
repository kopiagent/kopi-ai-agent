...
# Electron exits, so the gateway never gets respawned after a `kopi
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Kopi' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Kopi shells out to git from many non-interactive contexts — MCP catalog
