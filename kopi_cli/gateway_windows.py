...
_TASK_DESCRIPTION = "Kopi Agent Gateway - Messaging Platform Integration"
...
def _preserve_kopi_home_path(path: str | Path) -> str:
    """Render Kopi-owned paths under the configured KOPI_HOME spelling.
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Kopi profile."""
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m kopi_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``kopi gateway run``.
    ...
    prog_args = [python_exe_path, "-m", "kopi_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Kopi builds and failed
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (KOPI_HOME, VIRTUAL_ENV,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m kopi_cli.main gateway run`` directly — NOT
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Kopi gateway install prompt.")
                if start_now:
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Kopi gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: kopi gateway status")
                    return
