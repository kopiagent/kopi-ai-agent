...
``cli.py``'s ``KopiCLI`` class. ``KopiCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Kopi changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Kopi hasn't edited any files here yet.")
            return
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from kopi_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `kopi profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: kopi -p {imported}")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        # same directory as a startup `kopi -c`/`--resume`. The startup resume
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Kopi-managed isolated debug "
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from kopi_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from kopi_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from kopi_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from kopi_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                ...
                behavior = "Enter will redirect the current run while Kopi is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from kopi_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Kopi Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Kopi Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Kopi Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="☕  Update Kopi Agent",
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Kopi' hotword listener.
