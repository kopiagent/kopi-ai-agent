"""Dashboard process-hygiene helpers — extracted from ``kopi_cli/main.py``.
...
``_detect_concurrent_kopi_instances``) are lifted verbatim. References to
helpers that STAY in ``kopi_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``kopi_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``kopi_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``kopi_cli.main`` reference (call-time; keeps patches working)."""
...
def _detect_concurrent_kopi_instances(
    ...
    for shim in _m()._kopi_exe_shims(scripts_dir):
        try:
