...
# load_kopi_dotenv can call _sanitize_env_file_if_needed multiple times
...
def _known_kopi_env_keys() -> set[str]:
    """Return the combined set of known Kopi env-var keys.
    ...
    ``load_kopi_dotenv()`` calls.
    """
    from kopi_cli.config import _EXTRA_ENV_KEYS
    from kopi_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Kopi process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Kopi keys absent from ``.env``.
    ...
    parent Kopi process injects and that silently change *which provider
    ...
    ``tests/kopi_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source_values(
    kopi_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``kopi_home``."""
    home_key = str(Path(kopi_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(kopi_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_kopi_dotenv()'s .op.env bootstrap: the 1Password
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``kopi_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_kopi_dotenv(
    *,
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Kopi keys
    ...
    # TERMINAL_ENV=docker left in ~/.kopi/.env (e.g. written by an older
    # `kopi setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_kopi_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``kopi_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process KOPI_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_kopi_dotenv(kopi_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_kopi_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Kopi reads
    ...
    if not cfg:
        ...
        # on its next load_kopi_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_kopi_dotenv() calls per startup don't
