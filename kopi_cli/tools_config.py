"""
Unified tool configuration for Kopi Agent.
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``kopi tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`kopi_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
# Config-only capabilities: they appear in `kopi tools` for provider/API-key
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Kopi routes X searches through xAI's built-in x_search "
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Kopi telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $KOPI_HOME/bin is never on PATH, so a bare which()
    # misses the uv Kopi installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    falling through to the full upstream installer. ``kopi update`` sets
    ...
    a further ~600s wait on Windows). ``kopi computer-use install
    ...
    ``kopi update`` already prints a contextual line before its update
    ...
    # caller: `kopi update` (require_confirmed_update=True) keeps the
    ...
    # `kopi computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: kopi computer-use install --upgrade"
            )
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $KOPI_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # Kopi installed for exactly this toolchain.
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.kopi/config.yaml")
            else:
...
#:
#: Saving ``kopi tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[kopi-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``kopi tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"kopi-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (kopi-acp, kopi-webhook) stay narrow.
...
def _agent_browser_installed() -> bool:
    ...
    from kopi_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``kopi tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from kopi_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (kopi_cli/web_server.py
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``kopi model`` aux-task
