...
Diagnoses issues with Kopi Agent setup.
"""
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. KOPI_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("KOPI_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("KOPI_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `kopi doctor --fix`, or "
...
def run_doctor(args):
    ...
    try:
        import sqlite3
        from kopi_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Kopi already refuses to enable WAL on fresh DBs.
    ...
    if env_path.exists():
        ...
        # matches kopi_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    if config_path.exists():
        ...
        try:
            ...
            if isinstance(user_providers, dict):
                from kopi_cli.config import is_provider_enabled
    ...
    if config_path.exists():
        ...
        try:
            from kopi_cli.config import load_env, read_user_config_raw, remove_env_value
        ...
        try:
            from kopi_cli.config import load_env as _load_env_depr
            from kopi_cli.config import read_user_config_raw as _read_raw_depr
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'kopi-ai-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'kopi-ai-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `kopi acp --setup-browser` installs agent-browser into the
        # Kopi-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_kopi_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(KOPI_HOME / "node" / "bin")
            or _which_in(KOPI_HOME / "node")
        )
        _legacy_ab = _which_in(KOPI_HOME / "node_modules" / ".bin")
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Kopi one.
                        check_info(
    ...
    try:
        from kopi_cli.config import read_user_config_raw as _read_raw_mem
