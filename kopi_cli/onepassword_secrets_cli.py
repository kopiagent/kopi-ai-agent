...
the CLI through OS package managers and signed installers, so Kopi expects
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Kopi resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Kopi will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Takes effect on the next Kopi invocation."
    )
    if not op_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the 1Password integration is currently disabled — "
            "run `kopi secrets onepassword setup` to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "[cyan]kopi[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
