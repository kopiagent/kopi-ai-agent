...
manual BotFather token copy-paste. Kopi talks only to the Nous onboarding
...
DEFAULT_MANAGER_BOT = "KopiSetupBot"

DEFAULT_BOT_NAME = "Kopi Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Kopi Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Kopi Telegram onboarding service.")
