...
DESKTOP_CRON_STARTUP_DELAY_SECONDS = 30
...
def _start_desktop_cron_ticker(
    ...
    *,
    ...
    if startup_delay is None:
        startup_delay = DESKTOP_CRON_STARTUP_DELAY_SECONDS
    provider = resolve_cron_scheduler()
    _log.info(
        "Desktop cron scheduler started (provider=%s, interval=%ds, startup_delay=%.1fs)",
        provider.name,
        interval,
        startup_delay,
    )
...
def _warm_gateway_module() -> None:
    ...
    ``kopi_cli.gateway``. But the first WS connection and its initial
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import kopi_cli.gateway eagerly *before* the lifespan yield so the
...
def _resolve_session_token() -> str:
    return os.environ.get("KOPI_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `kopi egress setup` "
            "and `kopi egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
...
from kopi_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Kopi
...
from kopi_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from kopi_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # KOPI_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_kopi_home())
            )
        ...
        try:
            from kopi_state import SessionDB as _SDB
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from kopi_cli.auth import get_nous_auth_status_local
...
def _spawn_kopi_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    # _KOPI_GATEWAY=1. Inheriting it makes a spawned `kopi gateway restart`
    ...
    action_env = {**os.environ, "KOPI_NONINTERACTIVE": "1"}
    action_env.pop("_KOPI_GATEWAY", None)
...
from kopi_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from kopi_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from kopi_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from kopi_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/kopi is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (KOPI_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
async def get_egress_status():
    ...
    from kopi_cli.proxy_cli import format_status_text
...
async def get_model_options(
    ...
    """
    try:
        from kopi_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from kopi_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from kopi_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from kopi_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from kopi_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "teams": {
        "description": "Connect Kopi to Microsoft Teams chats via the Bot Framework.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Kopi.",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Kopi from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Kopi over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Kopi through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Kopi over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Kopi via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Kopi Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
# kopi_cli/setup_hidden_env.py. Shared with the `kopi setup gateway`
# wizard so the surfaces ask for the same things.
from kopi_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # KOPI_HOME contextvar override (#56986 / #69143), so the profile's
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the KOPI_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized KOPI_HOME)
...
def _open_session_db_for_profile(profile: Optional[str], *, read_only: bool):
    ...
    from kopi_state import SessionDB, _default_db_path, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``kopi serve`` — it runs neither the
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (KOPI_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.kopi``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    KOPI_HOME outside the profiles tree) has no profile-dir equivalent, so
...
from kopi_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from kopi_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
from kopi_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from kopi_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from kopi_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.kopi_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_kopi_home_override(flow.kopi_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.kopi_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = KopiTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        kopi_home=flow.kopi_home,
                    )
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `kopi auth remove`) can seed again.
        # Mirrors the `kopi auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``kopi auth remove`` uses: each source cleans up its
...
from kopi_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from kopi_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from kopi_constants import set_kopi_home_override, reset_kopi_home_override
    from kopi_cli.mcp_config import _save_bearer_auth_token
...
from kopi_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from kopi_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_chat_argv(
    ...
    """
    from kopi_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Kopi TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides KOPI_HOME afterwards.
    from tools.environments.local import build_subprocess_env
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        engine = KopiConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("KOPI_TUI_RESUME") or raw_resume
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Kopi Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``KOPI_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Kopi Teal canvas, then
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_kopi_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``KOPI_HOME``.
    """
    themes_dir = get_process_kopi_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    # KOPI_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_kopi_home() / "plugins", "user"),
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                ...
                from kopi_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`kopi plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
