"""Kopi-managed uv and Python runtime repair.

Kopi owns its own uv binary at ``$KOPI_HOME/bin/uv`` (or ``uv.exe`` on
...
The Python backing the install is different: it is shared by every Kopi
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.kopi-runtime/python``. A
...
from kopi_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".kopi-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Kopi keeps *its* uv binary.
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Kopi-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Kopi keeps databases "
            "out of WAL mode on this SQLite build. The next `kopi update` "
            "will retry."
        )
        return
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``kopi update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under KOPI_HOME
    caches the last successful self-update time.
    """
    try:
        from kopi_constants import get_kopi_home

        stamp = get_kopi_home() / "cache" / ".uv_self_update_stamp"
...
def _reload_kopi_constants():
    """Re-execute ``kopi_constants`` from disk and return the fresh module.

    ``kopi update`` imports ``kopi_constants`` from the OLD checkout,
    ...
    return importlib.reload(kopi_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Kopi generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("kopi_cli.main")
    ...
    if holders:
        ...
        return True, f"other Kopi processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Kopi-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``kopi update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``kopi update`` repairs the runtime (issue class:
