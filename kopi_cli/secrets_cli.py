...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Kopi process."
    )
    console.print(
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Kopi will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `kopi secrets bitwarden setup` to pick a "
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "run `kopi secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]kopi[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
