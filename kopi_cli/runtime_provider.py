...
from kopi_cli.providers import custom_provider_aliases, custom_provider_slug
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``KOPI_INFERENCE_PROVIDER`` equivalent). When neither
...
def resolve_runtime_provider(
    *,
    ...
    from kopi_cli.config import is_provider_enabled, load_config
