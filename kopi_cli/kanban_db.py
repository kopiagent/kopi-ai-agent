...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``kopi_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from kopi_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("KOPI_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
class Task:
    ...
    # ``kopi_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``kopi_cli.sqlite_safe_read``).
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see kopi_cli.sqlite_safe_read), so it must only run once the board has
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Kopi treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``kopi kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    with _cross_process_init_lock(path):
        ...
        from kopi_state import preflight_db_writability
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from kopi_cli.sqlite_safe_read import file_length_matches_header
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``kopi kanban attach``) so name-sanitisation, the size cap, and the
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Kopi' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Kopi-managed storage. A board ``default_workdir`` pointing at a
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Kopi checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Kopi treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Kopi creates/reuses that linked worktree. With
      no ``workspace_path``, Kopi anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Kopi uses
      ``wt/<task-id>``.
...
def _module_kopi_argv() -> list[str]:
    """Return the interpreter-bound Kopi CLI invocation."""
...
def _absolute_kopi_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Kopi shim."""
...
def _kopi_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Kopi executable path.
...
def _default_spawn(
    ...
    *,
    ...
    # read on the board and in `kopi kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["KOPI_SESSION_SOURCE"] = "kanban"
    ...
    _retag_legacy_worker_sessions(env["KOPI_KANBAN_WORKSPACES_ROOT"])
    ...
    # A worker must NEVER boot the interactive TUI: an inherited KOPI_TUI=1
    ...
    # older kopi builds on PATH that predate the flag's precedence.
    env.pop("KOPI_TUI", None)
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Kopi Agent or activate its venv before running the kanban dispatcher."
        )
