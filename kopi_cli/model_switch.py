...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``kopi model`` wizard — not a
    ...
    while ``kopi model`` still live-probed the full ``/v1/models`` list.
...
async def resolve_display_context_length_async(
    ...
    ``kopi_cli.route_identity.should_clear_context_pin_async``, which
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Compared against kopi_id / mdev_id (section 1), pid / kopi_slug
    ...
    for kopi_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip kopi_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose kopi_id matches the canonical profile name so
        ...
        # same kopi_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if kopi_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
    ...
    for pid, overlay in KOPI_OVERLAYS.items():
        ...
        if kopi_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or kopi_slug.lower() in _excluded:
            continue
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            if group_key not in ep_groups:
                ...
                # Kopi's own writer uses (mirrors section-4 grouping).
            ...
            # ``kopi model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``kopi model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``kopi model``. Pin a dict catalog with
