"""kopi import-agent — import Claude Code / Codex CLI setups into Kopi.

Usage:
    kopi import-agent                       # auto-detect ~/.claude or ~/.codex
    kopi import-agent claude-code           # import from ~/.claude
    kopi import-agent codex                 # import from ~/.codex
    kopi import-agent claude-code --dry-run # preview only, no changes
    kopi import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``kopi claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in KOPI_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → KOPI_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in KOPI_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in KOPI_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → KOPI_HOME/skills/codex-imports/<name>/
...
deliberately via ``kopi setup`` or config.yaml.
"""
...
# Same entry delimiter as the Kopi memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `kopi config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`kopi config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_kopi.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Kopi command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Kopi glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Kopi equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → KOPI_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``kopi import-agent`` (invoked from kopi_cli.main)."""
    from kopi_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: kopi import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: kopi import-agent claude-code   or   kopi import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Kopi — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: kopi import-agent "
    ...
    print_info("Secrets:     never imported — run 'kopi setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: kopi import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'kopi setup' "
               "to configure providers, or add them to ~/.kopi/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from kopi_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'kopi setup' or ~/.kopi/.env.")
        print()
