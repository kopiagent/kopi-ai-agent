"""``kopi sync`` subcommand parser — Skill Sync.

Cloned from ``kopi_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    kopi sync status                 show gate/opt-in/head state
    kopi sync pull                   pull and materialize opted-in skills
    kopi sync push                   push opted-in skills
    kopi sync now                    reconcile: pull then push
    kopi sync enable <skill>         opt a skill into sync
    kopi sync disable <skill>        opt a skill out of sync
    kopi sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    kopi sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  kopi sync status            what is synced, and from where\n"
            "  kopi sync enable my-skill   include a skill in your sync\n"
            "  kopi sync now               pull, then push\n"
            "  kopi sync propose my-skill  share a skill with your team\n"
        ),
