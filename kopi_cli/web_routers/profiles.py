...
late-binding seam in :mod:`kopi_cli.web_deps` so tests that
...
from kopi_cli.web_deps import late
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps kopi_cli.profiles.export_profile /
# import_profile (the same machinery behind `kopi profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_kopi_home() / "profile-exports"
        try:
