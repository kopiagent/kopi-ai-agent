...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Kopi-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Kopi that doesn't ship
...
#
# Some vendors expose several Kopi provider slugs (one per endpoint /
...
# In-repo fallback for the model Kopi silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from kopi_cli.model_catalog import get_default_model_from_cache
...
# ``kopi model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in kopi_cli/web_server.py and
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Kopi lands on when none is selected.
            desc = "default"
        else:
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from kopi_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Kopi release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``kopi model`` shows.
            from kopi_cli.providers import is_official_openai_host
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # env var, so it must change the fingerprint too or `kopi config set
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Kopi' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"kopi-ai-agent/{_KOPI_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=kopi"
    ...
    *,
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Kopi actually ships.  (#46850)
            #
