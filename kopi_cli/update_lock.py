"""Cross-process mutual exclusion for in-flight Kopi updates.

Three different surfaces can start an update of the same install tree:

* ``kopi update`` from a terminal,
* the dashboard's Update button (``POST /api/kopi/update`` →
  ``_spawn_kopi_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``kopi-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``kopi update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <KOPI_HOME>/.kopi-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``kopi update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Kopi is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``kopi-setup`` binary under ``~/.kopi`` is only refreshed by a full
  installer run (``copy_self_to_kopi_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Kopi is still running") on every
  GUI update forever, with no Kopi process actually running.
"""
...
MARKER_NAME = ".kopi-update-in-progress"

# Set by an orchestrating updater (the Tauri `kopi-setup --update` flow) to
# its own pid before spawning `kopi update` as a child stage. The parent
...
HANDOFF_PID_ENV = "KOPI_UPDATE_HANDOFF_PID"
...
# to show "Kopi is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Kopi home (never the context-local profile override):
    the Rust updater resolves ``$KOPI_HOME`` or the platform default, and the
    ...
    """
    from kopi_constants import get_process_kopi_home

    return get_process_kopi_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``kopi update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``kopi-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Kopi update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `kopi update` as a stage): we run under ITS claim
