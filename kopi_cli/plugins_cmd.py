...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``kopi setup`` / ``kopi plugins disable basic`` can park the plugin
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Kopi core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Kopi
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    ...
    # Sibling of the CLI ``kopi plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``kopi_cli.main`` never covers them. After a
