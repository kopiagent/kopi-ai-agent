...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("KOPI_CRON_SESSION"):
        return False
    if (os.getenv("KOPI_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Kopi-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Kopi session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
