...
def _scan_gateway_pids(
    ...
    # Forward slashes on both sides of the KOPI_HOME= match — see
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``kopi update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Kopi's detached watcher as well would escape the
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # interpreter and captures the stable cwd + env overlay (KOPI_HOME,
...
def _kopi_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``KOPI_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_kopi_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = (_kopi_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("KOPI_HOME", "").strip()
    if not unit_home:
        return
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Kopi Docker image — that one runs s6-overlay as
...
SERVICE_DESCRIPTION = "Kopi Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(kopi_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Kopi root to compare against. Used when
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # kopi_constants cached from before this symbol existed. See
            # _reload_kopi_constants() in kopi_cli/managed_uv.py.
            from kopi_cli.managed_uv import _reload_kopi_constants

            venv_python_path = _reload_kopi_constants().venv_python_path
...
def _kopi_home_for_target_user(target_home_dir: str) -> str:
    ...
    """
    current_kopi_raw = os.environ.get("KOPI_HOME", "").strip()
    current_kopi = (
        Path(current_kopi_raw).expanduser()
        if current_kopi_raw
        else get_kopi_home()
    )
    ...
    # different KOPI_HOME into the generated service unit.
    current_default = Path.home() / ".kopi"
...
def _systemd_watchdog_seconds(kopi_home: str | Path | None = None) -> int:
    ...
    if kopi_home is not None:
        ...
        override_token = set_kopi_home_override(kopi_home)
        reset_home_override = reset_kopi_home_override
    try:
...
def _systemd_watchdog_service_fields(
    kopi_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(kopi_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], kopi_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Kopi-managed Node under ``$KOPI_HOME/node`` goes first when it
    ...
    *kopi_root* is the Kopi home the unit will run against. System units
    ...
    """
    from kopi_constants import iter_kopi_node_dirs

    for directory in iter_kopi_node_dirs(kopi_root):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Kopi home is known — probing here would stat the calling
    ...
    if system:
        ...
        # the remapped kopi_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(kopi_home) if kopi_home else None
        )
        path_entries = [
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m kopi_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="KOPI_HOME={kopi_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned KOPI_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo kopi gateway … --system``, KOPI_HOME is often stripped
    # and falls back to ``/root/.kopi``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / KOPI_HOME comparisons
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single KOPI_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Kopi install"
    )
    return True
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("KOPI_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Existing system units already pin KOPI_HOME; adopt it before any
    ...
    # ``sudo kopi gateway install --system --force`` would bake /root/.kopi
...
def systemd_start(system: bool = False):
    ...
    # KOPI_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Kopi install"
    )
    return True
...
def launchd_status(deep: bool = False):
    ...
    # Kopi PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Kopi PID file).
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # the env vars ``KOPI_GATEWAY_MAX_STARTS`` /
    # ``KOPI_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("KOPI_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("KOPI_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``kopi gateway run`` enters through this CLI wrapper, not through
...
def _setup_standard_platform(platform: dict):
    ...
    from kopi_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _setup_weixin():
    ...
    print_info(
        "  3. Kopi will store the returned account_id/token in ~/.kopi/.env."
    )
    print_info(
